"""
Bloxfruits Autofish - Made by AsphaltCake

RESOLUTION & SCALING COMPATIBILITY:
- DPI-aware: Works correctly with Windows display scaling (100%, 125%, 150%, etc.)
- All resolutions: Automatically scales from 1920x1080 baseline to any resolution
- Multi-monitor: Uses primary monitor, handles different aspect ratios
- Physical pixels: All coordinates use actual screen pixels, not logical pixels
- Tested: 1366x768, 1920x1080, 2560x1440, 3840x2160 (4K)
"""
...
# DPI AWARENESS SETUP
# ============================================================================
# Make program DPI aware for Windows to handle display scaling correctly
# This ensures coordinates, screenshots, and dxcam regions all use physical pixels
# Without this, coordinates would be in logical pixels causing misalignment
try:
    # Windows 8.1+: Per-monitor DPI awareness
    ctypes.windll.shcore.SetProcessDpiAwareness(2)  # PROCESS_PER_MONITOR_DPI_AWARE
except:
    try:
        # Windows Vista/7: System DPI awareness
        ctypes.windll.user32.SetProcessDPIAware()
    except:
        # DPI awareness not supported (very old Windows)
        pass
...
def get_actual_screen_size():
    """Get actual screen size in physical pixels (accounts for DPI scaling)"""
    return (
        ctypes.windll.user32.GetSystemMetrics(0),  # Width
        ctypes.windll.user32.GetSystemMetrics(1)   # Height
    )
...
class DualAreaSelector:
    ...
    def __init__(self, parent, screenshot, indicator_box, fish_box, callback):
        ...
        # Get actual physical screen dimensions (not logical)
        self.screen_width, self.screen_height = get_actual_screen_size()
        
        print(f"[AREA_SELECTOR] Screenshot size: {screenshot.size}")
        print(f"[AREA_SELECTOR] Physical screen: {self.screen_width}x{self.screen_height}")
...
class CentralStorage:
    ...
    def __init__(self, config_file="bloxfruits_config.json"):
        ...
        # Using GetSystemMetrics for accurate physical resolution
        screen_width = ctypes.windll.user32.GetSystemMetrics(0)
        screen_height = ctypes.windll.user32.GetSystemMetrics(1)
        
        print(f"[INIT] Detected screen resolution: {screen_width}x{screen_height}")
        ...
        print(f"[INIT] Scale factors - X: {scale_x:.3f}, Y: {scale_y:.3f}")
        ...
        self.settings = {
            ...
            "detection_settings": {
                ...
                "kd": 0.5,
                "click_duration": 1.0,
                "cast_hold_duration": 0.8,
                "cast_release_delay": 1.0
            },
    ...
    def get_feature_toggle(self, toggle_name):
        ...
        defaults = {"get_chest": True, "new_fish_wait": True}
...
class BloxfruitsAutofishGUI:
    def __init__(self, root):
        ...
        # Get actual screen info for validation
        actual_width, actual_height = get_actual_screen_size()
        print(f"[GUI] Running on {actual_width}x{actual_height} display (physical pixels)")
    ...
    def setup_gui(self):
        ...
        # ============ CONTROLS SECTION ============
        controls_frame = tk.LabelFrame(self.root, text="Controls", font=("Arial", 10, "bold"),
                                       padx=10, pady=5)
        controls_frame.pack(pady=5, padx=10, fill="x")
        
        # Start/Stop button
        self.start_stop_frame = tk.Frame(controls_frame)
        ...
        # Change Area button
        self.change_area_frame = tk.Frame(controls_frame)
        ...
        # Exit button
        self.exit_frame = tk.Frame(controls_frame)
        ...
        # ============ KEYBINDS SECTION ============
        keybinds_frame = tk.LabelFrame(self.root, text="Keybinds", font=("Arial", 10, "bold"),
                                       padx=10, pady=5)
        keybinds_frame.pack(pady=5, padx=10, fill="x")
        
        # Not Rod Keybind
        not_rod_label = tk.Label(keybinds_frame, text="Not Rod:", font=("Arial", 9))
        ...
        not_rod_combo = ttk.Combobox(keybinds_frame, textvariable=self.not_rod_var,
        ...
        # Rod Keybind
        rod_label = tk.Label(keybinds_frame, text="Rod:", font=("Arial", 9))
        rod_label.grid(row=0, column=2, padx=5, sticky="w")
        ...
        rod_combo = ttk.Combobox(keybinds_frame, textvariable=self.rod_var,
                                values=["N/A", "1", "2", "3", "4", "5", "6", "7", "8", "9"],
                                width=5, font=("Arial", 9), state="readonly")
        rod_combo.grid(row=0, column=3, padx=5)
        rod_combo.bind('<<ComboboxSelected>>', lambda e: self.on_rod_change())
        
        # ============ CAST/REEL SETTINGS ============
        cast_reel_frame = tk.LabelFrame(self.root, text="Cast & Reel Settings", font=("Arial", 10, "bold"),
                                        padx=10, pady=5)
        cast_reel_frame.pack(pady=5, padx=10, fill="x")
        
        # Lure timeout
        lure_label = tk.Label(cast_reel_frame, text="Lure Timeout:", font=("Arial", 9))
        lure_label.grid(row=0, column=0, padx=5, sticky="w")
        
        self.timeout_var = tk.StringVar(value=str(self.storage.get_detection_setting('lure_timeout')))
        timeout_spinbox = tk.Spinbox(cast_reel_frame, from_=1, to=60, 
                                    textvariable=self.timeout_var,
                                    width=5, font=("Arial", 9),
                                    command=self.on_timeout_change)
        timeout_spinbox.grid(row=0, column=1, padx=5)
        timeout_spinbox.bind('<Return>', lambda e: self.on_timeout_change())
        timeout_spinbox.bind('<FocusOut>', lambda e: self.on_timeout_change())
        
        lure_unit = tk.Label(cast_reel_frame, text="sec", font=("Arial", 9))
        lure_unit.grid(row=0, column=2, sticky="w")
        
        # Color tolerance
        color_label = tk.Label(cast_reel_frame, text="Tolerance:", font=("Arial", 9))
        ...
        tolerance_spinbox = tk.Spinbox(cast_reel_frame, from_=0, to=50, 
                                      textvariable=self.tolerance_var,
                                      width=5, font=("Arial", 9),
                                      command=self.on_tolerance_change)
        tolerance_spinbox.grid(row=0, column=4, padx=5)
        ...
        # Row 2: Cast timing
        cast_hold_label = tk.Label(cast_reel_frame, text="Cast Hold:", font=("Arial", 9))
        cast_hold_label.grid(row=1, column=0, padx=5, sticky="w")
        
        self.cast_hold_var = tk.StringVar(value=str(self.storage.get_detection_setting('cast_hold_duration')))
        cast_hold_spinbox = tk.Spinbox(cast_reel_frame, from_=0.1, to=10, increment=0.1,
                                       textvariable=self.cast_hold_var,
                                       width=5, font=("Arial", 9),
                                       command=lambda: self.on_cast_timing_change('cast_hold_duration', self.cast_hold_var))
        cast_hold_spinbox.grid(row=1, column=1, padx=5)
        cast_hold_spinbox.bind('<Return>', lambda e: self.on_cast_timing_change('cast_hold_duration', self.cast_hold_var))
        cast_hold_spinbox.bind('<FocusOut>', lambda e: self.on_cast_timing_change('cast_hold_duration', self.cast_hold_var))
        
        cast_hold_unit = tk.Label(cast_reel_frame, text="sec", font=("Arial", 9))
        cast_hold_unit.grid(row=1, column=2, sticky="w")
        
        cast_delay_label = tk.Label(cast_reel_frame, text="Release Delay:", font=("Arial", 9))
        cast_delay_label.grid(row=1, column=3, padx=5, sticky="w")
        
        self.cast_delay_var = tk.StringVar(value=str(self.storage.get_detection_setting('cast_release_delay')))
        cast_delay_spinbox = tk.Spinbox(cast_reel_frame, from_=0.1, to=10, increment=0.1,
                                        textvariable=self.cast_delay_var,
                                        width=5, font=("Arial", 9),
                                        command=lambda: self.on_cast_timing_change('cast_release_delay', self.cast_delay_var))
        cast_delay_spinbox.grid(row=1, column=4, padx=5)
        cast_delay_spinbox.bind('<Return>', lambda e: self.on_cast_timing_change('cast_release_delay', self.cast_delay_var))
        cast_delay_spinbox.bind('<FocusOut>', lambda e: self.on_cast_timing_change('cast_release_delay', self.cast_delay_var))
        
        cast_delay_unit = tk.Label(cast_reel_frame, text="sec", font=("Arial", 9))
        cast_delay_unit.grid(row=1, column=5, sticky="w")
        
        # ============ MINIGAME SETTINGS ============
        minigame_frame = tk.LabelFrame(self.root, text="Minigame Settings", font=("Arial", 10, "bold"),
                                       padx=10, pady=5)
        minigame_frame.pack(pady=5, padx=10, fill="x")
        
        # Row 1: Fish timeout and tolerances
        fish_timeout_label = tk.Label(minigame_frame, text="Fish Timeout:", font=("Arial", 9))
        fish_timeout_label.grid(row=0, column=0, padx=5, sticky="w")
        
        self.fish_timeout_var = tk.StringVar(value=str(self.storage.get_detection_setting('fish_timeout')))
        fish_timeout_spinbox = tk.Spinbox(minigame_frame, from_=1, to=60, 
                                         textvariable=self.fish_timeout_var,
                                         width=5, font=("Arial", 9),
                                         command=self.on_fish_timeout_change)
        fish_timeout_spinbox.grid(row=0, column=1, padx=5)
        ...
        fish_timeout_unit = tk.Label(minigame_frame, text="sec", font=("Arial", 9))
        fish_timeout_unit.grid(row=0, column=2, sticky="w")
        
        # Get Chest checkbox
        self.get_chest_var = tk.BooleanVar(value=self.storage.get_feature_toggle('get_chest'))
        get_chest_check = tk.Checkbutton(minigame_frame, text="Get Chest", 
                                         variable=self.get_chest_var,
                                         font=("Arial", 9),
                                         command=self.on_get_chest_toggle)
        get_chest_check.grid(row=0, column=3, padx=10)
        
        # Row 2: Color Tolerances
        tk.Label(minigame_frame, text="Color Tolerances:", font=("Arial", 9, "bold")).grid(row=1, column=0, columnspan=5, sticky="w", pady=(5,2))
        
        # Bar tolerance
        bar_label = tk.Label(minigame_frame, text="Bar:", font=("Arial", 9))
        bar_label.grid(row=2, column=0, padx=5, sticky="w")
        
        self.bar_tol_var = tk.StringVar(value=str(self.storage.get_detection_setting('bar_tolerance')))
        bar_tol_spinbox = tk.Spinbox(minigame_frame, from_=0, to=50, 
        ...
        bar_tol_spinbox.grid(row=2, column=1, padx=2, sticky="w")
        ...
        # Arrow tolerance
        arrow_tol_label = tk.Label(minigame_frame, text="Arrow:",
                                   font=("Arial", 9))
        arrow_tol_label.grid(row=2, column=2, padx=5, sticky="w")
        
        self.arrow_tol_var = tk.StringVar(value=str(self.storage.get_detection_setting('arrow_tolerance')))
        arrow_tol_spinbox = tk.Spinbox(minigame_frame, from_=0, to=50, 
                                       textvariable=self.arrow_tol_var,
                                       width=4, font=("Arial", 9),
                                       command=lambda: self.on_specific_tolerance_change('arrow_tolerance', self.arrow_tol_var))
        arrow_tol_spinbox.grid(row=2, column=3, padx=2, sticky="w")
        ...
        # Fish tolerance
        fish_tol_label = tk.Label(minigame_frame, text="Fish:", 
                                  font=("Arial", 9))
        fish_tol_label.grid(row=3, column=0, padx=5, sticky="w")
        
        self.fish_tol_var = tk.StringVar(value=str(self.storage.get_detection_setting('fish_tolerance')))
        fish_tol_spinbox = tk.Spinbox(minigame_frame, from_=0, to=50, 
                                      textvariable=self.fish_tol_var,
                                      width=4, font=("Arial", 9),
                                      command=lambda: self.on_specific_tolerance_change('fish_tolerance', self.fish_tol_var))
        fish_tol_spinbox.grid(row=3, column=1, padx=2, sticky="w")
        ...
        # Treasure tolerance
        treasure_tol_label = tk.Label(minigame_frame, text="Treasure:", 
                                      font=("Arial", 9))
        treasure_tol_label.grid(row=3, column=2, padx=5, sticky="w")
        
        self.treasure_tol_var = tk.StringVar(value=str(self.storage.get_detection_setting('treasure_tolerance')))
        treasure_tol_spinbox = tk.Spinbox(minigame_frame, from_=0, to=50, 
        ...
        treasure_tol_spinbox.grid(row=3, column=3, padx=2, sticky="w")
        ...
        # Row 3: PD Controller
        tk.Label(minigame_frame, text="PD Controller:", font=("Arial", 9, "bold")).grid(row=4, column=0, columnspan=5, sticky="w", pady=(5,2))
        
        kp_label = tk.Label(minigame_frame, text="Kp:", font=("Arial", 9))
        kp_label.grid(row=5, column=0, padx=5, sticky="w")
        
        self.kp_var = tk.StringVar(value=str(self.storage.get_detection_setting('kp')))
        kp_spinbox = tk.Spinbox(minigame_frame, from_=0, to=10, increment=0.1,
                                textvariable=self.kp_var,
                                width=5, font=("Arial", 9),
                                command=lambda: self.on_pd_change('kp', self.kp_var))
        kp_spinbox.grid(row=5, column=1, padx=5)
        ...
        kd_label = tk.Label(minigame_frame, text="Kd:", font=("Arial", 9))
        kd_label.grid(row=5, column=2, padx=5, sticky="w")
        
        self.kd_var = tk.StringVar(value=str(self.storage.get_detection_setting('kd')))
        kd_spinbox = tk.Spinbox(minigame_frame, from_=0, to=10, increment=0.1,
                                textvariable=self.kd_var,
                                width=5, font=("Arial", 9),
                                command=lambda: self.on_pd_change('kd', self.kd_var))
        kd_spinbox.grid(row=5, column=3, padx=5)
        ...
        # ============ POST-MINIGAME SETTINGS ============
        post_frame = tk.LabelFrame(self.root, text="Post-Minigame Settings", 
        ...
        new_fish_wait_check = tk.Checkbutton(post_frame, text="Enable New Fish Wait", 
                                             variable=self.new_fish_wait_var,
                                             font=("Arial", 9, "bold"),
                                             command=self.on_new_fish_wait_toggle)
        new_fish_wait_check.grid(row=0, column=0, columnspan=3, sticky="w", pady=(0,5))
        
        click_dur_label = tk.Label(post_frame, text="Click Duration:", font=("Arial", 9))
        click_dur_label.grid(row=1, column=0, padx=5, sticky="w")
        
        self.click_dur_var = tk.StringVar(value=str(self.storage.get_detection_setting('click_duration')))
        click_dur_spinbox = tk.Spinbox(post_frame, from_=0.1, to=10, increment=0.1,
                                       textvariable=self.click_dur_var,
                                       width=5, font=("Arial", 9),
                                       command=lambda: self.on_click_duration_change())
        click_dur_spinbox.grid(row=1, column=1, padx=5)
        ...
        click_unit = tk.Label(post_frame, text="sec (10 clicks total)", font=("Arial", 9))
        click_unit.grid(row=1, column=2, sticky="w")        # Status label
    ...
    def toggle_change_area(self):
        ...
        try:
            # Capture full screen at actual resolution (handles DPI scaling)
            screenshot = ImageGrab.grab(all_screens=False)
            
            # Get actual screen size for validation
            actual_width, actual_height = get_actual_screen_size()
            print(f"[CHANGE_AREA] Screenshot captured: {screenshot.size}, Expected: {actual_width}x{actual_height}")
            
            # Resize screenshot if it doesn't match (shouldn't happen with DPI awareness, but just in case)
            if screenshot.size != (actual_width, actual_height):
                print(f"[CHANGE_AREA] WARNING: Screenshot size mismatch! Resizing...")
                screenshot = screenshot.resize((actual_width, actual_height), Image.Resampling.LANCZOS)
    ...
    def on_cast_timing_change(self, setting_name, var):
        """Handle cast timing change"""
        try:
            duration = float(var.get())
            if 0.1 <= duration <= 10:
                self.storage.set_detection_setting(setting_name, duration)
                display_name = setting_name.replace('_', ' ').replace('cast ', '').title()
                self.status_label.config(text=f"{display_name} set to {duration:.1f}s", fg="green")
                self.root.after(2000, lambda: self.status_label.config(text="Status: Idle", fg="gray"))
            else:
                var.set(str(self.storage.get_detection_setting(setting_name)))
        except ValueError:
    ...
    def check_color_in_area(self, x1, y1, x2, y2, target_color, tolerance=0):
        ...
        Args:
            x1, y1, x2, y2: Area coordinates (physical pixels)
        ...
        # Grab screenshot of area (all_screens=False for primary monitor only)
        screenshot = ImageGrab.grab(bbox=(x1, y1, x2, y2), all_screens=False)
    ...
    def _cast(self):
        ...
        # Get cast timing settings
        cast_hold_duration = self.storage.get_detection_setting('cast_hold_duration')
        cast_release_delay = self.storage.get_detection_setting('cast_release_delay')
        
        # Hold left click
        print(f"[CAST] Holding left click for {cast_hold_duration}s...")
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
        
        # Hold for configured duration
        time.sleep(cast_hold_duration)
        
        # Release left click
        print("[CAST] Releasing left click...")
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
        
        # Wait configured delay after release
        print(f"[CAST] Waiting {cast_release_delay}s after release...")
        time.sleep(cast_release_delay)
    ...
    def _fish(self):
        ...
        actual_width, actual_height = get_actual_screen_size()
        if region[2] > actual_width or region[3] > actual_height:
            print(f"[FISH] WARNING: Region {region} exceeds screen bounds {actual_width}x{actual_height}")
