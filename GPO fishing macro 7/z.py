...
class GPOFishingMacro:
    def __init__(self, root):
        ...
        # Default fruit key (initialize before loading)
        self.fruit_key = '3'
        ...
        # Auto-store fruit settings (initialize before loading)
        # Default store_fruit_point - relative to 2560x1440
        # store_fruit_point: (1279, 1151) = (49.96%, 79.93%)
        # water_point_3: (1046, 387) = (40.86%, 26.88%)
        self.auto_store_fruit = False
        self.store_fruit_point = (int(screen_width * 0.4996), int(screen_height * 0.7993))
        self.water_point_3 = (int(screen_width * 0.4086), int(screen_height * 0.2688))
        
        # Auto-select bait settings (initialize before loading)
        # Default bait_point - relative to 2560x1440
        # bait_point: (1283, 1062) = (50.12%, 73.75%)
        # water_point_2: (1003, 424) = (39.18%, 29.44%)
        self.auto_select_bait = False
        self.bait_point = (int(screen_width * 0.5012), int(screen_height * 0.7375))
        self.water_point_2 = (int(screen_width * 0.3918), int(screen_height * 0.2944))
        ...
        try:
            if self.settings_file.exists():
                with open(self.settings_file, 'r') as f:
                    ...
                    # Load fruit key
                    self.fruit_key = settings.get('fruit_key', '3')
                    ...
                    if settings.get('water_point'):
                        self.water_point = tuple(settings['water_point'])
                    if settings.get('yes_point'):
                        self.yes_point = tuple(settings['yes_point'])
                    if settings.get('no_point'):
                        self.no_point = tuple(settings['no_point'])
                    if settings.get('number_point'):
                        self.number_point = tuple(settings['number_point'])
                    # Note: If points not in settings, keep the defaults set during initialization
                    # Load auto-store fruit settings
                    self.auto_store_fruit = settings.get('auto_store_fruit', False)
                    if settings.get('store_fruit_point'):
                        self.store_fruit_point = tuple(settings['store_fruit_point'])
                    if settings.get('water_point_3'):
                        self.water_point_3 = tuple(settings['water_point_3'])
                    # Note: If points not in settings, keep the defaults set during initialization
                    # Load auto-select bait settings
                    self.auto_select_bait = settings.get('auto_select_bait', False)
                    if settings.get('bait_point'):
                        self.bait_point = tuple(settings['bait_point'])
                    if settings.get('water_point_2'):
                        self.water_point_2 = tuple(settings['water_point_2'])
                    # Note: If points not in settings, keep the defaults set during initialization
    ...
    def save_settings(self):
        ...
        try:
            settings = {
                ...
                "water_point_3": self.water_point_3,
                "auto_select_bait": self.auto_select_bait,
                "bait_point": self.bait_point,
                "water_point_2": self.water_point_2
            }
    ...
    def create_widgets(self):
        ...
        # Title
        tk.Label(self.scrollable_frame, text="GPO Fishing Macro").pack(pady=5)
        ...
        # Color Tolerances
        frame_color1 = tk.Frame(self.scrollable_frame)
        frame_color1.pack(pady=4)
        ...
        frame_color2 = tk.Frame(self.scrollable_frame)
        frame_color2.pack(pady=4)
        ...
        frame_fruit = tk.Frame(self.scrollable_frame)
        frame_fruit.pack(pady=5)
        self.auto_store_var = tk.BooleanVar(value=self.auto_store_fruit)
        tk.Checkbutton(frame_fruit, text="Auto Store Fruit", variable=self.auto_store_var, 
                      command=self.toggle_auto_store).pack()
        
        # Auto Select Bait Checkbox
        frame_bait = tk.Frame(self.scrollable_frame)
        frame_bait.pack(pady=5)
        self.auto_select_bait_var = tk.BooleanVar(value=self.auto_select_bait)
        tk.Checkbutton(frame_bait, text="Auto Select Bait", variable=self.auto_select_bait_var, 
                      command=self.toggle_auto_select_bait).pack()
        
        # Auto Buy Bait Settings (initially hidden)
        self.auto_buy_frame = tk.Frame(self.scrollable_frame)
        
        # Loops Per Purchase
        loops_frame = tk.Frame(self.auto_buy_frame)
        loops_frame.pack(pady=3)
        ...
        # Auto Store Fruit Settings (initially hidden)
        self.auto_store_frame = tk.Frame(self.scrollable_frame)
        
        # Fruit Key
        fruit_key_frame = tk.Frame(self.auto_store_frame)
        fruit_key_frame.pack(pady=3)
        tk.Label(fruit_key_frame, text="Fruit Key:").pack(side="left", padx=2)
        self.fruit_key_var = tk.StringVar(value=self.fruit_key)
        fruit_dropdown = tk.OptionMenu(fruit_key_frame, self.fruit_key_var, '1', '2', '3', '4', '5', '6', '7', '8', '9', '0')
        fruit_dropdown.config(width=5)
        fruit_dropdown.pack(side="left", padx=2)
        tk.Button(fruit_key_frame, text="Save", command=self.save_fruit_key, width=8).pack(side="left")
        
        # Store Fruit Point
        store_fruit_frame = tk.Frame(self.auto_store_frame)
        store_fruit_frame.pack(pady=3)
        self.store_fruit_btn = tk.Button(store_fruit_frame, text="Store Fruit Point: Not Set", 
                                         command=lambda: self.capture_point('store_fruit'), width=28)
        self.store_fruit_btn.pack()
        
        # Water Point 3
        water_point_3_frame = tk.Frame(self.auto_store_frame)
        water_point_3_frame.pack(pady=3)
        self.water_point_3_btn = tk.Button(water_point_3_frame, text="Water Point 3: Not Set", 
                                           command=lambda: self.capture_point('water_3'), width=28)
        self.water_point_3_btn.pack()
        
        # Save Auto Store Settings Button
        save_store_frame = tk.Frame(self.auto_store_frame)
        save_store_frame.pack(pady=6)
        tk.Button(save_store_frame, text="Save Auto Store Settings", 
                 command=self.save_auto_store_settings, width=28).pack()
        
        # Update store fruit button labels if points are set
        self.update_store_fruit_button()
        self.update_water_point_3_button()
        
        # Auto Select Bait Settings (initially hidden)
        self.auto_select_bait_frame = tk.Frame(self.scrollable_frame)
        
        # Bait Point
        bait_point_frame = tk.Frame(self.auto_select_bait_frame)
        bait_point_frame.pack(pady=3)
        self.bait_point_btn = tk.Button(bait_point_frame, text="Bait Point: Not Set", 
                                        command=lambda: self.capture_point('bait'), width=28)
        self.bait_point_btn.pack()
        
        # Water Point 2
        water_point_2_frame = tk.Frame(self.auto_select_bait_frame)
        water_point_2_frame.pack(pady=3)
        self.water_point_2_btn = tk.Button(water_point_2_frame, text="Water Point 2: Not Set", 
                                           command=lambda: self.capture_point('water_2'), width=28)
        self.water_point_2_btn.pack()
        
        # Save Auto Select Bait Settings Button
        save_bait_frame = tk.Frame(self.auto_select_bait_frame)
        save_bait_frame.pack(pady=6)
        tk.Button(save_bait_frame, text="Save Auto Select Bait Settings", 
                 command=self.save_auto_select_bait_settings, width=28).pack()
        
        # Update bait point buttons labels if points are set
        self.update_bait_point_button()
        self.update_water_point_2_button()
        
        # Status
        self.status_label = tk.Label(self.scrollable_frame, text="Status: Stopped")
        ...
        # Show/hide auto store frame based on checkbox
        if self.auto_store_fruit:
            self.auto_store_frame.pack(pady=5, before=self.status_label)
        
        # Show/hide auto select bait frame based on checkbox
        if self.auto_select_bait:
            self.auto_select_bait_frame.pack(pady=5, before=self.status_label)
        
        # Adjust window size based on enabled features
    ...
    def save_fruit_key(self):
        """Save fruit key value from dropdown"""
        self.fruit_key = self.fruit_key_var.get()
        self.save_settings()
        self.status_label.config(text="Fruit key saved!")
        self.root.after(2000, lambda: self.status_label.config(
            text="Status: Running" if self.is_running else "Status: Stopped"
        ))
    ...
    def toggle_auto_store(self):
        """Toggle the visibility of auto store settings"""
        self.auto_store_fruit = self.auto_store_var.get()
        self.update_gui_size()
        self.save_settings()
    
    def toggle_auto_select_bait(self):
        """Toggle the visibility of auto select bait settings"""
        self.auto_select_bait = self.auto_select_bait_var.get()
        self.update_gui_size()
        self.save_settings()
    
    def update_gui_size(self):
        """Update GUI size based on which settings are visible"""
        # Pack or unpack frames
        if self.auto_buy_bait:
            if not self.auto_buy_frame.winfo_manager():
                self.auto_buy_frame.pack(pady=5, before=self.status_label)
        else:
            self.auto_buy_frame.pack_forget()
        
        if self.auto_store_fruit:
            if not self.auto_store_frame.winfo_manager():
                self.auto_store_frame.pack(pady=5, before=self.status_label)
        else:
            self.auto_store_frame.pack_forget()
        
        if self.auto_select_bait:
            if not self.auto_select_bait_frame.winfo_manager():
                self.auto_select_bait_frame.pack(pady=5, before=self.status_label)
        else:
            self.auto_select_bait_frame.pack_forget()
        
        # Keep window at fixed height with scrolling
        # Max height set to 600 pixels to fit most screens
    ...
    def capture_point(self, point_name):
        ...
        def _on_click(x, y, button, pressed):
            if pressed:
                ...
                elif point_name == 'store_fruit':
                    self.store_fruit_point = (x, y)
                elif point_name == 'bait':
                    self.bait_point = (x, y)
                elif point_name == 'water_2':
                    self.water_point_2 = (x, y)
                elif point_name == 'water_3':
                    self.water_point_3 = (x, y)
                
                # Update UI
                self.root.after(0, self.update_point_buttons)
                self.root.after(0, self.update_store_fruit_button)
                self.root.after(0, self.update_bait_point_button)
                self.root.after(0, self.update_water_point_2_button)
                self.root.after(0, self.update_water_point_3_button)
    ...
    def update_store_fruit_button(self):
        """Update the store fruit button label to show current point coordinate"""
        if self.store_fruit_point:
            self.store_fruit_btn.config(text=f"Store Fruit Point: {self.store_fruit_point}")
    
    def update_bait_point_button(self):
        """Update the bait point button label to show current point coordinate"""
        if self.bait_point:
            self.bait_point_btn.config(text=f"Bait Point: {self.bait_point}")
    
    def update_water_point_2_button(self):
        """Update the water point 2 button label to show current point coordinate"""
        if self.water_point_2:
            self.water_point_2_btn.config(text=f"Water Point 2: {self.water_point_2}")
    
    def update_water_point_3_button(self):
        """Update the water point 3 button label to show current point coordinate"""
        if self.water_point_3:
            self.water_point_3_btn.config(text=f"Water Point 3: {self.water_point_3}")
    ...
    def save_auto_store_settings(self):
        """Save auto store fruit settings"""
        if not self.store_fruit_point or not self.water_point_3:
            messagebox.showwarning("Warning", "Please set both store fruit point and water point 3 before saving.")
            return
        self.save_settings()
        self.status_label.config(text="Auto Store settings saved!")
        self.root.after(2000, lambda: self.status_label.config(
            text="Status: Running" if self.is_running else "Status: Stopped"
        ))
    
    def save_auto_select_bait_settings(self):
        """Save auto select bait settings"""
        if not self.bait_point or not self.water_point_2:
            messagebox.showwarning("Warning", "Please set both bait point and water point 2 before saving.")
            return
        self.save_settings()
        self.status_label.config(text="Auto Select Bait settings saved!")
        self.root.after(2000, lambda: self.status_label.config(
            text="Status: Running" if self.is_running else "Status: Stopped"
        ))
    ...
    def perform_store_fruit(self):
        """Press fruit key and click on the store fruit point to store fruit"""
        if not self.store_fruit_point:
            print("Store fruit point not set!")
            return
        
        print(f"Storing fruit - pressing key {self.fruit_key} and clicking at {self.store_fruit_point}")
        
        # Press fruit key
        keyboard.press(self.fruit_key)
        keyboard.release(self.fruit_key)
        time.sleep(0.5)  # Wait 500ms after pressing fruit key
        
        # Click on store fruit point
        win32api.SetCursorPos(self.store_fruit_point)
        # anti-roblox: perform a tiny relative move so the client registers the cursor reposition
        try:
            win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, 0, 1, 0, 0)
        except:
        ...
        time.sleep(0.05)
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
        time.sleep(1.0)  # Wait 2 seconds after clicking store fruit
        ...
        # Move cursor to water point 3
        if self.water_point_3:
            win32api.SetCursorPos(self.water_point_3)
            # anti-roblox: perform a tiny relative move so the client registers the cursor reposition
            try:
                win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, 0, 1, 0, 0)
            except:
    ...
    def cast_rod(self):
        ...
        # Auto select bait if enabled
        if self.auto_select_bait and self.bait_point and self.water_point_2:
            print("Auto selecting bait...")
            # Click on bait point
            win32api.SetCursorPos(self.bait_point)
            # anti-roblox: perform a tiny relative move so the client registers the cursor reposition
            try:
                win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, 0, 1, 0, 0)
            except:
            ...
            # Move mouse to water point 2
            win32api.SetCursorPos(self.water_point_2)
            # anti-roblox: perform a tiny relative move so the client registers the cursor reposition
            try:
                win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, 0, 1, 0, 0)
            except:
                        ...
                        if self.waiting_for_bite:
                            ...
                            if self.color_not_found_time is None:
                                ...
                                if elapsed >= self.end_fish_timeout:
                                    ...
                                    # Check if we need to store fruit (do this BEFORE buying bait or recasting)
                                    if self.auto_store_fruit and self.store_fruit_point:
                                        print("Storing fruit...")
