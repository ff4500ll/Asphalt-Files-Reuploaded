...
class HotkeyGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("GPO autofish - by asphalt")
    ...
    def create_widgets(self):
        ...
        # Title
        title = ttk.Label(main_frame, text="GPO autofish - by asphalt", font=('Arial', 16, 'bold'))
    ...
    def main_loop(self):
        ...
        with mss.mss() as sct:
            ...
            while self.main_loop_active:
                ...
                # Convert to numpy array (BGRA from mss, convert to BGR)
                img = np.array(screenshot)[:, :, :3]  # Drop alpha channel, keep BGR
                ...
                for row_idx in range(height):
                    for col_idx in range(width):
                        ...
                        if r == target_color[0] and g == target_color[1] and b == target_color[2]:
                            point1_x = x + col_idx
                            point1_y = y + row_idx + 5  # Move down 5 pixels from detected position
                            found_first = True
                            break
                ...
                # Get temp area from main frame (already captured)
                ...
                # Extract real area from main frame
                real_x_offset = real_x - x
                real_y_offset = real_y - y
                real_img = img[real_y_offset:real_y_offset+real_height, real_x_offset:real_x_offset+real_width]
                ...
                # Allow gaps based on white height (white_height * 2)
