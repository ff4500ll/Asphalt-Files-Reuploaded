...
class HotkeyConfigApp:
    def __init__(self, root):
        ...
        self.first_launch_config = {
            ...
            # Fishing Control Parameters
            "fish_kp": 1.0,          # Proportional gain (1.0 = strong response)
            "fish_dead_zone": 5.0,   # Dead zone in pixels (no action if error < 5px)
            "fish_time_step": 0.01,  # Time between loops in seconds (faster = more responsive)
            "fish_detection_timeout": 3.0,
    ...
    def update_fish_flow_details(self):
        ...
        if not self.active_settings.get("flow_fish_enabled", True):
            ...
            self.fish_kp_var = tk.StringVar(value=str(self.active_settings.get("fish_kp", 1.0)))
            ...
            kp_desc = tk.Label(
                settings_frame,
                text="Proportional gain - how strongly to react to position error (1.0 = strong, higher = more aggressive)",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
                fg="#666666"
            )
            ...
            # Proportional Controller - Dead Zone
            dead_zone_frame = tk.Frame(settings_frame, bg="#f5f5f5")
            dead_zone_frame.grid(row=4, column=0, padx=10, pady=5, sticky="w")

            dead_zone_label = tk.Label(
                dead_zone_frame,
                text="Dead Zone:",
                font=("Segoe UI", 10),
                bg="#f5f5f5",
            ...
            dead_zone_label.pack(side="left")

            self.fish_dead_zone_var = tk.StringVar(value=str(self.active_settings.get("fish_dead_zone", 5.0)))
            self.fish_dead_zone_var.trace_add("write", lambda *args: self.save_setting("fish_dead_zone", self.fish_dead_zone_var))
            dead_zone_entry = tk.Entry(
                dead_zone_frame,
                textvariable=self.fish_dead_zone_var,
                font=("Segoe UI", 10),
                width=8
            )
            dead_zone_entry.pack(side="left", padx=5)

            dead_zone_unit = tk.Label(
                dead_zone_frame,
                text="pixels",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
            ...
            dead_zone_unit.pack(side="left")

            # Description for Dead Zone
            dead_zone_desc = tk.Label(
                settings_frame,
                text="Dead zone in pixels - no action taken if error < this value (prevents jitter, try 5-10px)",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
                fg="#666666"
            )
            dead_zone_desc.grid(row=5, column=0, padx=10, pady=(0, 5), sticky="w")

            # Proportional Controller - Time Step
            time_step_frame = tk.Frame(settings_frame, bg="#f5f5f5")
            time_step_frame.grid(row=6, column=0, padx=10, pady=5, sticky="w")

            time_step_label = tk.Label(
                time_step_frame,
                text="Time Step:",
                font=("Segoe UI", 10),
                bg="#f5f5f5",
            ...
            time_step_label.pack(side="left")

            self.fish_time_step_var = tk.StringVar(value=str(self.active_settings.get("fish_time_step", 0.01)))
            self.fish_time_step_var.trace_add("write", lambda *args: self.save_setting("fish_time_step", self.fish_time_step_var))
            time_step_entry = tk.Entry(
                time_step_frame,
                textvariable=self.fish_time_step_var,
                font=("Segoe UI", 10),
                width=8
            )
            time_step_entry.pack(side="left", padx=5)

            time_step_unit = tk.Label(
                time_step_frame,
                text="seconds",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
            ...
            time_step_unit.pack(side="left")

            # Description for Time Step
            time_step_desc = tk.Label(
                settings_frame,
                text="Loop delay between control updates (lower = faster response, higher CPU usage)",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
                fg="#666666"
            )
            time_step_desc.grid(row=7, column=0, padx=10, pady=(0, 5), sticky="w")

            # PD Controller - PD Clamp
            ...
            pd_clamp_desc = tk.Label(
                settings_frame,
                text="Maximum control signal magnitude (limits max correction strength)",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
                fg="#666666"
            )
            ...
            # Detection Timeout
            detection_timeout_frame = tk.Frame(settings_frame, bg="#f5f5f5")
            detection_timeout_frame.grid(row=10, column=0, padx=10, pady=5, sticky="w")

            detection_timeout_label = tk.Label(
                detection_timeout_frame,
            ...
            detection_timeout_label.pack(side="left")

            self.fish_detection_timeout_var = tk.StringVar(value=str(self.active_settings.get("fish_detection_timeout", 3.0)))
            self.fish_detection_timeout_var.trace_add("write", lambda *_args: self.save_setting("fish_detection_timeout", self.fish_detection_timeout_var))
            detection_timeout_entry = tk.Entry(
                detection_timeout_frame,
                textvariable=self.fish_detection_timeout_var,
                font=("Segoe UI", 10),
                width=8
            )
            detection_timeout_entry.pack(side="left", padx=5)

            detection_timeout_unit = tk.Label(
                detection_timeout_frame,
                text="seconds",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
            ...
            detection_timeout_unit.pack(side="left")

            # Description for Detection Timeout
            detection_timeout_desc = tk.Label(
                settings_frame,
                text="Seconds to wait before giving up if detections are lost (0 = wait forever)",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
                fg="#666666"
            )
            detection_timeout_desc.grid(row=11, column=0, padx=10, pady=(0, 5), sticky="w")

            # Fish Timeout
            fish_timeout_frame = tk.Frame(settings_frame, bg="#f5f5f5")
            fish_timeout_frame.grid(row=12, column=0, padx=10, pady=5, sticky="w")
            ...
            fish_timeout_desc.grid(row=13, column=0, padx=10, pady=(0, 5), sticky="w")

            # Show Live Feed Checkbox
            live_feed_frame = tk.Frame(settings_frame, bg="#f5f5f5")
            live_feed_frame.grid(row=14, column=0, padx=10, pady=10, sticky="w")
            ...
            live_feed_desc.grid(row=15, column=0, padx=10, pady=(0, 5), sticky="w")
    ...
    def reset_to_defaults(self):
        """Reset all settings to first launch defaults by deleting Config.txt and reloading"""
        ...
        # Reload config (this will recreate Config.txt with defaults)
        self.load_config()

        # Update GUI dropdowns to reflect the new settings
        for action, key_var in self.dropdown_widgets.items():
        ...
        try:
            while self.loop_running:
                ...
                if self.active_settings.get("flow_fish_enabled", True):
                    ...
                    # Initial startup detection - track if target or bar has moved yet
                    initial_target_x = None
                    initial_bar_center_x = None
                    game_started = False
                
                    # Bang-Bang Controller with Hysteresis (no Kp needed)
                
                    # Get controller parameters from settings
                    time_step = self.active_settings.get("fish_time_step", 0.01)
                
                    print("[Fish] Starting detection and tracking...")
                    print(f"[Fish] Bang-Bang Controller with Hysteresis initialized - TimeStep: {time_step}s")
                
                    # Main control loop
                    while self.loop_running:
                    
                        # YOLO Detection - track elements (use mss for 10x faster capture)
                        monitor = {"top": fish_y1, "left": fish_x1, "width": fish_x2 - fish_x1, "height": fish_y2 - fish_y1}
                        sct_img = sct.grab(monitor)
                        fish_frame = np.array(sct_img)
                        fish_frame = cv2.cvtColor(fish_frame, cv2.COLOR_BGRA2BGR)
                    
                        # No need to resize - model was trained at imgsz=320, already fast
                        original_height, original_width = fish_frame.shape[:2]
                        ...
                        # Extract YOLO detections (no scaling needed - using full resolution)
                        target_line_x = None
                        bar_left_x = None
                        bar_right_x = None
                        bar_center_x = None
                        ...
                        # Check if game has started (detect initial movement)
                        if not game_started:
                            if target_line_x is not None and bar_center_x is not None:
                                # Store initial positions on first detection
                                if initial_target_x is None:
                                    initial_target_x = target_line_x
                                    initial_bar_center_x = bar_center_x
                                    print(f"[Fish] Initial positions - Target: {initial_target_x}px, Bar: {initial_bar_center_x}px")

                                # Check if either target or bar has moved (threshold: 3 pixels)
                                movement_threshold = 3
                                target_moved = abs(target_line_x - initial_target_x) > movement_threshold
                                bar_moved = abs(bar_center_x - initial_bar_center_x) > movement_threshold

                                if target_moved or bar_moved:
                                    game_started = True
                                    print(f"[Fish] Movement detected! Target moved: {target_moved}, Bar moved: {bar_moved}")
                                else:
                                    # Still in startup - spam click to initiate movement
                                    should_hold = not is_holding

                                    # Execute startup spam
                                    ...
                                    print(f"[Fish] STARTUP SPAM | {'HOLD' if is_holding else 'RELEASE'} | Waiting for movement...")
                                    continue
                    
                        # Game has started - display tracking info
                        if target_line_x is not None and bar_center_x is not None:
                            error = target_line_x - bar_center_x
                            print(f"[Fish] TRACKING | Target: {target_line_x}px | Bar: {bar_center_x}px | Error: {error:+.0f}px")
                    
                        # ==================== BANG-BANG CONTROLLER WITH HYSTERESIS ====================
                        if game_started and target_line_x is not None and bar_center_x is not None:
                            # 1. Calculate Error (Fish position - Player position)
                            error = target_line_x - bar_center_x
                        
                            # 2. Predictive control with hysteresis to prevent overshooting
                            # If we're currently pressing (moving right), we need MORE margin to release
                            # If we're currently released (drifting left), we need LESS margin to press
                        
                            # Define hysteresis thresholds based on current state
                            if is_holding:
                                # Currently moving right - need to stop BEFORE target (anticipate overshoot)
                                press_threshold = 20    # Start pressing again if error > 20px right
                                release_threshold = -5  # Release if error > 5px left (we've overshot)
                            else:
                                # Currently drifting left - can press earlier
                                press_threshold = 10    # Press if error > 10px right
                                release_threshold = -15 # Keep released if error > 15px left
                        
                            # 3. Decision Logic with hysteresis
                            if error > press_threshold:
                                # Target is far to the right - press
                                action = "PRESS"
                                if not is_holding:
                                    mouse_down()
                                    is_holding = True
                            elif error < release_threshold:
                                # Target is far to the left - release
                                action = "RELEASE"
                                if is_holding:
                                    mouse_up()
                                    is_holding = False
                                ...
                                action = f"HOLD_STATE ({'PRESSING' if is_holding else 'RELEASED'})"
                        
                            # 4. Debug output
                            print(f"[Bang-Bang] Error: {error:+.0f}px | Thresholds: [{release_threshold:+.0f}, {press_threshold:+.0f}] | Action: {action}")
                    
                        elif game_started:
                            # Lost tracking - maintain current state (don't change anything)
                            print(f"[Bang-Bang] Lost tracking - maintaining state ({'HOLDING' if is_holding else 'RELEASED'})")
                    
                        # Add a small delay to control loop rate
