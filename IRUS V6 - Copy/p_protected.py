...
LOG_FILE = base64.b64decode(b"RGVidWcudHh0").decode()
...
CONFIG_FILE = base64.b64decode(b"Q29uZmlnLnR4dA==").decode()
...
def get_rod_colors(rod_type):
    ...
    # Return the configuration for the specified rod type, or Default as fallback
    return rod_configs.get(rod_type, rod_configs["Default"])
# ------------------------------------------
        ...
        if is_capture_valid and width == canvas_w and height == canvas_h:
            try:
                ...
                if CV_AVAILABLE:
                    ...
                    # Get rod-specific colors
                    rod_type = self.master_app.rod_type_var.get()
                    rod_colors = get_rod_colors(rod_type)
                    ...
                    if rod_colors["target_line_alt"] is not None:
                        # Mask 1 Alt: Target Line Color (Alternative)
                        color1_alt_bgr = hex_to_bgr(rod_colors["target_line_alt"])
...
class Application(tk.Tk):
    ...
    def setup_ui(self):
        ...
        rod_types = ["Default", "Evil Pitch Fork", "Onirifalx", "Polaris Serenade", "Sword of Darkness", 
                    "Wingripper", "Axe of Rhoads", "Chrysalis", "Luminescent Oath", "Ruinous Oath", "Duskwire"]
        ...
        rod_combo.bind('<<ComboboxSelected>>', lambda event: self.save_config())
        ttk.Label(rod_type_frame, text="(Only 'Default' has color detection enabled)", foreground="orange", font=('Arial', 8)).pack(side="left")
        ...
        target_line_tolerance_entry.bind('<KeyRelease>', lambda event: self.save_config())
        ...
        indicator_arrow_tolerance_entry.bind('<KeyRelease>', lambda event: self.save_config())
        ...
        box_color_tolerance_entry.bind('<KeyRelease>', lambda event: self.save_config())
        ...
        self.autocast_hold_time_entry.bind('<KeyRelease>', lambda event: self.save_config())
        ...
        self.autocast_wait_time_entry.bind('<KeyRelease>', lambda event: self.save_config())
        ...
        self.refresh_rod_delay_entry.bind('<KeyRelease>', lambda event: self.save_config())
        ...
        self.shake_pixel_tolerance_entry.bind('<KeyRelease>', lambda event: self.save_config())
        ...
        self.shake_circle_tolerance_entry.bind('<KeyRelease>', lambda event: self.save_config())
        ...
        self.navigation_key_entry.bind('<KeyRelease>', lambda event: self.save_config())
    ...
    def _process_and_control(self, sct_instance, time_delta, current_time):
        ...
        if rod_colors["target_line_alt"] is not None:
            # Target Line Color (Alternative)
            color1_alt_bgr = hex_to_bgr(rod_colors["target_line_alt"])
            lower1_alt, upper1_alt = _get_bgr_bounds(color1_alt_bgr, target_line_tol)
            mask_alternative = cv2.inRange(cv_img_bgr, lower1_alt, upper1_alt)
            target_line_mask = cv2.bitwise_or(target_line_mask, mask_alternative)

        # Calculate target line stats if any mask was created
        if rod_colors["target_line"] is not None or rod_colors["target_line_alt"] is not None:
        ...
        if rod_colors["box_color_1"] is not None or rod_colors["box_color_2"] is not None:
