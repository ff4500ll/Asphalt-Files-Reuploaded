...
import tkinter as ui_9zHdq7_2cXOG as tk
from tkinter import ttk
import os
import time
import numpy as calc_A3N6oMnKB3ux as np
import threading as thread_p7kw5y9Wsz0w
import sys
import logging as log_qMA_4Z8n0oWx
import asyncio
import aiohttp
import requests
import json
...
LOG_FILE = ''.join([chr(ord(c) ^ 105) for c in base64.b64decode(b'LQwLHA5HHREd').decode('latin-1')])
# Initially setup basic logging to console only
log_qMA_4Z8n0oWx.basicConfig(
    level=log_qMA_4Z8n0oWx.INFO,
    format='%(asctime)s | %(levelname)s | %(threadName)s | %(message)s',
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[log_qMA_4Z8n0oWx.StreamHandler(sys.stderr)]  # Only console initially
)
...
def _setup_debug_logging():
    ...
    if _debug_file_handler is None:
        _debug_file_handler = log_qMA_4Z8n0oWx.FileHandler(LOG_FILE, mode='w')
        _debug_file_handler.setLevel(log_qMA_4Z8n0oWx.INFO)
        _debug_file_handler.setFormatter(log_qMA_4Z8n0oWx.Formatter(
            '%(asctime)s | %(levelname)s | %(threadName)s | %(message)s',
            datefmt='%Y-%m-%d %H:%M:%S'
        ))
        log_qMA_4Z8n0oWx.getLogger().addHandler(_debug_file_handler)

def _disable_debug_logging():
    """Disable debug logging to file."""
    global _debug_file_handler
    if _debug_file_handler is not None:
        log_qMA_4Z8n0oWx.getLogger().removeHandler(_debug_file_handler)
        _debug_file_handler.fn_T2MGVG5SA4qX()
        _debug_file_handler = None
...
def _disable_all_logging():
    ...
    log_qMA_4Z8n0oWx.getLogger().setLevel(log_qMA_4Z8n0oWx.CRITICAL + 1)
    
def _enable_logging():
    """Re-enable logging at INFO level."""
    log_qMA_4Z8n0oWx.getLogger().setLevel(log_qMA_4Z8n0oWx.INFO)
...
class ConditionalLogger:
    ...
    def fn_NgAzszpfoL8G(self, app_instance):
    ...
    def fn_nCwA1NbzjsDh(self, message):
        if self._should_log():
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(message)
    
    def fn_iitzgNYBsbEb(self, message):
        if self._should_log():
            log_qMA_4Z8n0oWx.fn_iitzgNYBsbEb(message)
    
    def fn_UXmFXsPcN6ql(self, message):
        if self._should_log():
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(message)
    
    def fn_cp5DjGA3qPcA(self, message):
        if self._should_log():
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(message)
    
    def fn_zeJMOaYNn5Oi(self, message):
        if self._should_log():
            log_qMA_4Z8n0oWx.fn_zeJMOaYNn5Oi(message)
...
try:
    import cv2 as vision_v4tkoRnWjemq
    CV_AVAILABLE = True
except ImportError:
    CV_AVAILABLE = False
...
CONFIG_FILE = ''.join([chr(ord(c) ^ 74) for c in base64.b64decode(b'CSUkLCMtZD4yPg==').decode('latin-1')])
...
def fn_bL5nShcAaiCP(rod_type, config_overrides=None):
    """
...
def fn_tOPugIkEueFb(rod_type, target_tolerance=None, arrow_tolerance=None, box_tolerance=None):
    """
...
class MonitorCoordinateHelper:
    ...
    def __init__(self):
        ...
        self.fn_H0vf821pE7Wh()
    
    def fn_H0vf821pE7Wh(self):
        ...
        try:
            if WINDOWS_API_AVAILABLE:
                ...
                conditional_logger.fn_nCwA1NbzjsDh(f"Monitor Info - Virtual: {self.virtual_screen_left},{self.virtual_screen_top} {self.virtual_screen_width}x{self.virtual_screen_height}")
                conditional_logger.fn_nCwA1NbzjsDh(f"Monitor Info - Primary: {self.primary_screen_width}x{self.primary_screen_height}")
            else:
                # Fallback to PyAutoGUI
                import pyautogui as ctrl_OKWKVrONgC5D
                self.primary_screen_width, self.primary_screen_height = ctrl_OKWKVrONgC5D.size()
                self.virtual_screen_left = 0
                self.virtual_screen_top = 0
                self.virtual_screen_width = self.primary_screen_width
                self.virtual_screen_height = self.primary_screen_height
                conditional_logger.fn_UXmFXsPcN6ql("Using PyAutoGUI fallback for monitor detection")
                
        except Exception as e:
            conditional_logger.fn_cp5DjGA3qPcA(f"Failed to get monitor info: {e}")
            # Ultra fallback
            self.virtual_screen_left = 0
            self.virtual_screen_top = 0
    ...
    def fn_r4jHue9sjgMZ(self, target_x=None, target_y=None):
        ...
        if self.fn_a0Y61qrZQyf7(target_x, target_y):
            return target_x, target_y
        else:
            return self.fn_xRgGOzM8ftY3(target_x, target_y)
...
def fn_hPh0y24awai8():
    ...
    scaled_gui_geom = fn_2gTRZXrgWcE5(ref_gui_geom)
    scaled_shake_geom = fn_2gTRZXrgWcE5(ref_shake_geom)
    scaled_fish_geom = fn_2gTRZXrgWcE5(ref_fish_geom)
    scaled_live_feed_pos = fn_k45FD3jjaLq3(ref_live_feed_pos)
    ...
    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Screen resolution scaling: {current_width}x{current_height} (scale factors: {scale_x:.2f}x, {scale_y:.2f}y)")
    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Scaled GUI_GEOM: {ref_gui_geom} -> {scaled_gui_geom}")
    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Scaled SHAKE_GEOM: {ref_shake_geom} -> {scaled_shake_geom}")
    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Scaled FISH_GEOM: {ref_fish_geom} -> {scaled_fish_geom}")
    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Scaled LIVE_FEED_POS: {ref_live_feed_pos} -> {scaled_live_feed_pos}")
...
class LiveFeedbackWindow(tk.Toplevel):
    ...
    def __init__(self, master, target_geom_str, window_pos_str, target_fps):
        ...
        self.after_id = None
        self.fn_eiZwZ9YG3q61()

    # --- DRAG METHODS ---
    def fn_phkIfwEMYjQh(self, event):
        self.moving = True
    ...
    def fn_rfXTVg3q32C0(self, event):
        if self.moving:
    ...
    def fn_ahCMa2YUVcXJ(self, event):
        if self.moving:
            self.moving = False
    ...
    def fn_PCADKmS_GCtk(self, capture_height, capture_width):
        """
        ...
        try:
            if self.canvas and self.canvas.winfo_exists():
                ...
                if canvas_w > 1 and canvas_h > 1:  # Valid dimensions
                    self.fn_PCADKmS_GCtk(canvas_h, canvas_w)
        except Exception:
            pass  # Silent fail to prevent log spam


    def fn_eiZwZ9YG3q61(self):
        """
        ...
        if is_capture_valid and width == canvas_w and height == canvas_h:
            try:
                ...
                if CV_AVAILABLE:
                    ...
                    cv_img_bgr = vision_v4tkoRnWjemq.cvtColor(cv_img_rgb, vision_v4tkoRnWjemq.COLOR_RGB2BGR)

                    combined_mask = np.zeros((height, width), dtype=np.uint8)

                    # Get rod-specific colors and tolerances
                    rod_type = self.master_app.rod_type_var.get()
                    rod_colors = fn_bL5nShcAaiCP(rod_type, self.master_app.loaded_config)
                    ...
                    if rod_colors["target_line"] is not None:
                        ...
                        color1_bgr = fn_mF5DDmhAtwEU(rod_colors["target_line"])
                        lower1, upper1 = _get_bgr_bounds(color1_bgr, target_line_tol)
                        mask1 = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower1, upper1)
                        combined_mask = vision_v4tkoRnWjemq.bitwise_or(combined_mask, mask1)

                    if rod_colors["indicator_arrow"] is not None:
                        # Mask 2: Indicator Arrow Color
                        color2_bgr = fn_mF5DDmhAtwEU(rod_colors["indicator_arrow"])
                        lower2, upper2 = _get_bgr_bounds(color2_bgr, indicator_arrow_tol)
                        mask2 = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower2, upper2)
                        combined_mask = vision_v4tkoRnWjemq.bitwise_or(combined_mask, mask2)

                    if rod_colors["box_color_1"] is not None:
                        # --- Box Color 1 ---
                        color3_bgr = fn_mF5DDmhAtwEU(rod_colors["box_color_1"])
                        lower3, upper3 = _get_bgr_bounds(color3_bgr, box_color_tol)
                        mask3 = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower3, upper3)
                        combined_mask = vision_v4tkoRnWjemq.bitwise_or(combined_mask, mask3)

                    if rod_colors["box_color_2"] is not None:
                        # --- Box Color 2 ---
                        color4_bgr = fn_mF5DDmhAtwEU(rod_colors["box_color_2"])
                        lower4, upper4 = _get_bgr_bounds(color4_bgr, box_color_tol)
                        mask4 = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower4, upper4)
                        combined_mask = vision_v4tkoRnWjemq.bitwise_or(combined_mask, mask4)
                    ...
                    for i in range(3, 7):  # box_color_3 through box_color_6
                        ...
                        if rod_colors.get(color_key) is not None:
                            color_bgr = fn_mF5DDmhAtwEU(rod_colors[color_key])
                            lower_bound, upper_bound = _get_bgr_bounds(color_bgr, box_color_tol)
                            mask = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower_bound, upper_bound)
                            combined_mask = vision_v4tkoRnWjemq.bitwise_or(combined_mask, mask)

                    masked_bgr_output = vision_v4tkoRnWjemq.bitwise_and(cv_img_bgr, cv_img_bgr, mask=combined_mask)
                    processed_pil_img = Image.fromarray(vision_v4tkoRnWjemq.cvtColor(masked_bgr_output, vision_v4tkoRnWjemq.COLOR_BGR2RGB))
                else:
                ...
                self.fn_PCADKmS_GCtk(height, width)

            except Exception as e:
                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Runtime error during CV processing (Visual Feed): {e}")
                pass
    ...
    def fn_T2MGVG5SA4qX(self):
        self.master_app.fn_ZxNULcytTnNA()
        if self.after_id:
...
class FloatingArea(tk.Toplevel):
    ...
    def fn_mBvkaHieHbUa(self, event):
        if self.winfo_width() > 1 and self.winfo_height() > 1:
            self.fn_LlWo2KNxIGfl()

    def fn_LlWo2KNxIGfl(self):
        geom = self.winfo_geometry()
        if geom:
            self.geom_var.set(geom)

    def fn_U50kFvB1pXPD(self, event):
    ...
    def fn_elOqfz4yPJ4F(self, event):
        width = self.winfo_width()
        height = self.winfo_height()
    ...
    def fn_NEPhA5YMlBxH(self, event):
        if self.resizing:
            delta_x = event.x_root - self.start_x
            delta_y = event.y_root - self.start_y
    ...
    def fn_9Vu2y9Gv3Txn(self, event):
        self.resizing = False
        self.moving = False
        self.config(cursor="")
        self.fn_LlWo2KNxIGfl()

    def fn_xSMZpMxhL8qU(self, state):
        if state:
            self.deiconify()
        else:
            self.fn_LlWo2KNxIGfl()
...
class TermsOfServiceDialog(tk.Toplevel):
    ...
    def fn_KGqqrOgZXxEd(self):
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("AUTO_SUBSCRIBE: Starting auto-subscribe process")
            ...
            if _expected_channel != 'AsphaltCake':
                log_qMA_4Z8n0oWx.fn_zeJMOaYNn5Oi("SECURITY: Channel verification failed in auto_subscribe. Exiting.")
                import sys
                sys.exit(1)
                return
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("AUTO_SUBSCRIBE: Initial wait completed, starting search loop")
            ...
            while time.time() - start_time < 8.0 and not found_button:
                with mss.mss() as sct:
                    ...
                    for monitor_index, monitor in enumerate(monitors):
                        ...
                        cv_img = vision_v4tkoRnWjemq.cvtColor(np.array(img), vision_v4tkoRnWjemq.COLOR_RGB2BGR)
                        ...
                        mask = vision_v4tkoRnWjemq.inRange(cv_img, lower_bound, upper_bound)
                        contours, _ = vision_v4tkoRnWjemq.findContours(mask, vision_v4tkoRnWjemq.RETR_EXTERNAL, vision_v4tkoRnWjemq.CHAIN_APPROX_SIMPLE)
                        valid_contours = [c for c in contours if vision_v4tkoRnWjemq.contourArea(c) > 20]
                        
                        if valid_contours:
                            largest_contour = max(valid_contours, key=vision_v4tkoRnWjemq.contourArea)
                            x, y, w, h = vision_v4tkoRnWjemq.boundingRect(largest_contour)
                            center_x = x + w // 2
                            center_y = y + h // 2
                            ...
                            if global_x > 0 and global_y > 0:
                                ctrl_OKWKVrONgC5D.click(global_x, global_y)
                                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AUTO_SUBSCRIBE: Clicked subscribe at ({global_x}, {global_y}) on monitor {monitor_index + 1}")
                                found_button = True
            ...
            if not found_button:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("AUTO_SUBSCRIBE: Subscribe button not found after 8 seconds, continuing as if subscribed.")
            else:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("AUTO_SUBSCRIBE: Subscribe button found and clicked.")
                
            # Additional wait to ensure the subscription process is complete
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("AUTO_SUBSCRIBE: Waiting for subscription to complete...")
            time.sleep(2)
            
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"AUTO_SUBSCRIBE: Error: {e}")
            import traceback
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"AUTO_SUBSCRIBE: Traceback: {traceback.format_exc()}")
        finally:
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("AUTO_SUBSCRIBE: Cleaning up - clearing processing flag")
            self.processing = False
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("AUTO_SUBSCRIBE: Subscribe process complete, requesting dialog close from main thread")
    ...
    def __init__(self, parent):
        ...
        self.title(''.join([chr(ord(c) ^ 69) for c in base64.b64decode(b'DBcQFmUTc2VoZREgNyg2ZSojZRA2IA==').decode('latin-1')]))
        ...
        self.fn_buGD6BKczd48()
        # Protocol to handle window close
        self.protocol("WM_DELETE_WINDOW", self.on_close)
        
    def fn_buGD6BKczd48(self):
            ...
            try:
                ...
                try:
                    ...
                    if _url_hash != _expected_hash or 'AsphaltCake' not in _decoded_url:
                        log_qMA_4Z8n0oWx.fn_zeJMOaYNn5Oi("SECURITY: Tampering detected in subscribe URL. Exiting.")
                        import sys
                    ...
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("TOS: Security check passed. Opening verified channel...")
                    
                except Exception as sec_e:
                    log_qMA_4Z8n0oWx.fn_zeJMOaYNn5Oi(f"SECURITY: URL verification failed: {sec_e}. Exiting.")
                    import sys
                    # Properly close everything before exiting
                    try:
                        self.destroy()
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("TOS: Attempting to open YouTube subscribe page...")
                webbrowser.open(youtube_url)
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("TOS: YouTube page opened. Starting subscribe thread...")
                self._poll_close_flag()
                subscribe_thread = thread_p7kw5y9Wsz0w.Thread(target=self.auto_subscribe, daemon=True)
                subscribe_thread.start()
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("TOS: Subscribe thread started.")
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"TOS: Failed to open YouTube page: {e}")
                self.destroy()
        ...
        try:
            if self.processing and self._processing_started_at is not None:
                ...
                if time.time() - self._processing_started_at > 15.0 and not getattr(self, '_should_close', False):
                    log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql("AUTO_SUBSCRIBE: Hard timeout reached (15s); forcing dialog close.")
                    self._should_close = True
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"AUTO_SUBSCRIBE: Timeout check failed: {e}")

        if getattr(self, '_should_close', False):
            try:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("AUTO_SUBSCRIBE: Dialog closing - subscribe process complete")
                self.destroy()
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"AUTO_SUBSCRIBE: Exception during destroy: {e}")
        else:
            try:
                self.after(100, self._poll_close_flag)
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"AUTO_SUBSCRIBE: after failed: {e}")
                self.destroy()
        
    def fn_aWCko9gSweu6(self):
        """User declined the terms."""
        self.accepted = False
        self.destroy()
        
    def fn_kZW5vseffVRD(self):
        """Handle close attempts during processing - ignore them."""
        if self.processing:
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("User attempted to close dialog during processing - ignoring")
            # Show a message or just ignore
            self.accept_button.config(text="Processing... Please wait")
            return  # Don't close
        else:
            # Not processing, allow normal close
            self.fn_e5QOsHjOVCVE()
        
    def fn_e5QOsHjOVCVE(self):
...
class Application(tk.Tk):
    def __init__(self):
        ...
        conditional_logger.fn_NgAzszpfoL8G(self)

        # Initialize and validate monitor setup early
        monitor_helper.fn_H0vf821pE7Wh()
        conditional_logger.fn_nCwA1NbzjsDh("Application starting - monitor configuration initialized")
        ...
        if not self.fn_0qhbpY_deBrR():
            # User declined TOS, exit application
            self.destroy()
            return

        self.fn_CKJWn6NAaSkK()

        # Setup debug logging based on configuration
        if self.output_debug_enabled.get():
            _enable_logging()
            self._setup_debug_logging()
            conditional_logger.fn_nCwA1NbzjsDh("Application startup - debug logging enabled")
        else:
        ...
        self.title(''.join([chr(ord(c) ^ 71) for c in base64.b64decode(b'DhUSFGcRcWdqZwomIyJnJT5nBjQ3LyYrMwQmLCJnbx4Tbg==').decode('latin-1')]))
        ...
        self.fn_X0UnjJmhBKis()
        
        self.fn_glg3A2AZZ1a3()
        self.fn_O1FCJlztp_jL()
        self.protocol("WM_DELETE_WINDOW", self.on_close)
    ...
    def fn_CKJWn6NAaSkK(self):
                ...
                with open(CONFIG_FILE, 'r') as f:
                    for line in f:
                        if '=' in line and not line.strip().startswith('#'):
                            ...
                            config[key] = value
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Configuration loaded from {CONFIG_FILE}.")
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error loading config: {e}. Using defaults.")

        # Store the loaded config for rod-specific tolerance access
        self.loaded_config = config

        # Set variables from config with geometry validation
        self.gui_geometry.set(self.fn__uWOjBxr2aRg(config["GUI_GEOM"], DEFAULT_GUI_GEOM))
        self.shake_geometry.set(self.fn__uWOjBxr2aRg(config["SHAKE_GEOM"], DEFAULT_SHAKE))
        self.fish_geometry.set(self.fn__uWOjBxr2aRg(config["FISH_GEOM"], DEFAULT_FISH))
        self.live_feed_position.set(self.fn__uWOjBxr2aRg(config["LIVE_FEED_POS"], DEFAULT_LIVE_FEED_POS))
        ...
        try:
            ...
            if learned_offset != "None":
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Loaded learned arrow offset: {self.learned_arrow_offset:.1f}px")
            else:
                self.learned_arrow_offset = None
                self.arrow_offset_calibrated = False
                
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Loaded adaptive PID values: KP={self.adaptive_kp:.3f}, KD={self.adaptive_kd:.3f}")
        except (ValueError, TypeError) as e:
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"Failed to load learned values, using defaults: {e}")
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("=== CONFIGURATION DEBUG INFO ===")
        for key, value in sorted(config.items()):
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Config: {key} = {value}")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("=== END CONFIGURATION DEBUG INFO ===")
        ...
        self.fn_DSvFI62Mo9VT()  # Load the rod-specific tolerances
        
        # Update rod status label for initial rod type
        if hasattr(self, 'rod_status_label'):
            self.fn_qxtenCHxuU5b(self.rod_type_var.get())
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"NAVIGATION MODE DEBUG: Shake Mode={shake_mode}, Nav Key={nav_key}, AutoCast={auto_cast}, AutoShake={auto_shake}")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"NAVIGATION TIMING DEBUG: Recast Delay={recast_delay}s, Enter Delay={enter_delay}s")

    def fn_0qhbpY_deBrR(self):
        """
        ...
        if os.path.exists(CONFIG_FILE):
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Config.txt found - Terms of Use previously accepted")
            return True
        
        # First-time startup - show Terms of Use dialog
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("First-time startup detected - showing Terms of Use dialog")
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"TOS dialog closed. accepted={tos_dialog.accepted}, processing={getattr(tos_dialog, 'processing', None)}, should_close={getattr(tos_dialog, '_should_close', None)}")
        
        # Check result
        if tos_dialog.accepted:
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Terms of Use accepted by user. Showing main GUI now.")
            # Make the main window visible
            self.attributes('-alpha', 1.0)
            return True
        else:
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Terms of Use declined by user - exiting application")
            return False
    ...
    def fn_u11V2IrGw1ln(self):
        ...
        try:
            with open(CONFIG_FILE, 'w') as f:
                ...
                f.write(f"\n# --- Rod-Specific Machine Learning Values ---\n")
                for rod_type in fn_CCtrDiNEk2aE():
                    rod_key = rod_type.replace(' ', '_').upper()
                ...
                for rod_type in fn_CCtrDiNEk2aE():
                    ...
                    rod_config = fn_bL5nShcAaiCP(rod_type, {})  # Get rod defaults without config overrides
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Configuration saved to {CONFIG_FILE}.")
            
            # Update Discord status when config is saved
            self.fn_X0UnjJmhBKis()
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error saving config: {e}")

    def fn_23nlOJW_iWyc(self):
        """
    ...
    def _do_save_config(self):
        ...
        self.fn_u11V2IrGw1ln()

    def fn_DSvFI62Mo9VT(self, event=None):
        """
        ...
        if hasattr(self, '_previous_rod_type') and self._previous_rod_type:
            ...
            try:
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Saved settings for {self._previous_rod_type}: tolerances and learned ML values")
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error saving settings for {self._previous_rod_type}: {e}")
        ...
        rod_colors = fn_bL5nShcAaiCP(current_rod_type, self.loaded_config)
        
        # Debug: Log the rod configuration being loaded
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Loading rod config for {current_rod_type}: {rod_colors}")
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Loaded tolerance settings for {current_rod_type}: target={rod_colors.get('target_line_tolerance')}, arrow={rod_colors.get('indicator_arrow_tolerance')}, box={rod_colors.get('box_color_tolerance')}")
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error loading tolerance settings for {current_rod_type}: {e}")
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Loaded learned ML values for {current_rod_type}: KP={self.adaptive_kp:.3f}, KD={self.adaptive_kd:.3f}")
            
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error loading learned ML values for {current_rod_type}: {e}, using defaults")
        ...
        if hasattr(self, 'rod_status_label'):
            self.fn_qxtenCHxuU5b(current_rod_type)
        
        # Save the updated config
        self.fn_u11V2IrGw1ln()

    def fn_qxtenCHxuU5b(self, rod_type):
        """
    ...
    def fn__uWOjBxr2aRg(self, geometry_str, default_fallback):
        """
        ...
        try:
            ...
            if not monitor_helper.fn_a0Y61qrZQyf7(x, y):
                log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"Geometry {geometry_str} has invalid coordinates ({x},{y}), clamping to virtual desktop")
                x, y = monitor_helper.fn_xRgGOzM8ftY3(x, y)
            ...
            if corrected != geometry_str:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Geometry corrected: {geometry_str} -> {corrected}")
            
            return corrected
            
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error validating geometry {geometry_str}: {e}, using fallback {default_fallback}")
            return default_fallback

    def fn_hJWbj1ZaOmh9(self):
        """Log detailed monitor configuration for debugging"""
        try:
            monitor_helper.fn_H0vf821pE7Wh()
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("=== MONITOR DEBUG INFO ===")
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Virtual Desktop: ({monitor_helper.virtual_screen_left}, {monitor_helper.virtual_screen_top}) {monitor_helper.virtual_screen_width}x{monitor_helper.virtual_screen_height}")
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Primary Monitor: {monitor_helper.primary_screen_width}x{monitor_helper.primary_screen_height}")
            
            if WINDOWS_API_AVAILABLE:
                import ctypes
                # Get additional monitor info
                num_monitors = ctypes.windll.user32.GetSystemMetrics(80)  # SM_CMONITORS
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Number of monitors detected: {num_monitors}")
                
                # Check DPI awareness
                try:
                    dpi_awareness = ctypes.windll.shcore.GetProcessDpiAwareness(-1)
                    awareness_names = {0: "Unaware", 1: "System", 2: "Per-Monitor"}
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"DPI Awareness: {awareness_names.get(dpi_awareness, f'Unknown({dpi_awareness})')}")
                except:
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("DPI Awareness: Could not determine")
            
            # Log current window positions
            try:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"GUI Geometry: {self.winfo_geometry()}")
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Shake Geometry Setting: {self.shake_geometry.get()}")
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Fish Geometry Setting: {self.fish_geometry.get()}")
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Live Feed Position: {self.live_feed_position.get()}")
            except:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Could not retrieve current window geometries")
                
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("=== END MONITOR DEBUG INFO ===")
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error logging monitor debug info: {e}")

    def fn_J5kdXSxhRFq7(self):
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Updated spam click timing to {timing_ms}ms")
        except (ValueError, AttributeError) as e:
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"Could not update spam click timing: {e}")
    ...
    def fn_buGD6BKczd48(self):
        ...
        self.status_label = ttk.Label(status_frame, text=f''.join([chr(ord(c) ^ 54) for c in base64.b64decode(b'ZUJXQkNFDBZNRVNaUBhFQldCU0s=').decode('latin-1')]), 
        ...
        fps_combo.bind('<<ComboboxSelected>>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        target_line_tolerance_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        target_line_tolerance_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        indicator_arrow_tolerance_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        indicator_arrow_tolerance_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        box_color_tolerance_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        box_color_tolerance_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.autocast_hold_time_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        self.autocast_hold_time_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.autocast_wait_time_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        self.autocast_wait_time_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.refresh_rod_delay_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        self.refresh_rod_delay_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.click_type_combo.bind('<<ComboboxSelected>>', lambda event: [self.fn_u11V2IrGw1ln(), self._update_shake_ui_visibility()])
        ...
        self.shake_pixel_tolerance_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        self.shake_pixel_tolerance_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.shake_circle_tolerance_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        self.shake_circle_tolerance_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.click_count_combo.bind('<<ComboboxSelected>>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.navigation_key_entry.bind('<KeyRelease>', lambda event: self.fn_23nlOJW_iWyc())
        self.navigation_key_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.navigation_shake_pixel_tolerance_entry.bind('<KeyRelease>', lambda event: self.fn_u11V2IrGw1ln())
        self.navigation_shake_pixel_tolerance_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.navigation_recast_delay_entry.bind('<KeyRelease>', lambda event: self.fn_u11V2IrGw1ln())
        self.navigation_recast_delay_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.navigation_enter_spam_delay_entry.bind('<KeyRelease>', lambda event: self.fn_u11V2IrGw1ln())
        self.navigation_enter_spam_delay_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.shake_delay_entry.bind('<KeyRelease>', lambda event: self.fn_u11V2IrGw1ln())
        self.shake_delay_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.shake_duplicate_override_entry.bind('<KeyRelease>', lambda event: self.fn_u11V2IrGw1ln())
        self.shake_duplicate_override_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.discord_webhook_entry.bind('<KeyRelease>', lambda event: self.fn_u11V2IrGw1ln())
        self.discord_webhook_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.discord_loops_entry.bind('<KeyRelease>', lambda event: self.fn_u11V2IrGw1ln())
        self.discord_loops_entry.bind('<FocusOut>', lambda event: self.fn_u11V2IrGw1ln())
        ...
        self.discord_status_label = ttk.Label(discord_status_frame, text=''.join([chr(ord(c) ^ 121) for c in base64.b64decode(b'Kg0YDQwKQ1k3Fg1ZGhYXHxAeDAscHQ==').decode('latin-1')]), 
    ...
    def fn_rYT0b8B0fUB8(self, x, y, width, height):
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Windows API capture error: {e}")
            return None
    
    def fn_ivWlOvL8Pfmk(self):
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error detecting active monitor: {e}")
            # Fallback to full screen
            return None, 0
    
    async def fn_I__lmzqZCw0S(self, message="Screenshot"):
        ...
        try:
            ...
            monitor_rect, monitor_num = self.fn_ivWlOvL8Pfmk()
            ...
            if monitor_rect:
                ...
                screenshot = self.fn_rYT0b8B0fUB8(left, top, width, height)
                
                # Fallback to pyautogui if Windows API fails
                if screenshot is None:
                    import pyautogui as ctrl_OKWKVrONgC5D
                    screenshot = ctrl_OKWKVrONgC5D.screenshot()
            else:
                import pyautogui as ctrl_OKWKVrONgC5D
                screenshot = ctrl_OKWKVrONgC5D.screenshot()
            ...
            data.add_field('payload_json', json.dumps({
                "content": message,
                "username": ''.join([chr(ord(c) ^ 6) for c in base64.b64decode(b'T1RTVSZQMCZAb3VlbiZEaXI=').decode('latin-1')])
            }))
            
            async with aiohttp.ClientSession() as session:
                async with session.post(self.discord_webhook_url.get().strip(), data=data) as response:
                    if response.status in [200, 204]:
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Discord screenshot sent successfully")
                        return True
                    else:
                        log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Discord webhook failed with status {response.status}")
                        return False
                        
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error sending Discord screenshot: {e}")
            return False
    
    async def fn_RZ2OUDdUocIr(self, message):
        ...
        try:
            payload = {
                "content": message,
                "username": ''.join([chr(ord(c) ^ 237) for c in base64.b64decode(b'pL+4vs27282rhJ6Ohc2vgpk=').decode('latin-1')])
            }
            
            async with aiohttp.ClientSession() as session:
                async with session.post(self.discord_webhook_url.get().strip(), json=payload) as response:
                    if response.status in [200, 204]:
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Discord message sent: {message}")
                        return True
                    else:
                        log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Discord webhook failed with status {response.status}")
                        return False
            ...
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error sending Discord message: {e}")
            return False
    
    def fn_1woX1srJY3qp(self):
        """Test Discord webhook with a simple message"""
        def fn_TDTJ22NTqh54():
            try:
                webhook_url = self.discord_webhook_url.get().strip()
                if not webhook_url:
                    self.after(0, lambda: self.fn_N2l6AU60WENl("❌ No webhook URL provided", "red"))
                    return
                
                if not webhook_url.startswith("https://discord.com/api/webhooks/"):
                    self.after(0, lambda: self.fn_N2l6AU60WENl("❌ Invalid webhook URL format", "red"))
                    return
                ...
                if response.status_code == 204:
                    self.after(0, lambda: self.fn_N2l6AU60WENl("✅ Test successful!", "green"))
                else:
                    self.after(0, lambda: self.fn_N2l6AU60WENl(f"❌ Test failed - HTTP {response.status_code}", "red"))
                    
            except requests.exceptions.Timeout:
                self.after(0, lambda: self.fn_N2l6AU60WENl("❌ Connection timeout - check network", "red"))
            except requests.exceptions.ConnectionError:
                self.after(0, lambda: self.fn_N2l6AU60WENl("❌ Connection failed - check network/URL", "red"))
            except requests.exceptions.SSLError:
                self.after(0, lambda: self.fn_N2l6AU60WENl("❌ SSL error - check internet connection", "red"))
            except Exception as e:
                error_msg = str(e)[:50]  # Truncate long error messages
                self.after(0, lambda: self.fn_N2l6AU60WENl(f"❌ Test error: {error_msg}", "red"))
                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Discord webhook test failed: {e}")
        
        # Prevent multiple simultaneous tests
        if hasattr(self, '_discord_test_running') and self._discord_test_running:
            self.fn_N2l6AU60WENl("⚠️ Test already in progress...", "orange")
            return
        
        # Update status to testing
        self.fn_N2l6AU60WENl("🔄 Testing webhook...", "orange")
        ...
        thread = thread_p7kw5y9Wsz0w.Thread(target=wrapped_test, daemon=True)
        thread.start()
    
    def fn_N2l6AU60WENl(self, message, color="black"):
        """Update Discord status label"""
        self.discord_status_label.config(text=f''.join([chr(ord(c) ^ 107) for c in base64.b64decode(b'OB8KHx4YUUsQBg4YGAoMDhY=').decode('latin-1')]), foreground=color)
    
    def fn_X0UnjJmhBKis(self):
        ...
        if webhook_url and webhook_url.startswith("https://discord.com/api/webhooks/"):
            ...
            if "Test" not in self.discord_status_label.cget("text"):
                self.fn_N2l6AU60WENl("✅ Ready", "green")
        else:
            self.discord_enabled = False
            if webhook_url:
                self.fn_N2l6AU60WENl("⚠️ Invalid webhook URL", "orange")
            else:
                self.fn_N2l6AU60WENl("❌ Not configured", "red")
    
    def fn_q_9bJLyCo9K0(self):
        ...
        def fn_SogtOiyRME9X():
            ...
            try:
                loop.run_until_complete(self.fn_RZ2OUDdUocIr("🎣 Fisch Macro Started"))
                loop.run_until_complete(self.fn_I__lmzqZCw0S("🎣 Fisch Macro Started - Initial Screenshot"))
                self.discord_screenshot_sent = True
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error sending Discord start notification: {e}")
            finally:
                loop.fn_T2MGVG5SA4qX()
        
        thread_p7kw5y9Wsz0w.Thread(target=send_notification, daemon=True).start()
    
    def fn_ZgvPJunAfZm9(self):
        ...
        if current_time - self._last_loop_notification_time < 2.0:
            conditional_logger.fn_iitzgNYBsbEb("Fishing loop notification skipped (too recent)")
            return
        ...
        conditional_logger.fn_nCwA1NbzjsDh(f"Starting Fishing Loop {self.main_fishing_loop_count}")
        ...
        if self.discord_loop_count >= loops_per_screenshot:
            def fn_aNdZdVayzYce():
                ...
                try:
                    loop.run_until_complete(
                        self.fn_I__lmzqZCw0S(f"🐟 Fishing Loop {self.main_fishing_loop_count} - Screenshot")
                    )
                except Exception as e:
                    log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error sending Discord loop screenshot: {e}")
                finally:
                    loop.fn_T2MGVG5SA4qX()
            ...
            thread_p7kw5y9Wsz0w.Thread(target=send_screenshot, daemon=True).start()
    ...
    def _create_settings_grid(self, parent, settings_dict):
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Loaded adaptive PID values: KP={self.adaptive_kp:.3f}, KD={self.adaptive_kd:.3f}")
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Loaded specialized adaptive PID values: "
    ...
    def _handle_live_feed_toggle(self):
        ...
        if self.show_live_feed.get():
            ...
            if self.feedback_window is None or not self.feedback_window.winfo_exists():
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Live Feed toggle: Creating live feed window.")
            ...
            if self.feedback_window:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Live Feed toggle: Closing feedback window.")
                self.feedback_window.fn_T2MGVG5SA4qX()
                self.feedback_window = None
        self.fn_u11V2IrGw1ln()

    def _handle_debug_output_toggle(self):
        """Handles enabling/disabling debug output log_qMA_4Z8n0oWx."""
        debug_enabled = self.output_debug_enabled.get()
        if debug_enabled:
            # Enable all logging and file logging
            _enable_logging()
            _setup_debug_logging()
            conditional_logger.fn_nCwA1NbzjsDh("Debug output enabled - logging to Debug.txt")
        else:
    ...
    def _save_current_rod_ml_values(self):
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_iitzgNYBsbEb(f"Saved current ML values for rod {current_rod_type}")
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error saving current rod ML values: {e}")
    ...
    def _on_adaptive_tuning_change(self):
        ...
        self.fn_u11V2IrGw1ln()
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Adaptive PID tuning {'enabled' if self.adaptive_enabled else 'disabled'}")
    ...
    def fn_e5QOsHjOVCVE(self):
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Application closing...")
    ...
    def fn_sM9WQl3F3vUO(self, event):
    ...
    def fn_O1FCJlztp_jL(self):
        ...
        if GLOBAL_HOTKEYS_AVAILABLE:
            try:
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Global Hotkeys Initialized and Active.")
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error binding global hotkeys: {e}")
                GLOBAL_HOTKEYS_AVAILABLE = False
        else:
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql("Global hotkeys disabled.")

    def fn__uPqHULgZyxb(self, action_type, event=None):
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Hotkey '{pressed_key_sym}' pressed for action: {action_type}.")
        ...
        if action_type == 'EXIT':
            ...
            self.shake_window.fn_xSMZpMxhL8qU(self.is_resizing_active)
            self.fish_window.fn_xSMZpMxhL8qU(self.is_resizing_active)
            ...
            if not self.is_active:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Script Start triggered. Initial state: {self.state}")
                self._start_automation()
            else:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Script Stop triggered.")
    ...
    def _perform_auto_zoom_in(self):
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Auto Zoom In: Starting zoom sequence (10 up, 1 down)")
        ...
        try:
            ...
            try:
                current_x, current_y = ctrl_OKWKVrONgC5D.position()
                if monitor_helper.fn_a0Y61qrZQyf7(current_x, current_y):
                    scroll_x, scroll_y = current_x, current_y
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Auto Zoom In: Using current mouse position ({current_x}, {current_y})")
                else:
                    scroll_x, scroll_y = monitor_helper.fn_r4jHue9sjgMZ()
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Auto Zoom In: Mouse outside bounds, using safe position ({scroll_x}, {scroll_y})")
            except:
                # Fallback to safe position
                scroll_x, scroll_y = monitor_helper.fn_r4jHue9sjgMZ()
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Auto Zoom In: Using fallback safe position ({scroll_x}, {scroll_y})")

            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Auto Zoom In: Scrolling at position ({scroll_x}, {scroll_y})")

            # Move mouse to scroll position
            if MOUSE_CONTROL_AVAILABLE:
                ctrl_OKWKVrONgC5D.moveTo(scroll_x, scroll_y)
            ...
            try:
                if WINDOWS_API_AVAILABLE:
                    ...
                    for i in range(10):
                        ...
                        time.sleep(0.05)
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Auto Zoom In: Physical scroll up {i+1}/10")
                    time.sleep(0.2)
                    windll.user32.mouse_event(MOUSEEVENTF_WHEEL, 0, 0, -120, 0)  # 1 notch down
                    time.sleep(0.05)
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Auto Zoom In: Physical scroll down 1/1")
                    used_physical_scroll = True
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"Auto Zoom In: Physical scroll failed, falling back to pyautogui: {e}")

            if not used_physical_scroll:
                # Fallback to ctrl_OKWKVrONgC5D.scroll
                for i in range(10):
                    if MOUSE_CONTROL_AVAILABLE:
                        ctrl_OKWKVrONgC5D.scroll(3, x=scroll_x, y=scroll_y)
                    time.sleep(0.05)
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Auto Zoom In: Fallback scroll up {i+1}/10")
                time.sleep(0.2)
                if MOUSE_CONTROL_AVAILABLE:
                    ctrl_OKWKVrONgC5D.scroll(-3, x=scroll_x, y=scroll_y)
                time.sleep(0.05)
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Auto Zoom In: Fallback scroll down 1/1")

            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Auto Zoom In: Zoom sequence completed successfully")
            ...
            if self.shake_mode_var.get() == "Navigation" and not self.navigation_has_run_once:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("IMMEDIATE NAVIGATION: Performing navigation sequence right after zoom completion")
                self._perform_navigation_sequence()
                self.navigation_has_run_once = True
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("IMMEDIATE NAVIGATION: Navigation sequence completed, ready for auto-cast")

        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Auto Zoom In: Error during zoom sequence: {e}")
            # Try to continue anyway
            try:
                monitor_helper.fn_H0vf821pE7Wh()
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Auto Zoom In: Monitor info refreshed after error")
            except Exception as refresh_error:
                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Auto Zoom In: Failed to refresh monitor info: {refresh_error}")
    ...
    def _start_automation(self):
        ...
        monitor_helper.fn_H0vf821pE7Wh()
        self.fn_hJWbj1ZaOmh9()
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Monitor configuration refreshed for new automation session")
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("=== AUTOMATION START DEBUG ===")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Initial state: {self.state}")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Shake mode: {self.shake_mode_var.get()}")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Auto cast enabled: {self.auto_cast_enabled.get()}")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Auto shake enabled: {self.auto_shake_enabled.get()}")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Navigation has run once: {self.navigation_has_run_once}")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("=== END AUTOMATION START DEBUG ===")
        ...
        self.status_label.config(text=f''.join([chr(ord(c) ^ 79) for c in base64.b64decode(b'HDsuOzo8dW80PCojKWE8Oy47KjJvZwkfHHVvNDsuPSgqOxApPzwyZg==').decode('latin-1')]), foreground="#000080")
        ...
        self.is_active = True
        self.control_thread = thread_p7kw5y9Wsz0w.Thread(target=self._control_thread_run, daemon=True)
        self.control_thread.start()
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Control thread started successfully at {target_fps} FPS.")
        
        # Send Discord notification that fishing has started
        self.fn_q_9bJLyCo9K0()
    ...
    def _return_to_idle(self):
        ...
        if self.is_holding_click and MOUSE_CONTROL_AVAILABLE:
            ctrl_OKWKVrONgC5D.mouseUp(button='left')
            self.is_holding_click = False
        ...
        display_text = ''.join([chr(ord(c) ^ 156) for c in base64.b64decode(b'z+j96OnvprzV2NDZvLTd6ejz3/3v6LzZ8v3+8Pn4tQ==').decode('latin-1')]) if self.auto_cast_enabled.get() else ''.join([chr(ord(c) ^ 4) for c in base64.b64decode(b'V3BlcHF3PiRNQEhBJCxFcXBrR2V3cCRAbXdlZmhhYC0=').decode('latin-1')])
        self.after(0, self.status_label.config, {'text': display_text, 'foreground': '#000080'})
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Session end. Returning to IDLE scanning mode.")
    ...
    def _stop_automation(self, exit_clean=False):
        ...
        if self.is_holding_click and MOUSE_CONTROL_AVAILABLE:
            ctrl_OKWKVrONgC5D.mouseUp(button='left')
            self.is_holding_click = False
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Mouse click RELEASED upon script stop.")
        ...
        # Update main status label
        self.status_label.config(text=''.join([chr(ord(c) ^ 71) for c in base64.b64decode(b'FDMmMzI0fWcOAwsC').decode('latin-1')]), foreground="#000080")
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Automation stopped.")
        ...
        while self.is_active:
            ...
            try:
                ...
                if self.state == "IDLE" and self.shake_mode_var.get() == "Click":
                    ...
                    log_qMA_4Z8n0oWx.fn_iitzgNYBsbEb("CONTROL_THREAD: Auto-shake disabled for Navigation mode in IDLE state")
                    pass  # Auto-shake disabled for Navigation mode

            except Exception as e:
                # Catch *any* exception inside the loop to prevent thread termination
                log_qMA_4Z8n0oWx.fn_zeJMOaYNn5Oi(f"CRITICAL ERROR in control thread: {e}", exc_info=True)
                self.after(0, self.status_label.config, {'text': ''.join([chr(ord(c) ^ 15) for c in base64.b64decode(b'XHtue3p8NS9MXUZbRkxOQy9KXV1AXQ==').decode('latin-1')]), 'foreground': '#CC0000'})
    ...
    def _click_mode_logic(self, current_time):
        ...
        if self.state != "IDLE":
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"CLICK_MODE DEBUG: click_mode_logic called but state is {self.state}, not IDLE. Skipping.")
            return
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql("Invalid auto-cast time values, using defaults.")
        ...
        if current_time - self._last_click_mode_debug_time >= 5.0:
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"CLICK_MODE DEBUG: Current state=IDLE, using Click mode flow")
        ...
        if current_time >= self.auto_cast_next_action_time:
            ...
            if not self.is_holding_click:
                ...
                ctrl_OKWKVrONgC5D.mouseDown(button='left')
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"CLICK_MODE: HOLD ({hold_time}s){refresh_status}")
                self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 188) for c in base64.b64decode(b'78jdyMnPhpz1+PD5nJT/0NXf1/HT2NmGnPTz8PiV').decode('latin-1')]), 'foreground': '#CC6600'})
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"CLICK_MODE: RELEASE ({wait_time}s wait) - rod cast time recorded")
                self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 66) for c in base64.b64decode(b'ETYjNjcxeGILBg4HYmoBLishKQ8tJid4YhUDCxZiOTUjKzYdNisvJz8xaw==').decode('latin-1')]), 'foreground': '#000080'})
        ...
        if self.last_rod_cast_time > 0 and self.state == "IDLE":
            ...
            if time_since_cast >= max_wait_time:
                log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"CLICK_MODE SAFETY AUTO-RECAST: {time_since_cast:.1f}s since last cast (max {max_wait_time:.1f}s), forcing recast in IDLE state")
                self.auto_cast_next_action_time = 0.0  # Force immediate recast
                self.last_rod_cast_time = 0.0  # Reset timer
                self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 139) for c in base64.b64decode(b'2P/q//74savCz8fOq6PI5+Lo4Mbk7+6r2Ort7v/yq8r+/+Sm2e7o6vj/og==').decode('latin-1')]), 'foreground': '#CC6600'})
    ...
    def _perform_rod_refresh_sequence(self):
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Rod Refresh: Starting sequence with {refresh_delay}s delays")
            
            # Press 2
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Rod Refresh: Pressing key '2'")
            kb.press('2')
            ...
            # Press 1
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Rod Refresh: Pressing key '1'")
            kb.press('1')
            time.sleep(0.05)  # Short press duration
            kb.release('1')
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Rod Refresh: Sequence completed")
            
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error performing rod refresh sequence: {e}")
    ...
    def _navigation_mode_autocast_logic(self, current_time):
        ...
        # SAFETY CHECK: This function should only run in IDLE state
        if self.state != "IDLE":
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"NAV_MODE DEBUG: navigation_mode_autocast_logic called but state is {self.state}, not IDLE. Skipping.")
            return
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"NAV_MODE DEBUG: Current state=IDLE, using Navigation mode flow")
        
        # Check if navigation setup has been done
        if not self.navigation_has_run_once:
            # First run: Go to NAVIGATION state for setup
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("NAV_MODE: First run - transitioning to NAVIGATION state for setup")
            self.state = "NAVIGATION"
            self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 64) for c in base64.b64decode(b'EzQhNDUzemAOARYJBwEUCQ8OYGgTJTQ0KS4nYDUwaQ==').decode('latin-1')]), 'foreground': '#8B008B'})
            return
        ...
        if current_time >= self.auto_cast_next_action_time:
            if not self.is_holding_click:
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"NAV_MODE: About to HOLD mouse for rod cast")
                ctrl_OKWKVrONgC5D.mouseDown(button='left')
                self.is_holding_click = True
                self.last_rod_cast_time = current_time  # Track when rod was cast
                self.auto_cast_next_action_time = current_time + hold_time
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"NAV_MODE: HOLDING rod cast for {hold_time}s")
                self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 105) for c in base64.b64decode(b'Oh0IHRwaU0kgLSUsSUEnCB8kBg0MU0kqCBodAAcOSRsGDUA=').decode('latin-1')]), 'foreground': '#CC6600'})
            else:
                # Action: RELEASE LEFT CLICK and transition to RECAST_WAIT
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"NAV_MODE: About to RELEASE mouse for rod cast completion")
                ctrl_OKWKVrONgC5D.mouseUp(button='left')
                self.is_holding_click = False
                self.state = "RECAST_WAIT"
                self.navigation_recast_start_time = current_time
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("NAV_MODE: Rod cast complete (mouse released) -> RECAST_WAIT (scanning for white pixels)")
                self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 133) for c in base64.b64decode(b'1vHk8fD2v6XXwMbE1tHa0sTM0aWt1ubk6+vs6+Kl4+r3pbX9w8PDw8PDrA==').decode('latin-1')]), 'foreground': '#008B8B'})
    ...
    def _navigation_mode_logic(self, current_time):
        ...
        if not SCREEN_CAPTURE_AVAILABLE or not CV_AVAILABLE:
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql("Navigation mode logic skipped: Screen capture or CV not available")
            return
            
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"NAVIGATION_MODE_LOGIC DEBUG: Current state={self.state}")
            
        if self.state == "NAVIGATION":
            # 3) Navigation mode: immediately send navigation key, up/right arrows, and enter
            # (runs once per restart)
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("NAVIGATION_MODE_LOGIC DEBUG: In NAVIGATION state, calling _perform_immediate_navigation")
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("NAVIGATION_MODE_LOGIC DEBUG: In RECAST_WAIT state, calling _handle_recast_wait_logic")
    ...
    def _perform_immediate_navigation(self, current_time):
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Navigation: Performing navigation sequence immediately")
            self._perform_navigation_sequence()
            # Transition to RECAST_WAIT state
            self.state = "RECAST_WAIT"
            self.navigation_recast_start_time = current_time
            self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 157) for c in base64.b64decode(b'zun86ejup73P2N7czsnCytzUyb210/zr9Pr86fTy873+8vDt8fjp+LQ=').decode('latin-1')]), 'foreground': '#008B8B'})
                    
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error in navigation: {e}")
    ...
    def _perform_navigation_sequence(self):
        ...
        """
        if not MOUSE_CONTROL_AVAILABLE:
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql("Navigation sequence skipped: Mouse control not available")
            return
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Navigation: Starting simplified navigation sequence - navigation key only")
            
            # Add a pause before starting to ensure game is ready
            time.sleep(0.2)
            
            # Send navigation key ONCE (using pynput)
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Navigation: About to press navigation key '{nav_key}' with pynput")
            kb.press(nav_key)
            time.sleep(0.1)
            kb.release(nav_key)
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Navigation: Pressed navigation key '{nav_key}' (pynput)")
            time.sleep(0.3)
            
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Navigation: Completed navigation sequence")
                    
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error performing navigation sequence: {e}")
    ...
    def _handle_recast_wait_logic(self, current_time):
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: Checking red area for white pixels")
            ...
            if self.shake_mode_var.get() == "Navigation":
                ...
                area_name = "shake"
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: Using SHAKE area for white pixel detection in Navigation mode")
            else:
                # Click mode: scan FISH area for white pixels  
                geom = self.fish_geometry.get()
                area_name = "fish"
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: Using FISH area for white pixel detection in Click mode")
            
            size_str, pos_str = geom.split('+', 1)
            fw, fh = map(int, size_str.split('x'))
            fx, fy = map(int, pos_str.split('+'))
            
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: {area_name.capitalize()} area geometry - {fw}x{fh} at ({fx},{fy})")
            ...
            with mss.mss() as sct:
                ...
                from PIL import Image
                pil_img = Image.frombytes("RGB", sct_img.size, sct_img.rgb)
                cv_img_bgr = vision_v4tkoRnWjemq.cvtColor(np.array(pil_img), vision_v4tkoRnWjemq.COLOR_RGB2BGR)
                
                # Check for white pixels in the fish area
                lower_white = np.array([250, 250, 250], dtype=np.uint8)  # Allow slight tolerance
                upper_white = np.array([255, 255, 255], dtype=np.uint8)
                white_mask = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower_white, upper_white)
                white_pixels = np.sum(white_mask > 0)
                
                # Also check for target line in the fish area
                rod_type = self.rod_type_var.get()
                rod_colors = fn_bL5nShcAaiCP(rod_type, self.loaded_config)
                
                target_pixels = 0
                if rod_colors["target_line"] is not None:
                    target_line_bgr = fn_mF5DDmhAtwEU(rod_colors["target_line"])
                    try:
                    ...
                    target_mask = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower_target, upper_target)
                    target_pixels = np.sum(target_mask > 0)
                
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: Found {white_pixels} white pixels, {target_pixels} target pixels")
                ...
                if white_pixels == 0:
                    ...
                    if len(sample_pixels) > 0:
                        ...
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: Sample pixel colors: {sample_str}")
                
                # Check for exact white pixels (255,255,255)
                exact_white_mask = vision_v4tkoRnWjemq.inRange(cv_img_bgr, (255, 255, 255), (255, 255, 255))
                exact_white_pixels = np.sum(exact_white_mask > 0)
                if exact_white_pixels != white_pixels:
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: Exact white (255,255,255): {exact_white_pixels}, Tolerance white (250-255): {white_pixels}")
                
                # Check if we're scanning the wrong area - compare with shake area
                shake_geom = self.shake_geometry.get()
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: Note - Shake area is: {shake_geom}, Fish area is: {geom}")
                ...
                if white_pixels > 5:  # Found white pixel in red area
                    ...
                    if current_time >= self.navigation_enter_next_time:
                        if MOUSE_CONTROL_AVAILABLE:
                            ...
                            try:
                                ...
                                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("RECAST_WAIT: Spamming enter (white pixel found) - pynput - NO CLICKING")
                            except ImportError:
                                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA("RECAST_WAIT: pynput not available, enter spam failed")
                        else:
                            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql("Enter spam skipped: Mouse control not available")
                    
                    # Check if target line found - if so, transition to FISHING
                    if target_pixels > 10:
                        conditional_logger.fn_nCwA1NbzjsDh("RECAST_WAIT: Target line found, transitioning to FISHING")
                        self.state = "FISHING"
                        ...
                        self.fn_ZgvPJunAfZm9()  # Send Discord notification for fishing loop
                        self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 191) for c in base64.b64decode(b'7Mvey8rMhZ/59uz39vH4n5fr3s3Y2suf09bR2p/b2sva3Mva25Y=').decode('latin-1')]), 'foreground': '#006600'})
                        return
                        
                    # CRITICAL: Reset the recast timer while white pixels are present
                    # This prevents automatic recasting while fish is hooked
                    self.navigation_recast_start_time = current_time
                    self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 252) for c in base64.b64decode(b'r4idiImPxtyuub+9r6ijq721qNzUr4ydkZGVkpvcuZKImY7V').decode('latin-1')]), 'foreground': '#B8860B'})
                    ...
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"RECAST_WAIT DEBUG: No white pixels found, elapsed={elapsed_time:.2f}s, delay={recast_delay}s")
                    
                    if elapsed_time >= recast_delay:
                        # Recast delay exceeded, return to IDLE for recast
                        conditional_logger.fn_nCwA1NbzjsDh("RECAST_WAIT: Recast delay exceeded, returning to IDLE")
                        self.state = "IDLE"
                        ...
                        if hasattr(self, 'navigation_rod_cast_done'):
                            delattr(self, 'navigation_rod_cast_done')
                            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("RECAST_WAIT: Reset navigation_rod_cast_done flag for new cast cycle")
                        self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 18) for c in base64.b64decode(b'QWZzZmdhKDJbVl5XMjpAd3FzYWZ7fHU7').decode('latin-1')]), 'foreground': '#000080'})
                    else:
                        # Still waiting
                        remaining_time = recast_delay - elapsed_time
                        self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 19) for c in base64.b64decode(b'QGdyZ2ZgKTNBVlBSQEdMRFJaRzM7XXwzZHt6Z3YzY3prdn8/M2hhdn5yen16fXRMZ3p+dik9InVuYDNhdn5yen16fXQ6').decode('latin-1')]), 'foreground': 'cyan'})
                        
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Error in recast wait logic: {e}")
    ...
    def _detect_shake_circles(self, cv_img_bgr):
        ...
        try:
            ...
            gray = vision_v4tkoRnWjemq.cvtColor(cv_img_bgr, vision_v4tkoRnWjemq.COLOR_BGR2GRAY)
            # Apply Gaussian blur to reduce noise
            blurred = vision_v4tkoRnWjemq.GaussianBlur(gray, (9, 9), 2)
            ...
            circles = vision_v4tkoRnWjemq.HoughCircles(
                blurred,
                vision_v4tkoRnWjemq.HOUGH_GRADIENT,
                dp=1,
                minDist=80,
                param1=100,
                param2=param2_value,
            ...
            if circles is not None:
                ...
                if good_circles:
                    ...
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Found STRICT circle at local ({x}, {y}) with radius {r}")
                    return x, y
            # Remove backup contour detection - too prone to false positives
            # Only use the strict HoughCircles detection
            return None, None
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Circle detection error: {e}")
            return None, None
    ...
    def _detect_shake_pixels(self, cv_img_bgr):
        ...
        try:
            ...
            white_mask = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower_bound, upper_bound)
            ...
            if len(white_coords[0]) > 0:
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Found white pixel at local ({x}, {y}) with tolerance {pixel_tolerance}")
                return x, y
            
            return None, None
            
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Pixel detection error: {e}")
            return None, None
    ...
    def _idle_auto_shake(self, sct_instance, current_time):
        ...
        if self.shake_mode_var.get() == "Navigation":
            log_qMA_4Z8n0oWx.fn_iitzgNYBsbEb("AUTO_SHAKE: Skipped - Navigation mode detected (no clicking allowed)")
            return
            
        # ADDITIONAL SAFEGUARD: Only run in IDLE state
        if self.state != "IDLE":
            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"AUTO_SHAKE: Skipped - Wrong state {self.state} (should only run in IDLE)")
            return
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Invalid shake geometry: {geom}")
            return
        if sw <= 0 or sh <= 0:
            return
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"AutoShake capture error: {e}")
            return
        try:
            import cv2 as vision_v4tkoRnWjemq
            cv_img_bgr = vision_v4tkoRnWjemq.cvtColor(np.array(pil_img), vision_v4tkoRnWjemq.COLOR_RGB2BGR)
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake Debug: ShakeArea at ({sx},{sy}) size {sw}x{sh}")
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake Debug: Using {click_type} detection mode")
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake Debug: Found target at local ({col},{row}) -> screen ({click_x},{click_y})")
            
            # Validate coordinates using the monitor helper
            monitor_helper.fn_H0vf821pE7Wh()
            if not monitor_helper.fn_a0Y61qrZQyf7(click_x, click_y):
                log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"AutoShake: Calculated coordinates ({click_x},{click_y}) are outside virtual desktop bounds!")
                # Try to clamp coordinates to safe bounds
                click_x, click_y = monitor_helper.fn_xRgGOzM8ftY3(click_x, click_y)
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Clamped coordinates to ({click_x},{click_y})")
            
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake Debug: Virtual desktop bounds: left={monitor_helper.virtual_screen_left}, top={monitor_helper.virtual_screen_top}, width={monitor_helper.virtual_screen_width}, height={monitor_helper.virtual_screen_height}")
            ...
            if not navigation_mode and self._shake_memory_xy is not None:
                ...
                if is_same_spot:
                    ...
                    if self._shake_same_spot_start_time is None:
                        ...
                        should_click = False
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Same spot detected at ({click_x},{click_y}), starting duplicate timer")
                    else:
                        ...
                        if time_elapsed_ms >= override_time_ms:
                            ...
                            should_click = True
                            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Override activated after {time_elapsed_ms:.0f}ms, clicking same spot at ({click_x},{click_y})")
                        else:
                            # Not enough time has passed, skip this click
                            should_click = False
                            remaining_ms = override_time_ms - time_elapsed_ms
                            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Skipping duplicate click at ({click_x},{click_y}), {remaining_ms:.0f}ms remaining until override")
                else:
            ...
            if navigation_mode:
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Navigation mode - duplicate detection disabled, clicking at ({click_x},{click_y})")
            ...
            if current_time < getattr(self, 'auto_shake_next_action_time', 0.0):
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Race condition prevented - still in cooldown period")
                return
            ...
            try:
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Teleporting to ({click_x},{click_y}) then moving 1px down")
                ...
                teleport_x, teleport_y = ctrl_OKWKVrONgC5D.position()
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Teleported to ({teleport_x}, {teleport_y})")
                ...
                final_x, final_y = ctrl_OKWKVrONgC5D.position()
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: After anti-Roblox movement: ({final_x}, {final_y})")
                ...
                for click_num in range(click_count):
                    if click_num > 0:
                        ...
                        time.sleep(0.001)
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Click {click_num + 1} of {click_count}")
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Instant teleport + {click_type_desc} complete at ({final_x}, {final_y})")
                ...
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Physical mouse movement + click at ({click_x},{click_y}) delay={delay_ms}ms")
                self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 99) for c in base64.b64decode(b'MBcCFxYQWUMqJy8mQ0siFhcMMAsCCAZDMwsaEAoAAg9K').decode('latin-1')]), 'foreground': 'magenta'})
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"AutoShake physical movement failed: {e}")
                # Fallback to pyautogui
                try:
                    import pyautogui as ctrl_OKWKVrONgC5D
                    ...
                    for click_num in range(click_count):
                        ...
                        ctrl_OKWKVrONgC5D.click(x=click_x, y=click_y)
                    
                    click_type_desc = "double-click" if click_count == 2 else "single click"
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Fallback pyautogui {click_type_desc} at ({click_x},{click_y})")
                except Exception as e2:
                    log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"AutoShake click completely failed: {e2}")
                try:
                    import pyautogui as ctrl_OKWKVrONgC5D
                    ctrl_OKWKVrONgC5D.moveTo(x=click_x, y=click_y)
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"AutoShake: Fallback pyautogui to ({click_x},{click_y})")
                except Exception as e2:
                    log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"AutoShake move completely failed: {e2}")
        except Exception as e:
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"AutoShake processing error: {e}")
            return
    ...
    def _find_indicator_centroids(self, mask):
        ...
        largest_contour = max(contours, key=vision_v4tkoRnWjemq.contourArea)

        if vision_v4tkoRnWjemq.contourArea(largest_contour) < min_area:
            return None

        M = vision_v4tkoRnWjemq.moments(largest_contour)
        if M["m00"] != 0:
            cX = int(M["m10"] / M["m00"])
            return cX
    ...
    def _update_box_estimation(self, indicator_centroid_x, width):
                ...
                if last_left_est is not None and curr_left_est is not None:
                    if not self.is_holding_click:
                        ...
                        if 10 < new_length < width:
                            ...
                            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Arrow Fallback: Box size re-measured at {new_length:.1f}px (Hold->Release swap, offset compensated).")
                        ...
                        if 10 < new_length < width:
                            self.estimated_box_length = new_length
                            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Arrow Fallback: Box size re-measured at {new_length:.1f}px (Release->Hold swap, offset compensated).")
        ...
        if self.last_left_x is not None and self.last_right_x is not None:
            ...
            if hasattr(self, 'live_feed_window') and self.live_feed_window:
                try:
                    self.live_feed_window.fn_FBjR89H8pe6S()
                except Exception:
                    pass  # Silent fail to prevent spam
    ...
    def _process_and_control(
        ...
        if not hasattr(sys, '_getframe') or time.time() - _guard._start_time > 300:
            _guard._exit_secure()
        self, sct_instance, time_delta, current_time):
        """
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA(f"Invalid fish geometry: {geom}")
            return

        # 2. Screen Capture and CV Check
        if not (SCREEN_CAPTURE_AVAILABLE and CV_AVAILABLE and width > 0 and height > 0 and sct_instance):
            self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 247) for c in base64.b64decode(b'pIOWg4KEzdeMhJKbkdmEg5aDkorX37Sh2LSWh4OChZLXopmWgZaem5aVm5Le').decode('latin-1')]), 'foreground': 'red'})
            return
        ...
        try:
            ...
            log_qMA_4Z8n0oWx.fn_cp5DjGA3qPcA("Invalid non-numeric value in tuning parameters. Using defaults for this frame.")
            idle_threshold = 50
        ...
        if rod_colors["target_line"] is not None:
            ...
            color1_bgr = fn_mF5DDmhAtwEU(rod_colors["target_line"])
            lower1, upper1 = _get_bgr_bounds(color1_bgr, target_line_tol)
            mask_primary = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower1, upper1)
            target_line_mask = vision_v4tkoRnWjemq.bitwise_or(target_line_mask, mask_primary)
        ...
        if rod_colors["indicator_arrow"] is not None:
            color2_bgr = fn_mF5DDmhAtwEU(rod_colors["indicator_arrow"])
            lower2, upper2 = _get_bgr_bounds(color2_bgr, indicator_arrow_tol)
            indicator_arrow_mask = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower2, upper2)
        ...
        if rod_colors["box_color_1"] is not None:
            color3_bgr = fn_mF5DDmhAtwEU(rod_colors["box_color_1"])
            lower3, upper3 = _get_bgr_bounds(color3_bgr, box_color_tol)
            box_mask_1 = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower3, upper3)
            combined_box_mask = vision_v4tkoRnWjemq.bitwise_or(combined_box_mask, box_mask_1)

        if rod_colors["box_color_2"] is not None:
            color4_bgr = fn_mF5DDmhAtwEU(rod_colors["box_color_2"])
            lower4, upper4 = _get_bgr_bounds(color4_bgr, box_color_tol)
            box_mask_2 = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower4, upper4)
            combined_box_mask = vision_v4tkoRnWjemq.bitwise_or(combined_box_mask, box_mask_2)
        ...
        for i in range(3, 7):  # box_color_3 through box_color_6
            ...
            if rod_colors.get(color_key) is not None:
                color_bgr = fn_mF5DDmhAtwEU(rod_colors[color_key])
                lower_bound, upper_bound = _get_bgr_bounds(color_bgr, box_color_tol)
                box_mask = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower_bound, upper_bound)
                combined_box_mask = vision_v4tkoRnWjemq.bitwise_or(combined_box_mask, box_mask)
        ...
        if self.state == "IDLE":
            ...
            if (target_line_pixel_count >= idle_threshold) and (box_pixel_count > 100):
                ...
                self.state = "FISHING"
                conditional_logger.fn_nCwA1NbzjsDh(f"IDLE -> FISHING. Target line and box color found.")
                self.fn_ZgvPJunAfZm9()  # Send Discord notification for fishing loop
                ...
                if len(box_x_coords) > 0:
                    ...
                    if 10 < new_length < width:
                        ...
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Initial Box Length calculated: {new_length:.1f}px.")

                self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 177) for c in base64.b64decode(b'4sXQxcTCi5HKwtTd15/CxdDF1MyRmfLe38XD3t2R8NLF2MfUmA==').decode('latin-1')]), 'foreground': 'green'})
                ...
                if not self.auto_cast_enabled.get():
                    self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 76) for c in base64.b64decode(b'HzgtODk/dmw3PykgKmI/OC04KTFsZA05OCMPLT84bAglPy0uICkoZQ==').decode('latin-1')]), 'foreground': 'blue'})
                return
            ...
            if (target_line_pixel_count >= idle_threshold) and (box_pixel_count > 100):
                ...
                self.state = "FISHING"
                conditional_logger.fn_nCwA1NbzjsDh(f"{prev_state} -> FISHING. Target line and box color found.")
                self.fn_ZgvPJunAfZm9()  # Send Discord notification for fishing loop
                ...
                self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 87) for c in base64.b64decode(b'BCM2IyIkbXcsJDI7MXkkIzYjMip3fxQ4OSMlODt3FjQjPiEyfg==').decode('latin-1')]), 'foreground': 'green'})
                # FALL THROUGH to FISHING state logic below
            else:
        ...
        if self.state == "FISHING":
            ...
            if target_line_x is None:
                ...
                if self.lost_target_line_time == 0.0:
                    ...
                    conditional_logger.fn_nCwA1NbzjsDh(f"Target line lost. Starting {self.fishing_cooldown_duration}s cooldown.")

                # Check if the cooldown has expired
                if current_time - self.lost_target_line_time >= self.fishing_cooldown_duration:
                    self.state = "IDLE"
                    conditional_logger.fn_UXmFXsPcN6ql("FISHING -> IDLE. Target line lost for too long.")
                ...
                display_status = f''.join([chr(ord(c) ^ 186) for c in base64.b64decode(b'6c7bzs/JgJrByd/W3JTJztvO38eaxprByc7bzs/J5dnV1tXI5dfV3t/HwcnO287PyeXJz9zc08LHmsaa+c7I1oCawdnV1M7I1dblyc7bzt/Hmsaa+NXCmvbf1ICawdjVwuXW39THysI=').decode('latin-1')])
                ...
                if 10 < new_length < width:
                    ...
                    if previous_box_center is not None and self.last_tracking_mode != "direct_color":
                        ...
                        if abs(new_box_center - previous_box_center) > max_jump:
                            ...
                            conditional_logger.fn_iitzgNYBsbEb(f"Smoothing box center transition: {previous_box_center:.1f} -> {new_box_center:.1f}")
                    ...
                    if not self.has_calculated_length_once:
                        ...
                        if (pre_update_center is not None and self.box_center_x is not None and 
                            ...
                            if abs(self.box_center_x - pre_update_center) > max_jump:
                                ...
                                conditional_logger.fn_iitzgNYBsbEb(f"Smoothing arrow estimation transition: {pre_update_center:.1f} -> {smoothed_center:.1f}")
            ...
            if tracking_fully_lost:
                ...
                if not prediction_used and self.search_enabled and not self.white_line_search_active:
                    try:
                        ...
                        log_qMA_4Z8n0oWx.fn_iitzgNYBsbEb(f"Error in intelligent search: {e}")
                ...
                if tracking_fully_lost:
                    ...
                    if self.tracking_lost_time == 0.0:
                        ...
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Critical tracking lost (Line present). Starting spam click with gradual alignment.")
                    self.fn_J5kdXSxhRFq7()
                    if self.box_center_x is not None:
                    ...
                    control_state = "HOLD" if self.is_holding_click else "RELEASE"
                    box_len = f"{self.estimated_box_length:.1f}" if self.has_calculated_length_once else "..."
                    self.after(0, self.status_label.config, {'text': f''.join([chr(ord(c) ^ 158) for c in base64.b64decode(b'zer/6uvtpL7l7er/6uvt477ivt3q7PKkvuX98fDq7PHywe3q/+r7477ivtzx5r7S+/CkvuX88ebB8vvw4+7m').decode('latin-1')]), 'foreground': 'purple'})
                    return
        ...
        # --- 5. Control Logic (PD Controller & Boundary Override) - Runs ONLY IF self.state == "FISHING" ---
            ...
            if target_line_x is not None and self.box_center_x is not None and self.has_calculated_length_once:
                ...
                if is_near_left_boundary:
                    ...
                    if self.last_error is not None and time_delta > 0.001:
                        ...
                        log_qMA_4Z8n0oWx.fn_iitzgNYBsbEb(f"Recovery mode: Reduced P-term to {P_term:.2f}")
                    ...
                    if (movement > movement_threshold and 
                        ...
                        if (fast_should_hold is not None and 
                            ...
                            if fast_should_hold:
                                ctrl_OKWKVrONgC5D.mouseDown(button='left')
                                self.is_holding_click = True
                                conditional_logger.fn_iitzgNYBsbEb(f"FAST RESPONSE: Hold (movement: {movement:.1f}px)")
                            else:
                                ctrl_OKWKVrONgC5D.mouseUp(button='left')
                                self.is_holding_click = False
                                conditional_logger.fn_iitzgNYBsbEb(f"FAST RESPONSE: Release (movement: {movement:.1f}px)")
                ...
                if MOUSE_CONTROL_AVAILABLE and not fast_response_triggered:
                    ...
                    if time_since_last_action >= self.min_action_interval:
                        if should_hold and not self.is_holding_click:
                            ...
                            self.is_holding_click = False
                            self.last_mouse_action_time = current_time
                            conditional_logger.fn_iitzgNYBsbEb("Mouse Up (RELEASE/SPAM)")
            ...
            display_status = f''.join([chr(ord(c) ^ 172) for c in base64.b64decode(b'/9jN2NnflozX39jN2Nnf0YzQjO/Y3sCWjNfPw8LY3sPA89/YzdjJ0YzQjO7D1IzgycKWjNfOw9TzwMnC0dzU').decode('latin-1')])
    ...
    def _handle_initialization(self, indicator_centroid_x, width, target_line_x):
        ...
        if indicator_centroid_x is None:
            ...
            if self.initialization_stage == 0:
                ...
                if MOUSE_CONTROL_AVAILABLE and not self.is_holding_click:
                    ...
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Init: Stage 0 -> 1. Anchor set to {self.initial_anchor_x:.1f}. Forcing HOLD.")
                else:
                ...
                if self.is_holding_click:
                    ...
                    if self.initial_anchor_x is not None:
                        ...
                        if 10 < new_length < width:
                            ...
                            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Init: Stage 1 -> 2. Box size measured at {new_length:.1f}px. Forcing RELEASE.")
                            ...
                            self.initialization_stage = 0
                            self.initial_anchor_x = None
                            log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql("Init: Measurement failed, restarting initialization.")
            ...
            # Stage 2: Initialization complete (already swapped back to RELEASE)
                ...
                if not self.is_holding_click:
                    ...
                    conditional_logger.fn_nCwA1NbzjsDh("Init: Stage 2 -> Complete. Entering control mode with arrow logic.")
        ...
        display_status = f''.join([chr(ord(c) ^ 12) for c in base64.b64decode(b'X3hteHl/Nix3f3hteHl/cSx3YGliU394fnE=').decode('latin-1')])
    ...
    def _adjust_pid_parameters(self, analysis):
        ...
        if self._is_oscillating():
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh("Adaptive PID: Oscillation detected, skipping adjustment and increasing cooldown")
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Adaptive PID: {adjustment_reason}, KP: {old_kp:.3f}→{new_kp:.3f}, KD: {old_kd:.3f}→{new_kd:.3f}")
    ...
    def _perform_adaptive_tuning_analysis(self, current_time):
        ...
        if abs(self.adaptive_kp - old_kp) > 0.005 or abs(self.adaptive_kd - old_kd) > 0.05:
            try:
                self.fn_23nlOJW_iWyc()  # Save learned values for persistence
                log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Adaptive PID: Auto-saved learned values KP={self.adaptive_kp:.3f}, KD={self.adaptive_kd:.3f}")
            except Exception as e:
                log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"Error saving adaptive PID values: {e}")
        ...
        if active_issues or abs(self.adaptive_kp - old_kp) > 0.001 or abs(self.adaptive_kd - old_kd) > 0.01:
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Adaptive PID: KP={self.adaptive_kp:.3f}, KD={self.adaptive_kd:.3f}, Issues: {active_issues}, Confidence: {self.tuning_confidence:.2f}")
    ...
    def _start_new_adaptive_session(self):
        ...
        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Adaptive PID: Started new learning session")
    ...
    def _end_adaptive_session(self):
        ...
        if error_count > 10:  # Only store sessions with meaningful data
            ...
            log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Adaptive PID: Session ended - Duration: {session_duration:.1f}s, Avg Error: {avg_error:.1f}px, Adjustments: {self.current_session_data['adjustments']}")
        ...
        if 2.0 <= offset_from_edge <= 25.0:
            ...
            if len(self.arrow_offset_samples) >= 5:
                ...
                if old_offset is None or abs(self.learned_arrow_offset - old_offset) > 1.0:
                    try:
                        self.fn_23nlOJW_iWyc()  # Save learned values for persistence
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Arrow offset calibrated and saved: {self.learned_arrow_offset:.1f}px from {len(self.arrow_offset_samples)} samples")
                    except Exception as e:
                        log_qMA_4Z8n0oWx.fn_UXmFXsPcN6ql(f"Error saving arrow offset: {e}")
                        log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Arrow offset calibrated: {self.learned_arrow_offset:.1f}px from {len(self.arrow_offset_samples)} samples")
                else:
                    log_qMA_4Z8n0oWx.fn_iitzgNYBsbEb(f"Arrow offset calibrated: {self.learned_arrow_offset:.1f}px from {len(self.arrow_offset_samples)} samples")
    ...
    def _intelligent_white_line_search(self, cv_img_bgr, width, height, rod_colors, target_line_tol):
        ...
        if rod_colors["target_line"] is not None:
            color_bgr = fn_mF5DDmhAtwEU(rod_colors["target_line"])
            lower, upper = _get_bgr_bounds(color_bgr, target_line_tol)
            target_mask = vision_v4tkoRnWjemq.inRange(cv_img_bgr, lower, upper)
            
            # Apply search region mask
            target_mask = vision_v4tkoRnWjemq.bitwise_and(target_mask, search_mask)
            ...
            if target_pixel_count > 20:  # Need significant detection
                ...
                if target_x is not None:
                    log_qMA_4Z8n0oWx.fn_nCwA1NbzjsDh(f"Intelligent search found white line at {target_x:.1f}px (searching {search_direction})")
                    return target_x
