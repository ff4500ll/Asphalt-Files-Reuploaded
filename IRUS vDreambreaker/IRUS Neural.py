...
class CentralStorage:
    def __init__(self, config_file="irus_config.json"):
        ...
        self.settings = {
            ...
            "quad_boxes": {
                ...
                "icon_box": {"x1": 764, "y1": 1201, "x2": 1795, "y2": 1211},
            ...
            "fish_settings": {
                ...
                "friend_color_tolerance": 5,
                "scan_fps": 1000,
                "delay_after_fish": 2.0,
                "rods": {
                    "Default": {
                        "icon_color_tolerance": 5,
                        "icon_colors": [],
                        "bar_color_tolerance": 5,
                        "bar_colors": []
                    }
                }
            }
    ...
    def get_shake_setting(self, setting_name):
        ...
        return self.settings["shake_settings"].get(setting_name, defaults.get(setting_name))
    ...
    def get_fish_setting(self, setting_name):
        ...
        defaults = {"friend_color_tolerance": 5, "scan_fps": 1000}
    ...
    def get_current_rod_name(self):
        """Get the name of the currently selected rod"""
        return self.settings["fish_settings"].get("current_rod", "Default")
    
    def set_current_rod(self, rod_name):
        """Set the currently selected rod"""
        self.settings["fish_settings"]["current_rod"] = rod_name
        self.save()
    
    def get_rod_setting(self, rod_name, setting_name):
        """Get a setting for a specific rod"""
        rods = self.settings["fish_settings"].get("rods", {})
        if rod_name not in rods:
            return None
        return rods[rod_name].get(setting_name)
    
    def set_rod_setting(self, rod_name, setting_name, value):
        """Set a setting for a specific rod"""
        if "rods" not in self.settings["fish_settings"]:
            self.settings["fish_settings"]["rods"] = {}
        if rod_name not in self.settings["fish_settings"]["rods"]:
            self.settings["fish_settings"]["rods"][rod_name] = {}
        self.settings["fish_settings"]["rods"][rod_name][setting_name] = value
        self.save()
    
    def get_all_rod_names(self):
        """Get list of all rod names"""
        rods = self.settings["fish_settings"].get("rods", {})
        return list(rods.keys())
    
    def add_rod(self, rod_name):
        """Add a new rod with default settings"""
        if "rods" not in self.settings["fish_settings"]:
            self.settings["fish_settings"]["rods"] = {}
        if rod_name not in self.settings["fish_settings"]["rods"]:
            self.settings["fish_settings"]["rods"][rod_name] = {
                "icon_color_tolerance": 5,
                "icon_colors": [],  # List of RGB colors for rod icon
                "bar_color_tolerance": 5,
                "bar_colors": []  # List of RGB colors for bar
            }
            self.save()
            return True
        return False
    
    def delete_rod(self, rod_name):
        """Delete a rod (cannot delete Default)"""
        if rod_name == "Default":
            return False
        if "rods" in self.settings["fish_settings"] and rod_name in self.settings["fish_settings"]["rods"]:
            del self.settings["fish_settings"]["rods"][rod_name]
            # If deleting current rod, switch to Default
            if self.get_current_rod_name() == rod_name:
                self.set_current_rod("Default")
            self.save()
            return True
        return False
...
class NeuralNetworkGUI(ctk.CTk):
    ...
    def _check_fish_elements(self, x, y):
        ...
        if self.fish_icon_dropdown_open:
            all_icons = self.storage.get_all_rod_names()
            option_height = 30
            ...
            for i, icon_name in enumerate(all_icons):
                ...
                if (dropdown_x <= x <= dropdown_x + dropdown_width and
                    option_y <= adjusted_y <= option_y + option_height):
                    self.storage.set_current_rod(icon_name)
                    self.fish_icon_dropdown_open = False
                    print(f"Selected rod: {icon_name}")
                    return
        ...
        if (add_button_x <= x <= add_button_x + add_button_width and
            ...
            # Prompt for new rod name
            from tkinter import simpledialog
            rod_name = simpledialog.askstring("Add Rod", "Enter rod name:")
            if rod_name and rod_name.strip():
                rod_name = rod_name.strip()
                if self.storage.add_rod(rod_name):
                    print(f"Added rod: {rod_name}")
                    self.storage.set_current_rod(rod_name)
                else:
                    print(f"Rod '{rod_name}' already exists")
            return
        ...
        if (delete_button_x <= x <= delete_button_x + delete_button_width and
            ...
            current_rod = self.storage.get_current_rod_name()
            if current_rod == "Default":
                print("Cannot delete Default rod")
            else:
                from tkinter import messagebox
                result = messagebox.askyesno("Delete Rod", f"Delete rod '{current_rod}'?")
                if result:
                    if self.storage.delete_rod(current_rod):
                        print(f"Deleted rod: {current_rod}")
            return
        ...
        if (button_x <= x <= button_x + button_width_config and
            ...
            current_rod = self.storage.get_current_rod_name()
            print(f"Configure Icon clicked for: {current_rod}")
            self._start_icon_recording(current_rod)
            return
        
        # Configure Bar Button (next to Configure Icon)
        button_x_bar = button_x + button_width_config + 20
        
        if (button_x_bar <= x <= button_x_bar + button_width_config and
            current_y <= adjusted_y <= current_y + button_height_config):
            current_rod = self.storage.get_current_rod_name()
            print(f"Configure Bar clicked for: {current_rod}")
            self._start_bar_recording(current_rod)
            return
        ...
        # Icon Color Tolerance +/- buttons (specific to current rod)
        current_rod = self.storage.get_current_rod_name()
        minus_x = control_x - button_width - 10
        plus_x = control_x + box_width + 10
        button_y_top = current_y - button_height / 2
        ...
        if (minus_x <= x <= minus_x + button_width and
            button_y_top + 10 <= adjusted_y <= button_y_bottom + 10):
            current_val = self.storage.get_rod_setting(current_rod, "icon_color_tolerance")
            if current_val is None:
                current_val = 5
            new_val = max(0, current_val - 1)
            self.storage.set_rod_setting(current_rod, "icon_color_tolerance", new_val)
            print(f"Icon Color Tolerance ({current_rod}): {new_val}")
            return
        ...
        # Bar Color Tolerance +/- buttons (specific to current rod)
        minus_x = control_x - button_width - 10
        plus_x = control_x + box_width + 10
        button_y_top = current_y - button_height / 2
        button_y_bottom = current_y + button_height / 2
        ...
        if (minus_x <= x <= minus_x + button_width and
            button_y_top + 10 <= adjusted_y <= button_y_bottom + 10):
            current_val = self.storage.get_rod_setting(current_rod, "bar_color_tolerance")
            if current_val is None:
                current_val = 5
            new_val = max(0, current_val - 1)
            self.storage.set_rod_setting(current_rod, "bar_color_tolerance", new_val)
            print(f"Bar Color Tolerance ({current_rod}): {new_val}")
            return
        ...
        # Delay After Fish +/- buttons
        minus_x = control_x
        plus_x = control_x + button_width + 10
        button_y_top = current_y
        button_y_bottom = current_y + button_height
        
        if (minus_x <= x <= minus_x + button_width and
            button_y_top + 10 <= adjusted_y <= button_y_bottom + 10):
            current_val = self.storage.get_fish_setting("delay_after_fish")
            new_val = max(0.0, current_val - 0.1)
            self.storage.set_fish_setting("delay_after_fish", new_val)
            print(f"Delay After Fish: {new_val:.1f}s")
            return
    ...
    def _start_icon_recording(self, rod_name):
        """Start recording icon frames for rod"""
        print(f"[Rod Icon Config] Starting recording for '{rod_name}'")
        ...
        import threading
        thread = threading.Thread(target=self._icon_recording_worker, args=(rod_name,), daemon=True)
        thread.start()
    
    def _start_bar_recording(self, rod_name):
        """Start recording bar frames for rod"""
        print(f"[Rod Bar Config] Starting recording for '{rod_name}'")
        ...
        # Start recording in a separate thread
        import threading
        thread = threading.Thread(target=self._bar_recording_worker, args=(rod_name,), daemon=True)
        thread.start()
    
    def _icon_recording_worker(self, rod_name):
        """Worker thread for recording rod icon frames"""
        import time
        import mss
        from PIL import Image
        ...
        print("[Rod Icon Config] Waiting for friend color (green) to be present...")
        ...
        if len(frames) == 0:
            print("[Rod Icon Config] ERROR: No frames captured! Minigame might have ended too quickly.")
        ...
        self.after(0, lambda: self._open_icon_playback(rod_name, frames, "icon"))
    
    def _bar_recording_worker(self, rod_name):
        """Worker thread for recording rod bar frames"""
        import time
        import mss
        from PIL import Image
        
        friend_box_dict = self.storage.get_quad_box('friend_box')
        fish_box_dict = self.storage.get_quad_box('fish_box')  # Use fish_box instead of icon_box
        
        friend_box = (friend_box_dict["x1"], friend_box_dict["y1"], 
                     friend_box_dict["x2"], friend_box_dict["y2"])
        fish_box = {"top": fish_box_dict["y1"], "left": fish_box_dict["x1"],
                   "width": fish_box_dict["x2"] - fish_box_dict["x1"],
                   "height": fish_box_dict["y2"] - fish_box_dict["y1"]}
        
        print("[Rod Bar Config] Waiting for friend color (green) to be present...")
        ...
        with mss.mss() as sct:
            ...
            while True:
                ...
                if has_green:
                    print("[Rod Bar Config] Green detected! Waiting for it to disappear...")
                    break
            ...
            while True:
                ...
                if not has_green:
                    print("[Rod Bar Config] Green disappeared! Starting recording...")
                    break
            ...
            # Step 3: Record frames until green reappears
            frames = []
            print("[Rod Bar Config] Recording frames...")
            ...
            while True:
                ...
                if has_green:
                    print(f"[Rod Bar Config] Recording complete! Captured {len(frames)} frames")
                    break
                
                # Capture fish_box frame (bar area)
                fish_img = sct.grab(fish_box)
                fish_pil = Image.frombytes('RGB', fish_img.size, fish_img.bgra, 'raw', 'BGRX')
                frames.append(fish_pil)
                
                time.sleep(0.05)
        
        # Check if we got frames
        if len(frames) == 0:
            print("[Rod Bar Config] ERROR: No frames captured! Minigame might have ended too quickly.")
            self.after(0, lambda: messagebox.showerror("Recording Failed", 
                "No frames were captured! The minigame ended too quickly.\\nTry again."))
            return
        
        # Open playback window on main thread
        self.after(0, lambda: self._open_icon_playback(rod_name, frames, "bar"))
    
    def _open_icon_playback(self, rod_name, frames, recording_type):
        """Open the frame playback and color selection window
        
        Args:
            rod_name: Name of the rod
            frames: List of PIL Image frames
            recording_type: Either 'icon' or 'bar'
        """
        type_label = "Icon" if recording_type == "icon" else "Bar"
        print(f"[Rod {type_label} Config] Opening playback window with {len(frames)} frames")
        ...
        playback_window.title(f"Configure Rod {type_label}: {rod_name}")
        ...
        def on_canvas_click(event):
            ...
            if 0 <= x < img.width and 0 <= y < img.height:
                ...
                # If this frame already had a color selected, remove the old one
                if frame_idx in frame_selections:
                    old_color = frame_selections[frame_idx][2]
                    if old_color in selected_colors:
                        selected_colors.remove(old_color)
                        print(f"[{type_label} Config] Removed old color: RGB{old_color} from frame {frame_idx + 1}")
                
                # Store new selection for this frame
                frame_selections[frame_idx] = (x, y, color)
                
                # Add new color to selected colors if not duplicate
                if color not in selected_colors:
                    selected_colors.append(color)
                    print(f"[{type_label} Config] Selected color: RGB{color} at frame {frame_idx + 1}")
                
                update_canvas()
        
        canvas.bind("<Button-1>", on_canvas_click)
        
        # Navigation and Clear Frame buttons
        ...
        def clear_frame():
            ...
            if frame_idx in frame_selections:
                ...
                if old_color in selected_colors:
                    selected_colors.remove(old_color)
                    print(f"[{type_label} Config] Cleared color: RGB{old_color} from frame {frame_idx + 1}")
                del frame_selections[frame_idx]
                update_canvas()
        ...
        tk.Button(nav_frame, text="Next (D) →", command=next_frame, 
                 font=("Arial", 12), width=15).pack(side='left', padx=10)
        tk.Button(nav_frame, text="Clear Frame (C)", command=clear_frame, 
                 font=("Arial", 12), width=15, bg='#FF6B6B', fg='white').pack(side='left', padx=10)
        ...
        def submit():
            ...
            # Save colors to storage based on recording type
            if recording_type == "icon":
                self.storage.set_rod_setting(rod_name, "icon_colors", selected_colors)
                print(f"[Rod Icon Config] Saved {len(selected_colors)} colors for '{rod_name}'")
            else:  # bar
                self.storage.set_rod_setting(rod_name, "bar_colors", selected_colors)
                print(f"[Rod Bar Config] Saved {len(selected_colors)} colors for '{rod_name}'")
            playback_window.destroy()
            messagebox.showinfo("Success", f"Saved {len(selected_colors)} {type_label.lower()} colors for '{rod_name}'!")
    ...
    def _execute_fish(self):
        ...
        # Reset fish control state for new minigame
        if hasattr(self, '_fish_control_state'):
            delattr(self, '_fish_control_state')
        if hasattr(self, '_fish_target_gap'):
            delattr(self, '_fish_target_gap')
        if hasattr(self, '_fish_debug_counter'):
            delattr(self, '_fish_debug_counter')
        ...
        try:
            ...
            current_rod = self.storage.get_current_rod_name()
            icon_colors = self.storage.get_rod_setting(current_rod, "icon_colors")
            icon_tolerance = self.storage.get_rod_setting(current_rod, "icon_color_tolerance")
            
            if not icon_colors:
                print(f"[Fish] ERROR: No icon colors configured for rod '{current_rod}'")
                return
            
            print(f"[Fish] Friend color: RGB{tuple(friend_color)}")
            print(f"[Fish] Friend color tolerance: {friend_tolerance}")
            print(f"[Fish] Current rod: {current_rod}")
            print(f"[Fish] Icon colors: {len(icon_colors)} colors")
            print(f"[Fish] Icon color tolerance: {icon_tolerance}")
            
            # Pre-process bar colors into ranges for faster detection
            def merge_colors_into_ranges(colors, tolerance):
                """Merge similar colors into min/max ranges based on tolerance"""
                if not colors:
                    return []
                
                ranges = []
                colors_array = np.array(colors, dtype=np.int16)
                ...
                for i in range(len(colors)):
                    ...
                    # Start a new range with this color
                    range_min = colors_array[i].copy()
                    range_max = colors_array[i].copy()
                    used[i] = True
                    
                    # Find all colors within 2*tolerance of this color (tolerance applies both ways)
                    for j in range(i + 1, len(colors)):
                        if used[j]:
                            continue
                        
                        # Check if color j is within tolerance of current range
                        if np.all(np.abs(colors_array[j] - colors_array[i]) <= 2 * tolerance):
                            # Expand range to include this color
                            range_min = np.minimum(range_min, colors_array[j])
                            range_max = np.maximum(range_max, colors_array[j])
                            used[j] = True
                    
                    # Apply tolerance to range bounds
                    range_min = np.maximum(0, range_min - tolerance)
                    range_max = np.minimum(255, range_max + tolerance)
                    ranges.append((range_min, range_max))
                
                return ranges
            
            icon_color_ranges = merge_colors_into_ranges(icon_colors, icon_tolerance)
            print(f"[Fish] Icon color ranges: {len(icon_color_ranges)} ranges (merged from {len(icon_colors)} colors)")

            # Main detection loop
            consecutive_failures = 0
            last_icon_match_idx = 0  # Cache for neighbor-priority search
            while self.bot_running and not self._should_stop():
                frame = camera.get_latest_frame()
                ...
                if frame is not None:
                    ...
                    # SECOND: Find icon color positions (leftmost and rightmost)
                    icon_x_left = None
                    icon_x_right = None
                    matched_icon_idx = None
                    
                    # Build priority search order: last match + neighbors first, then rest
                    icon_search_order = []
                    neighbor_range = 3  # Check ±3 indices around last match
                    for offset in range(-neighbor_range, neighbor_range + 1):
                        neighbor_idx = last_icon_match_idx + offset
                        if 0 <= neighbor_idx < len(icon_colors):
                            icon_search_order.append(neighbor_idx)
                    # Add remaining indices
                    for idx in range(len(icon_colors)):
                        if idx not in icon_search_order:
                            icon_search_order.append(idx)
                    
                    # Find icon leftmost and rightmost: scan row by row, left to right
                    height_icon, width_icon = icon_area.shape[:2]
                    icon_leftmost_col = None
                    icon_rightmost_col = None
                    
                    # Stack all icon range mins/maxs
                    icon_range_mins = np.array([r[0] for r in icon_color_ranges], dtype=np.uint8)
                    icon_range_maxs = np.array([r[1] for r in icon_color_ranges], dtype=np.uint8)
                    
                    # Find leftmost: scan left to right across all rows
                    for row in range(height_icon):
                        row_pixels = icon_area[row, :, :]
                        for col in range(width_icon):
                            pixel = row_pixels[col, :]
                            # Check if pixel matches any icon color range
                            matches = np.all((pixel >= icon_range_mins) & (pixel <= icon_range_maxs), axis=1)
                            if np.any(matches):
                                icon_leftmost_col = col
                                break
                        if icon_leftmost_col is not None:
                            break
                    
                    # Find rightmost: scan right to left across all rows
                    for row in range(height_icon):
                        row_pixels = icon_area[row, :, :]
                        for col in range(width_icon - 1, -1, -1):
                            pixel = row_pixels[col, :]
                            # Check if pixel matches any icon color range
                            matches = np.all((pixel >= icon_range_mins) & (pixel <= icon_range_maxs), axis=1)
                            if np.any(matches):
                                icon_rightmost_col = col
                                break
                        if icon_rightmost_col is not None:
                            break
                    
                    # If we found matches, use them
                    if icon_leftmost_col is not None and icon_rightmost_col is not None:
                        icon_x_left = icon_box[0] + icon_leftmost_col
                        icon_x_right = icon_box[0] + icon_rightmost_col
                        icon_x_center = (icon_x_left + icon_x_right) // 2
                    else:
                        icon_x_left = None
                    
                    if icon_x_left is None:
                        consecutive_failures += 1
                        continue  # Skip to next frame
                    
                    # THIRD: Find bar lines using vertical line detection (comet method)
                    bar_x_left = None
                    bar_x_right = None
                    
                    # Convert to grayscale
                    height_fish, width_fish = fish_area_crop.shape[:2]
                    gray = cv2.cvtColor(fish_area_crop, cv2.COLOR_BGR2GRAY) if len(fish_area_crop.shape) == 3 else fish_area_crop
                    
                    # Calculate pixel difference between adjacent columns (vertical edge strength)
                    diffs = np.zeros(width_fish, dtype=np.float32)
                    for x in range(1, width_fish):
                        diffs[x] = np.abs(gray[:, x].astype(np.int16) - gray[:, x-1].astype(np.int16)).sum()
                    
                    # Adaptive threshold search to find exactly 4 lines
                    expected_lines = 4
                    min_separation = 10
                    best_lines = []
                    best_score = -999
                    ...
                    for percentile in range(99, 85, -1):
                        ...
                        for x, d in candidates:
                            if not lines or all(abs(x - l[0]) > min_separation for l in lines):
                                lines.append((x, d))
                                if len(lines) >= expected_lines + 3:
                                    break
                        ...
                        score = -abs(len(lines) - expected_lines) * 100 + len(lines)
                        if score > best_score and len(lines) >= expected_lines - 1:
                            best_score = score
                            best_lines = lines
                    
                    # Fallback: use lower threshold if no good candidates
                    if not best_lines:
                        candidates = [(x, diffs[x]) for x in range(1, width_fish-1) if diffs[x] > np.percentile(diffs, 90)]
                        candidates.sort(key=lambda p: p[1], reverse=True)
                        lines = []
                        for x, d in candidates:
                            if not lines or all(abs(x - l[0]) > min_separation for l in lines):
                    ...
                    if len(best_lines) > expected_lines:
                        best_lines.sort(key=lambda p: p[1], reverse=True)
                        best_lines = best_lines[:expected_lines]
                    
                    # Extract X coordinates and sort
                    final_columns = sorted([x for x, d in best_lines])
                    
                    # Initialize target gap detection with stability check
                    if not hasattr(self, '_fish_target_gap'):
                        if not hasattr(self, '_fish_target_gap_candidates'):
                            self._fish_target_gap_candidates = []
                        
                        # Only consider as valid if we have exactly 4 lines
                        if len(final_columns) == 4:
                            # Sort columns to find the 2 middle ones (target zone)
                            sorted_cols = sorted(final_columns)
                            target_left = sorted_cols[1]
                            target_right = sorted_cols[2]
                            target_gap = target_right - target_left
                            
                            # Add to candidates list
                            self._fish_target_gap_candidates.append((sorted_cols.copy(), target_gap))
                            
                            # Need 3 consecutive stable detections before locking in target gap
                            if len(self._fish_target_gap_candidates) >= 3:
                                # Check if last 3 detections are stable (similar gap, similar positions)
                                recent_gaps = [gap for _, gap in self._fish_target_gap_candidates[-3:]]
                                recent_positions = [cols for cols, _ in self._fish_target_gap_candidates[-3:]]
                                
                                # Check gap stability (within 10px)
                                gap_range = max(recent_gaps) - min(recent_gaps)
                                
                                # Check position stability (columns shouldn't jump around too much)
                                position_stable = True
                                for i in range(4):
                                    col_values = [cols[i] for cols in recent_positions]
                                    if max(col_values) - min(col_values) > 30:  # Allow 30px variance
                                        position_stable = False
                                        break
                                
                                if gap_range <= 10 and position_stable:
                                    # Stable detection confirmed - lock in target gap
                                    self._fish_target_gap = int(np.mean(recent_gaps))
                                    avg_cols = [int(np.mean([cols[i] for cols in recent_positions])) for i in range(4)]
                                    print(f"[Fish] Target gap LOCKED: {self._fish_target_gap}px (between columns {avg_cols[1]} and {avg_cols[2]})")
                                    print(f"[Fish] Confirmed with 3 stable detections at: {avg_cols}")
                                else:
                                    # Not stable yet, keep waiting
                                    if len(self._fish_target_gap_candidates) > 10:
                                        # Reset if we have too many unstable readings
                                        print(f"[Fish] Target gap detection unstable after 10 attempts, resetting...")
                                        self._fish_target_gap_candidates = []
                    
                    # Filter out lines that match the target gap (remove target zone lines)
                    if hasattr(self, '_fish_target_gap') and len(final_columns) >= 2:
                        filtered_columns = []
                        gap_tolerance = 3  # Allow ±3 pixels tolerance for gap matching
                        
                        for i, col in enumerate(final_columns):
                            # Check if this column forms a target-gap with any other column
                            is_target_line = False
                            for j, other_col in enumerate(final_columns):
                                if i != j:
                                    gap = abs(other_col - col)
                                    if abs(gap - self._fish_target_gap) <= gap_tolerance:
                                        is_target_line = True
                                        break
                        ...
                        # Debug: Show filtering results occasionally
                        if not hasattr(self, '_fish_debug_counter'):
                            self._fish_debug_counter = 0
                        self._fish_debug_counter += 1
                        
                        if self._fish_debug_counter % 30 == 0:  # Every 30 frames
                            print(f"[Fish Debug] All lines: {sorted(final_columns)} | After filter: {sorted(filtered_columns)} | Target gap: {self._fish_target_gap}px")
                        
                        # Use filtered columns if we still have at least 2 lines
                        if len(filtered_columns) >= 2:
                            final_columns = filtered_columns
                    
                    if len(final_columns) >= 2:
                        bar_x_left = fish_box[0] + min(final_columns)
                        bar_x_right = fish_box[0] + max(final_columns)
                        bar_x_center = (bar_x_left + bar_x_right) // 2
                        
                        # Execute control system with detected positions
                        self._fish_control_system(icon_x_center, bar_x_center)
                        consecutive_failures = 0
                    else:
                        consecutive_failures += 1
                        # When lines not found, alternate mouse input
                        self._fish_control_fallback_alternate()
            ...
            # Delay after fish minigame (even on error)
            delay_after = self.storage.get_fish_setting("delay_after_fish")
            if delay_after > 0:
                print(f"[Fish] Waiting {delay_after:.1f}s after fish minigame")
                self._interruptible_sleep(delay_after)
    ...
    def _fish_control_system(self, icon_x, bar_x):
        """
        Asymmetric PD control system for fish minigame (comet method).
        Bar behaves like a rocket with constant acceleration (no friction).
        Hold left click = accelerate right, Release = accelerate left.
        
        Uses asymmetric damping:
        - Strong damping (2x) when approaching target to prevent overshoot
        - Weak damping (0.5x) when chasing target to allow fast movement
        
        Stability mode: Alternates hold/release for 200ms warmup period
        
        Args:
            icon_x: X coordinate of icon center (target position)
            bar_x: X coordinate of bar center (current position)
        """
        import time
        ...
        if not hasattr(self, '_fish_control_state'):
            self._fish_control_state = {
                'last_bar_x': bar_x,
                'last_icon_x': icon_x,
                'last_error': None,
                'last_time': time.time(),
                'mouse_held': False,
                'stability_check_mode': True,
                'stability_check_samples': [],
                'stability_mode': False,
                'stability_start_pos': None,
                'stability_start_target': None,
                'stability_toggle': False
            }
        
        state = self._fish_control_state
        
        # Calculate time delta
        current_time = time.time()
        dt = current_time - state['last_time']
        
        # PHASE 1: STABILITY CHECK - Wait for pos and target to be stable for 10 scans
        if state['stability_check_mode']:
            state['stability_check_samples'].append((bar_x, icon_x))
            
            # Keep only last 10 samples
            if len(state['stability_check_samples']) > 10:
                state['stability_check_samples'].pop(0)
            
            # Check if we have 10 samples and they're stable (not moving much)
            if len(state['stability_check_samples']) >= 10:
                all_bar_x = [s[0] for s in state['stability_check_samples']]
                all_icon_x = [s[1] for s in state['stability_check_samples']]
                
                # Check stability: positions shouldn't vary by more than 2 pixels
                bar_variance = max(all_bar_x) - min(all_bar_x)
                icon_variance = max(all_icon_x) - min(all_icon_x)
                
                bar_stable = bar_variance <= 2
                icon_stable = icon_variance <= 2
                
                if bar_stable and icon_stable:
                    # Stable! Enter stability mode
                    state['stability_check_mode'] = False
                    state['stability_mode'] = True
                    state['stability_start_pos'] = bar_x
                    state['stability_start_target'] = icon_x
                    print(f"[Fish Control] STABILITY CHECK PASSED - Starting warmup at Pos:{bar_x} Target:{icon_x}")
            
            # During stability check, don't move anything
            print(f"[Fish Control] STABILITY CHECK ({len(state['stability_check_samples'])}/10)")
            
            # Update state and return
            state['last_bar_x'] = bar_x
            state['last_icon_x'] = icon_x
            state['last_time'] = current_time
            return
        
        # PHASE 2: STABILITY MODE - Alternate hold/release until pos OR target moves by 3+ pixels
        if state['stability_mode']:
            pos_moved = abs(bar_x - state['stability_start_pos']) >= 3
            target_moved = abs(icon_x - state['stability_start_target']) >= 3
            
            if pos_moved or target_moved:
                # Exit stability mode - something moved
                state['stability_mode'] = False
                print(f"[Fish Control] STABILITY WARMUP COMPLETE - Starting PD control (Pos moved: {pos_moved}, Target moved: {target_moved})")
            else:
                # Continue alternating
                should_hold = state['stability_toggle']
                state['stability_toggle'] = not state['stability_toggle']
                
                # Apply mouse control
                if should_hold and not state['mouse_held']:
                    windll.user32.mouse_event(MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
                    state['mouse_held'] = True
                elif not should_hold and state['mouse_held']:
                    windll.user32.mouse_event(MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
                    state['mouse_held'] = False
                
                status = "HOLD (→)" if state['mouse_held'] else "RELEASE (←)"
                print(f"[Fish Control] STABILITY WARMUP {status} (Pos:{bar_x} Target:{icon_x})")
        ...
        if state['last_error'] is not None and dt > 0.001:
            error_rate = (error - state['last_error']) / dt
        else:
            error_rate = 0
        
        # PD CONTROLLER (675 method - with proper deadzone on error)
        # Simple PD with boundary override and spam clicking in deadzone
        
        kp = 10.0   # Proportional gain
        kd = 10.0   # Derivative gain
        pd_clamp = 50.0  # Clamp range
        error_deadband = 5.0   # Dead zone in pixels (check error, not control signal)
        
        # Check deadzone FIRST on raw error (before PD calculation)
        if abs(error) <= error_deadband:
            # Within deadzone - spam click (alternate state)
            should_hold = not state['mouse_held']
        else:
            # Outside deadzone - use PD control
            ...
            # Convert to mouse action
            if control_signal > 0:
                should_hold = True
            else:
                should_hold = False
        
        # Apply mouse control using windll
        if should_hold and not state['mouse_held']:
            windll.user32.mouse_event(MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
            state['mouse_held'] = True
        elif not should_hold and state['mouse_held']:
        ...
        # Single consolidated debug print per loop
        status = "HOLD (→)" if state['mouse_held'] else "RELEASE (←)"
        if abs(error) <= error_deadband:
            print(f"[Fish Control] Pos:{bar_x:4d} Target:{icon_x:4d} Err:{error:+4d} DEADZONE SPAM {status}")
        else:
            p_val = kp * error if abs(error) > error_deadband else 0
            d_val = kd * error_rate if abs(error) > error_deadband else 0
            ctrl_val = max(-pd_clamp, min(pd_clamp, p_val + d_val))
            print(f"[Fish Control] Pos:{bar_x:4d} Target:{icon_x:4d} Err:{error:+4d} P:{p_val:+5.1f} D:{d_val:+6.1f} Ctrl:{ctrl_val:+6.1f} {status}")
    ...
    def _fish_control_fallback_alternate(self):
        """
        Fallback control when lines can't be detected.
        Alternates hold/release to keep the bar moving.
        """
        from ctypes import windll
        ...
        if not hasattr(self, '_fish_fallback_state'):
            self._fish_fallback_state = {
                'mouse_held': False,
                'toggle': False
            }
        
        state = self._fish_fallback_state
        
        # Alternate hold/release
        should_hold = state['toggle']
        ...
        status = "HOLD (→)" if state['mouse_held'] else "RELEASE (←)"
        print(f"[Fish Control] FALLBACK {status} (Lines not detected)")
    ...
    def _adjust_scan_fps(self, delta, menu):
        ...
        if menu == "shake":
            ...
            if current_val is None:
                current_val = 1000
            new_val = max(0, min(1000, int(current_val) + delta))
    ...
    def _render_fish_menu_content(self, width, height, accent_color):
        ...
        self.canvas.create_rectangle(
            button_x_bar, current_y + scroll_y,
            button_x_bar + button_width_config, current_y + button_height_config + scroll_y,
            fill="#444444", outline=accent_color, width=2
        )
        
        self.canvas.create_text(
            button_x_bar + button_width_config / 2, current_y + button_height_config / 2 + scroll_y,
            text="Configure Bar",
            fill="white", font=("Arial", 12, "bold")
        )
        ...
        # Icon Color Tolerance (specific to current rod)
        icon_tolerance = self.storage.get_rod_setting(current_rod, "icon_color_tolerance")
        ...
        # Bar Color Tolerance (specific to current rod)
        bar_tolerance = self.storage.get_rod_setting(current_rod, "bar_color_tolerance")
        if bar_tolerance is None:
            bar_tolerance = 5
        
        self.canvas.create_text(
            margin_left, current_y + scroll_y,
            text="Bar Color Tolerance:",
            fill="white", font=("Arial", 14, "bold"),
            anchor="w"
        )
        
        self._render_numeric_control(control_x, current_y + 10 + scroll_y, bar_tolerance, 
                                     button_width, button_height, accent_color)
        ...
        # Delay After Fish
        delay_after_fish = self.storage.get_fish_setting("delay_after_fish")
        if delay_after_fish is None:
            delay_after_fish = 2.0
        
        self.canvas.create_text(
            margin_left, current_y + scroll_y,
            text="Delay After Fish:",
            fill="white", font=("Arial", 14, "bold"),
            anchor="w"
        )
        
        self.canvas.create_text(
            control_x - 50, current_y + 12 + scroll_y,
            text=f"{delay_after_fish:.1f}s",
        ...
        minus_x = control_x
        plus_x = control_x + button_width + 10
        button_y = current_y + 10 + scroll_y
        
        self.canvas.create_rectangle(
            minus_x, button_y, minus_x + button_width, button_y + button_height,
            fill="#444444", outline=accent_color, width=2
        )
        self.canvas.create_text(
            minus_x + button_width / 2, button_y + button_height / 2,
            text="-", fill="white", font=("Arial", 14, "bold")
        )
        
        self.canvas.create_rectangle(
            plus_x, button_y, plus_x + button_width, button_y + button_height,
            fill="#444444", outline=accent_color, width=2
        )
        self.canvas.create_text(
            plus_x + button_width / 2, button_y + button_height / 2,
            text="+", fill="white", font=("Arial", 14, "bold")
        )
        ...
        if self.fish_icon_dropdown_open:
            all_rods = self.storage.get_all_rod_names()
            option_height = 30
            
            for i, rod_name in enumerate(all_rods):
