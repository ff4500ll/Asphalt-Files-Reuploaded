...
class GPOSantaApp:
    def __init__(self):
        ...
        self.santa_confidence = tk.DoubleVar(value=self.settings.get('santa_confidence', 0.1))
    ...
    def _playback_loop(self):
        ...
        try:
            ...
            while self.is_running:
                ...
                print(f"[PLAYBACK] Starting sequence ({len(self.recorded_actions)} actions)")
            ...
            while self.is_running:
                    ...
                    if self.icon_state == "searching_left" or self.icon_state == "searching_right":
                        ...
                        if distance_from_center > center_tolerance:
                            ...
                            if current_time - self.last_arrow_burst_time >= 0.2:
                                if icon_center_x < screen_center:
                                    # Icon is to the left, press left arrow to move camera left
                                    pydirectinput.press('left')
                                    print(f"Centering: Icon at {icon_center_x:.0f}, pressing LEFT")
                                else:
                                    # Icon is to the right, press right arrow to move camera right
                                    pydirectinput.press('right')
                                    print(f"Centering: Icon at {icon_center_x:.0f}, pressing RIGHT")
                ...
                if not icon_found:
                    ...
                    if self.icon_state == "searching_left":
                        # Send left arrow bursts every 200ms
                        if current_time - self.last_arrow_burst_time >= 0.2:
                            pydirectinput.press('left')
                            self.last_arrow_burst_time = current_time
                            print("Searching: Burst LEFT")
                        
                    elif self.icon_state == "searching_right":
                        # Send right arrow bursts every 200ms
                        if current_time - self.last_arrow_burst_time >= 0.2:
                            pydirectinput.press('right')
                            self.last_arrow_burst_time = current_time
                            print("Searching: Burst RIGHT")
