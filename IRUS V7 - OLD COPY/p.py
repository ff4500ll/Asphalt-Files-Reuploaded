...
class Application(tk.Tk):
    def __init__(self):
        ...
        # --- Casting Flow Control Variables ---
        self.casting_flow_enabled = tk.BooleanVar(value=False)
        self.casting_flow_branch = tk.StringVar(value="perfect_cast")  # "perfect_cast" or "hold_release"
        
        # Perfect Cast Branch
        self.pc_delay1_enabled = tk.BooleanVar(value=True)
        self.pc_delay1_var = tk.StringVar(value="0.5")
        self.pc_perfect_cast_enabled = tk.BooleanVar(value=True)
        self.pc_delay2_enabled = tk.BooleanVar(value=True)
        self.pc_delay2_var = tk.StringVar(value="0.5")
        
        # Hold Release Branch  
        self.hr_delay1_enabled = tk.BooleanVar(value=True)
        self.hr_delay1_var = tk.StringVar(value="0.5")
        self.hr_hold_enabled = tk.BooleanVar(value=True)
        self.hr_delay2_enabled = tk.BooleanVar(value=True)
        self.hr_delay2_var = tk.StringVar(value="0.5")
        self.hr_release_enabled = tk.BooleanVar(value=True)
        self.hr_delay3_enabled = tk.BooleanVar(value=True)
        self.hr_delay3_var = tk.StringVar(value="0.5")
        # --- Auto Shake Settings ---
    ...
    def load_config(self):
        ...
        default_config = {
            ...
            "AUTO_ZOOM_IN": "True",
            # Casting Flow Control defaults
            "CASTING_FLOW_ENABLED": "False",
            "CASTING_FLOW_BRANCH": "perfect_cast",
            # Perfect Cast branch defaults
            "PC_DELAY1_ENABLED": "True",
            "PC_DELAY1_VALUE": "1.0",
            "PC_PERFECT_CAST_ENABLED": "True",
            "PC_DELAY2_ENABLED": "True",
            "PC_DELAY2_VALUE": "1.0",
            # Hold Release branch defaults
            "HR_DELAY1_ENABLED": "True",
            "HR_DELAY1_VALUE": "1.0",
            "HR_HOLD_ENABLED": "True",
            "HR_DELAY2_ENABLED": "True",
            "HR_DELAY2_VALUE": "2.0",
            "HR_RELEASE_ENABLED": "True",
            "HR_DELAY3_ENABLED": "True",
            "HR_DELAY3_VALUE": "1.0",
        ...
        self.casting_flow_enabled.set(config.get("CASTING_FLOW_ENABLED", "False") == "True")
        self.casting_flow_branch.set(config.get("CASTING_FLOW_BRANCH", "perfect_cast"))
        # Perfect Cast branch
        self.pc_delay1_enabled.set(config.get("PC_DELAY1_ENABLED", "True") == "True")
        self.pc_delay1_var.set(config.get("PC_DELAY1_VALUE", "1.0"))
        self.pc_perfect_cast_enabled.set(config.get("PC_PERFECT_CAST_ENABLED", "True") == "True")
        self.pc_delay2_enabled.set(config.get("PC_DELAY2_ENABLED", "True") == "True")
        self.pc_delay2_var.set(config.get("PC_DELAY2_VALUE", "1.0"))
        # Hold Release branch
        self.hr_delay1_enabled.set(config.get("HR_DELAY1_ENABLED", "True") == "True")
        self.hr_delay1_var.set(config.get("HR_DELAY1_VALUE", "1.0"))
        self.hr_hold_enabled.set(config.get("HR_HOLD_ENABLED", "True") == "True")
        self.hr_delay2_enabled.set(config.get("HR_DELAY2_ENABLED", "True") == "True")
        self.hr_delay2_var.set(config.get("HR_DELAY2_VALUE", "2.0"))
        self.hr_release_enabled.set(config.get("HR_RELEASE_ENABLED", "True") == "True")
        self.hr_delay3_enabled.set(config.get("HR_DELAY3_ENABLED", "True") == "True")
        self.hr_delay3_var.set(config.get("HR_DELAY3_VALUE", "1.0"))
                # Navigation Mode
    ...
    def save_config(self):
        ...
        try:
            with open(CONFIG_FILE, 'w') as f:
                ...
                # Casting Flow Control
                f.write(f"CASTING_FLOW_ENABLED={self.casting_flow_enabled.get()}\n")
                f.write(f"CASTING_FLOW_BRANCH={self.casting_flow_branch.get()}\n")
                # Perfect Cast branch
                f.write(f"PC_DELAY1_ENABLED={self.pc_delay1_enabled.get()}\n")
                f.write(f"PC_DELAY1_VALUE={self.pc_delay1_var.get()}\n")
                f.write(f"PC_PERFECT_CAST_ENABLED={self.pc_perfect_cast_enabled.get()}\n")
                f.write(f"PC_DELAY2_ENABLED={self.pc_delay2_enabled.get()}\n")
                f.write(f"PC_DELAY2_VALUE={self.pc_delay2_var.get()}\n")
                # Hold Release branch
                f.write(f"HR_DELAY1_ENABLED={self.hr_delay1_enabled.get()}\n")
                f.write(f"HR_DELAY1_VALUE={self.hr_delay1_var.get()}\n")
                f.write(f"HR_HOLD_ENABLED={self.hr_hold_enabled.get()}\n")
                f.write(f"HR_DELAY2_ENABLED={self.hr_delay2_enabled.get()}\n")
                f.write(f"HR_DELAY2_VALUE={self.hr_delay2_var.get()}\n")
                f.write(f"HR_RELEASE_ENABLED={self.hr_release_enabled.get()}\n")
                f.write(f"HR_DELAY3_ENABLED={self.hr_delay3_enabled.get()}\n")
                f.write(f"HR_DELAY3_VALUE={self.hr_delay3_var.get()}\n")
                # Navigation Mode
    ...
    def setup_ui(self):
        ...
        self._update_casting_flow_visibility()
        ...
        # Create scrollable frame for casting
        casting_canvas = tk.Canvas(casting_tab)
        self.casting_canvas = casting_canvas  # Store reference for theming
        casting_scrollbar = ttk.Scrollbar(casting_tab, orient="vertical", command=casting_canvas.yview)
        casting_scrollable_frame = ttk.Frame(casting_canvas)

        casting_scrollable_frame.bind(
            "<Configure>",
            lambda e: casting_canvas.configure(scrollregion=casting_canvas.bbox("all"))
        )

        casting_canvas.create_window((0, 0), window=casting_scrollable_frame, anchor="nw")
        casting_canvas.configure(yscrollcommand=casting_scrollbar.set)

        casting_canvas.pack(side="left", fill="both", expand=True)
        casting_scrollbar.pack(side="right", fill="y")

        # Casting header
        casting_header = ttk.Label(casting_scrollable_frame, text="🎣 Casting", 
                                  font=('Arial', 14, 'bold'))
        casting_header.pack(pady=(0, 5))
        
        casting_subtitle = ttk.Label(casting_scrollable_frame, text="Configure fishing cast detection and timing",
                                   foreground="gray", font=('Arial', 10))
        casting_subtitle.pack(pady=(0, 15))

        # First Stage (Casting) - matching the reference image
        first_stage_frame = ttk.LabelFrame(casting_scrollable_frame, text="FirstStage (Casting)", padding="15")
        first_stage_frame.pack(fill="x", pady=(0, 10), padx=10)

        # Enable FirstStage checkbox
        first_stage_main_frame = ttk.Frame(first_stage_frame)
        first_stage_main_frame.pack(fill="x", pady=5)

        self.first_stage_check = ttk.Checkbutton(first_stage_main_frame, 
                                               text="Enable FirstStage (Cast detection and initial fishing)",
                                               variable=self.casting_flow_enabled, 
                                               command=self._on_first_stage_change)
        self.first_stage_check.pack(side="left")

        # Description text matching the reference image
        first_stage_desc = ttk.Label(first_stage_frame, 
        ...
        first_stage_desc.pack(pady=(10, 0), anchor="w")

        # Casting Flow Control
        casting_flow_frame = ttk.LabelFrame(casting_scrollable_frame, text="Casting Flow Control", padding="15")
        casting_flow_frame.pack(fill="x", pady=(10, 0), padx=10)

        # Enable Casting Flow Control
        casting_flow_main_frame = ttk.Frame(casting_flow_frame)
        casting_flow_main_frame.pack(fill="x", pady=5)
        
        self.casting_flow_check = ttk.Checkbutton(casting_flow_main_frame, text="🎯 Enable Casting Flow", 
                                                 variable=self.casting_flow_enabled, command=self._on_casting_flow_change)
        self.casting_flow_check.pack(side="left")
        ttk.Label(casting_flow_main_frame, text="- Advanced casting sequence control", 
                 foreground="gray", font=('Arial', 8)).pack(side="left", padx=(10, 0))

        # Branch Selection
        self.casting_branch_frame = ttk.Frame(casting_flow_frame)
        # Don't pack yet - will be managed by visibility function
        
        ttk.Label(self.casting_branch_frame, text="🔀 Flow Branch:", font=('Arial', 10)).pack(side="left", padx=(20, 10))
        self.casting_branch_combo = ttk.Combobox(self.casting_branch_frame, textvariable=self.casting_flow_branch,
                                               values=["perfect_cast", "hold_release"], width=12, state="readonly")
        self.casting_branch_combo.pack(side="left", padx=(0, 10))
        self.casting_branch_combo.bind('<<ComboboxSelected>>', self._on_casting_branch_change)
        ttk.Label(self.casting_branch_frame, text="(Select casting flow type)", 
                 foreground="gray", font=('Arial', 8)).pack(side="left")

        # Perfect Cast Branch Frame
        self.perfect_cast_frame = ttk.LabelFrame(casting_flow_frame, text="Perfect Cast Branch", padding="10")
        # Don't pack yet - will be managed by visibility function
        
        # PC Delay 1
        pc_delay1_frame = ttk.Frame(self.perfect_cast_frame)
        pc_delay1_frame.pack(fill="x", pady=2)
        
        self.pc_delay1_check = ttk.Checkbutton(pc_delay1_frame, text="📏 Delay 1", 
                                             variable=self.pc_delay1_enabled, command=self._on_pc_delay1_change)
        self.pc_delay1_check.pack(side="left", padx=(0, 10))
        
        self.pc_delay1_entry = ttk.Entry(pc_delay1_frame, textvariable=self.pc_delay1_var, width=5)
        self.pc_delay1_entry.pack(side="left", padx=(0, 5))
        self.pc_delay1_entry.bind('<KeyRelease>', lambda event: self.save_config_debounced())
        self.pc_delay1_entry.bind('<FocusOut>', lambda event: self.save_config())
        
        ttk.Label(pc_delay1_frame, text="seconds", foreground="gray", font=('Arial', 8)).pack(side="left", padx=(0, 10))
        ttk.Label(pc_delay1_frame, text="→", font=('Arial', 12)).pack(side="left", padx=(10, 10))
        
        # PC Perfect Cast
        self.pc_perfect_cast_check = ttk.Checkbutton(pc_delay1_frame, text="⚡ Perfect Cast", 
                                                   variable=self.pc_perfect_cast_enabled, command=self._on_pc_perfect_cast_change)
        self.pc_perfect_cast_check.pack(side="left", padx=(0, 10))
        
        ttk.Label(pc_delay1_frame, text="→", font=('Arial', 12)).pack(side="left", padx=(10, 10))
        
        # PC Delay 2
        self.pc_delay2_check = ttk.Checkbutton(pc_delay1_frame, text="📏 Delay 2", 
                                             variable=self.pc_delay2_enabled, command=self._on_pc_delay2_change)
        self.pc_delay2_check.pack(side="left", padx=(0, 10))
        
        self.pc_delay2_entry = ttk.Entry(pc_delay1_frame, textvariable=self.pc_delay2_var, width=5)
        self.pc_delay2_entry.pack(side="left", padx=(0, 5))
        self.pc_delay2_entry.bind('<KeyRelease>', lambda event: self.save_config_debounced())
        self.pc_delay2_entry.bind('<FocusOut>', lambda event: self.save_config())
        
        ttk.Label(pc_delay1_frame, text="seconds", foreground="gray", font=('Arial', 8)).pack(side="left", padx=(0, 10))
        ttk.Label(pc_delay1_frame, text="→", font=('Arial', 12)).pack(side="left", padx=(10, 10))
        ttk.Label(pc_delay1_frame, text="🚪 Exit", foreground="green", font=('Arial', 10)).pack(side="left")

        # Hold Release Branch Frame
        self.hold_release_frame = ttk.LabelFrame(casting_flow_frame, text="Hold Release Branch", padding="10")
        # Don't pack yet - will be managed by visibility function
        
        # HR Row 1: Delay 1 → Hold
        hr_row1_frame = ttk.Frame(self.hold_release_frame)
        hr_row1_frame.pack(fill="x", pady=2)
        
        self.hr_delay1_check = ttk.Checkbutton(hr_row1_frame, text="📏 Delay 1", 
                                             variable=self.hr_delay1_enabled, command=self._on_hr_delay1_change)
        self.hr_delay1_check.pack(side="left", padx=(0, 10))
        
        self.hr_delay1_entry = ttk.Entry(hr_row1_frame, textvariable=self.hr_delay1_var, width=5)
        self.hr_delay1_entry.pack(side="left", padx=(0, 5))
        self.hr_delay1_entry.bind('<KeyRelease>', lambda event: self.save_config_debounced())
        self.hr_delay1_entry.bind('<FocusOut>', lambda event: self.save_config())
        
        ttk.Label(hr_row1_frame, text="seconds", foreground="gray", font=('Arial', 8)).pack(side="left", padx=(0, 10))
        ttk.Label(hr_row1_frame, text="→", font=('Arial', 12)).pack(side="left", padx=(10, 10))
        
        self.hr_hold_check = ttk.Checkbutton(hr_row1_frame, text="🖱️ Hold", 
                                           variable=self.hr_hold_enabled, command=self._on_hr_hold_change)
        self.hr_hold_check.pack(side="left", padx=(0, 10))
        ttk.Label(hr_row1_frame, text="(Hold left click)", foreground="gray", font=('Arial', 8)).pack(side="left")
        
        # HR Row 2: Delay 2 → Release → Delay 3 → Exit
        hr_row2_frame = ttk.Frame(self.hold_release_frame)
        hr_row2_frame.pack(fill="x", pady=2)
        
        ttk.Label(hr_row2_frame, text="                    ↓", font=('Arial', 12), foreground="blue").pack(side="left")
        
        hr_row3_frame = ttk.Frame(self.hold_release_frame)
        hr_row3_frame.pack(fill="x", pady=2)
        
        self.hr_delay2_check = ttk.Checkbutton(hr_row3_frame, text="📏 Delay 2", 
                                             variable=self.hr_delay2_enabled, command=self._on_hr_delay2_change)
        self.hr_delay2_check.pack(side="left", padx=(0, 10))
        
        self.hr_delay2_entry = ttk.Entry(hr_row3_frame, textvariable=self.hr_delay2_var, width=5)
        self.hr_delay2_entry.pack(side="left", padx=(0, 5))
        self.hr_delay2_entry.bind('<KeyRelease>', lambda event: self.save_config_debounced())
        self.hr_delay2_entry.bind('<FocusOut>', lambda event: self.save_config())
        
        ttk.Label(hr_row3_frame, text="seconds", foreground="gray", font=('Arial', 8)).pack(side="left", padx=(0, 10))
        ttk.Label(hr_row3_frame, text="→", font=('Arial', 12)).pack(side="left", padx=(10, 10))
        
        self.hr_release_check = ttk.Checkbutton(hr_row3_frame, text="🖱️ Release", 
                                              variable=self.hr_release_enabled, command=self._on_hr_release_change)
        self.hr_release_check.pack(side="left", padx=(0, 10))
        ttk.Label(hr_row3_frame, text="(Release left click)", foreground="gray", font=('Arial', 8)).pack(side="left", padx=(0, 10))
        
        ttk.Label(hr_row3_frame, text="→", font=('Arial', 12)).pack(side="left", padx=(10, 10))
        
        self.hr_delay3_check = ttk.Checkbutton(hr_row3_frame, text="📏 Delay 3", 
                                             variable=self.hr_delay3_enabled, command=self._on_hr_delay3_change)
        self.hr_delay3_check.pack(side="left", padx=(0, 10))
        
        self.hr_delay3_entry = ttk.Entry(hr_row3_frame, textvariable=self.hr_delay3_var, width=5)
        self.hr_delay3_entry.pack(side="left", padx=(0, 5))
        self.hr_delay3_entry.bind('<KeyRelease>', lambda event: self.save_config_debounced())
        self.hr_delay3_entry.bind('<FocusOut>', lambda event: self.save_config())
        
        ttk.Label(hr_row3_frame, text="seconds", foreground="gray", font=('Arial', 8)).pack(side="left", padx=(0, 10))
        ttk.Label(hr_row3_frame, text="→", font=('Arial', 12)).pack(side="left", padx=(10, 10))
        ttk.Label(hr_row3_frame, text="🚪 Exit", foreground="green", font=('Arial', 10)).pack(side="left")
    ...
    def _on_casting_flow_change(self):
        """Called when casting flow checkbox is changed - save config and update visibility."""
        self.save_config()
        self._update_casting_flow_visibility()

    def _on_casting_branch_change(self, event=None):
        """Called when casting branch selection changes - save config and update visibility."""
        self.save_config()
        self._update_casting_flow_visibility()

    def _update_casting_flow_visibility(self):
        """Shows or hides casting flow control sections based on enable state and branch selection."""
        if self.casting_flow_enabled.get():
            # Show the branch selection frame
            self.casting_branch_frame.pack(fill="x", pady=5)
            
            # Show/hide branch-specific frames based on selection
            branch = self.casting_flow_branch.get()
            if branch == "perfect_cast":
                self.perfect_cast_frame.pack(fill="x", pady=(10, 0))
                self.hold_release_frame.pack_forget()
            elif branch == "hold_release":
                self.hold_release_frame.pack(fill="x", pady=(10, 0))
                self.perfect_cast_frame.pack_forget()
            else:
                # No branch selected yet
                self.perfect_cast_frame.pack_forget()
                self.hold_release_frame.pack_forget()
        else:
            # Hide all casting flow frames
            self.casting_branch_frame.pack_forget()
            self.perfect_cast_frame.pack_forget()
            self.hold_release_frame.pack_forget()
