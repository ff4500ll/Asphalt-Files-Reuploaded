...
6. Enable integration and enjoy automated screenshots!"""
        ...
        if holding_swapped and self.last_indicator_x is not None:
            ...
            if self.has_calculated_length_once:
                ...
                if not self.is_holding_click:
                    ...
                    # Enhanced safety: max size is entire capture area width
                    ...
                    # Allow more forgiving size changes when current box is very small (likely corrupted or initial detection)
                    # Also allow large bars during arrow-only tracking (when color tracking never succeeded)
                    if self.estimated_box_length < 50:
                        max_change_ratio = 10.0  # Very small bar - allow large changes
                    elif self.estimated_box_length < 150 and not hasattr(self, '_color_tracking_succeeded_once'):
                        max_change_ratio = 10.0  # Medium bar but color never succeeded - likely arrow-only fish
                    else:
                        max_change_ratio = 3.0  # Normal operation
                    
                    # Only update box length from arrow if color tracking has never worked (arrow-only fish)
                    # This prevents arrow measurements from overwriting more accurate color-based measurements
                    if not getattr(self, '_color_tracking_succeeded_once', False):
                        if 10 < new_length < max_reasonable_size and size_change_ratio < max_change_ratio:
                            ...
                            logging.warning(f"Arrow Fallback: Rejected size {new_length:.1f}px (change ratio: {size_change_ratio:.2f}, max allowed: {max_change_ratio:.1f}x, size limit: {max_reasonable_size:.0f}px)")
    ...
    def _process_and_control(self, sct_instance, time_delta, current_time):
        ...
        if self.state == "FISHING" and target_line_x is not None:
            ...
            # If somehow no position was ever found, skip control
            # This can happen during initial frames after state transition or when steadying
            # Only warn if we've been in FISHING state for a while (not during startup)
            ...
            if box_position is not None:
                ...
                # --- C. Initial Steadying System: Distance-Based Movement Detection ---
                if self.initial_steadying_active and not self.initial_steadying_done_for_session:
                    ...
                    if steadying_duration > 0.2:
                        # Calculate distance from target line to box left edge
                        if target_line_x is not None and self.last_left_x is not None:
                            current_distance = abs(target_line_x - self.last_left_x)
                            
                            # Capture baseline distance on first measurement
                            if not hasattr(self, 'steady_baseline_distance'):
                                self.steady_baseline_distance = current_distance
                                logging.info(f"STEADYING: Baseline distance (target to left edge): {current_distance:.1f}px")
                            else:
                                # Check if distance changed by more than 3 pixels
                                distance_change = abs(current_distance - self.steady_baseline_distance)
                                
                                if distance_change >= 3.0:
                                    movement_detected = True
                                    logging.info(f"MOVEMENT DETECTED: Distance changed {self.steady_baseline_distance:.1f}px -> {current_distance:.1f}px (delta: {distance_change:.1f}px)")
                    ...
                    if movement_detected:
                        ...
                        if hasattr(self, 'steady_baseline_distance'):
                            delattr(self, 'steady_baseline_distance')
                        logging.info("STEADYING -> PD CONTROL: Distance change detected, switching immediately!")
                    else:
            ...
            if target_line_x is not None and self.box_center_x is not None and self.has_calculated_length_once:
                ...
                # No valid box position available
                # This is normal during initial steadying or right after state transition
                # Only warn if we've been in FISHING state for more than 1 second
                if not hasattr(self, '_fishing_start_time'):
                    self._fishing_start_time = current_time
                
                time_since_fishing_start = current_time - self._fishing_start_time
                
                # Only log warning if this persists after the startup period
                if time_since_fishing_start > 1.0 and not hasattr(self, '_no_position_warning_logged'):
                    logging.warning("BULLETPROOF PD: No valid box position available after 1s - tracking may have failed")
                    self._no_position_warning_logged = True
                
                status_color_mode = "No Position Data"
                status_suffix = " | Waiting for Tracking"
