...
class NeuralNetworkGUI(ctk.CTk):
    ...
    def _execute_fish(self):
        ...
        # Create visual overlay window for arrows
        overlay_window = None
        overlay_canvas = None
        arrow_ids = {'icon': None, 'bar': None}
        
        def create_overlay():
            nonlocal overlay_window, overlay_canvas
            overlay_window = tk.Toplevel(self)
            overlay_window.attributes('-transparentcolor', 'black')
            overlay_window.attributes('-topmost', True)
            overlay_window.overrideredirect(True)
            
            # Position below fish_box
            arrow_height = 30
            overlay_window.geometry(f"{fish_box[2] - fish_box[0]}x{arrow_height}+{fish_box[0]}+{fish_box[3] + 5}")
            
            overlay_canvas = tk.Canvas(overlay_window, bg='black', highlightthickness=0)
            overlay_canvas.pack(fill='both', expand=True)
        
        def get_arrow_coords_up(x, y):
            """Get coordinates for upward-pointing triangle arrow"""
            size = 15
            return [x, y - size, x - size//2, y, x + size//2, y]
        
        def update_arrows(icon_x, bar_x):
            """Update or create arrows pointing upward at icon and bar positions"""
            if overlay_canvas is None:
                return
            
            # Convert absolute screen coords to overlay canvas coords
            icon_canvas_x = icon_x - fish_box[0]
            bar_canvas_x = bar_x - fish_box[0]
            arrow_y = 25  # Y position in overlay (near bottom)
            
            # Icon arrow (green)
            icon_coords = get_arrow_coords_up(icon_canvas_x, arrow_y)
            if arrow_ids['icon']:
                try:
                    overlay_canvas.coords(arrow_ids['icon'], *icon_coords)
                except:
                    arrow_ids['icon'] = overlay_canvas.create_polygon(icon_coords, fill='#00ff00', outline='#00ff00')
            else:
                arrow_ids['icon'] = overlay_canvas.create_polygon(icon_coords, fill='#00ff00', outline='#00ff00')
            
            # Bar arrow (red)
            bar_coords = get_arrow_coords_up(bar_canvas_x, arrow_y)
            if arrow_ids['bar']:
                try:
                    overlay_canvas.coords(arrow_ids['bar'], *bar_coords)
                except:
                    arrow_ids['bar'] = overlay_canvas.create_polygon(bar_coords, fill='#ff0000', outline='#ff0000')
            else:
                arrow_ids['bar'] = overlay_canvas.create_polygon(bar_coords, fill='#ff0000', outline='#ff0000')
        
        # Create overlay on main thread
        self.after(0, create_overlay)
        time.sleep(0.1)  # Wait for overlay creation
