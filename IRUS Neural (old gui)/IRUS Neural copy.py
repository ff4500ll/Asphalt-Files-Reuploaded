...
class NeuralNetworkGUI(ctk.CTk):
    ...
    def _execute_fish(self):
        ...
        try:
            ...
            bar_colors = self.storage.get_rod_setting(current_rod, "bar_colors")
            ...
            if not bar_colors:
                print(f"[Fish] ERROR: No bar colors configured for rod '{current_rod}'")
                return
            ...
            print(f"[Fish] Bar colors: {len(bar_colors)} colors")
            print(f"[Fish] Bar color tolerance: {bar_tolerance}")
            ...
            bar_color_ranges = merge_colors_into_ranges(bar_colors, bar_tolerance)
            print(f"[Fish] Bar color ranges: {len(bar_color_ranges)} ranges (merged from {len(bar_colors)} colors)")

            # Main detection loop
            consecutive_failures = 0
            last_icon_match_idx = 0  # Cache for neighbor-priority search
            last_bar_match_idx = 0   # Cache for neighbor-priority search
            while self.bot_running and not self._should_stop():
                frame = camera.get_latest_frame()
                ...
                if frame is not None:
                    ...
                    # THIRD: Find bar color positions (leftmost and rightmost across ALL colors)
                    bar_x_left = None
                    bar_x_right = None
                    
                    height, width = fish_area_crop.shape[:2]
                    
                    # Stack all range mins/maxs for vectorized comparison
                    range_mins = np.array([r[0] for r in bar_color_ranges], dtype=np.uint8)  # (num_ranges, 3)
                    range_maxs = np.array([r[1] for r in bar_color_ranges], dtype=np.uint8)  # (num_ranges, 3)
                    
                    # Find leftmost: scan left to right across all rows
                    leftmost_col = None
                    for row in range(height):
                        row_pixels = fish_area_crop[row, :, :]  # Shape: (width, 3)
                        for col in range(width):
                            pixel = row_pixels[col, :]
                            # Check if pixel matches any color range
                            matches = np.all((pixel >= range_mins) & (pixel <= range_maxs), axis=1)
                            if np.any(matches):
                                leftmost_col = col
                                break
                    ...
                    if leftmost_col is not None and rightmost_col is not None:
                        bar_x_left = fish_box[0] + leftmost_col
                        bar_x_right = fish_box[0] + rightmost_col
                        bar_x_center = (bar_x_left + bar_x_right) // 2
                    
                    if bar_x_left is None:
                        consecutive_failures += 1
                    else:
    ...
    def _fish_control_system(self, icon_x, bar_x):
        """
        PD (Proportional-Derivative) controller for frictionless binary control.
        KP: Reacts to distance from target
        KD: Dampens based on rate of approach (derivative of error)
        """
        import time
        ...
        if not hasattr(self, '_fish_control_state'):
            self._fish_control_state = {
                ...
                'alternating': True,
                'initial_bar_x': bar_x,
                'initial_icon_x': icon_x
            }
            # Start with a hold to begin alternating
            self._mouse_down()
            self._fish_control_state['mouse_held'] = True
            return
        ...
        # Check if we're still in alternating mode (waiting for movement to start)
        if state['alternating']:
            bar_moved = abs(bar_x - state['initial_bar_x']) >= 5
            icon_moved = abs(icon_x - state['initial_icon_x']) >= 5
            
            if bar_moved or icon_moved:
                state['alternating'] = False
                print("[Fish Control] Movement detected - engaging PD control")
            else:
                # Alternate to prevent velocity buildup
                if state['mouse_held']:
                    self._mouse_up()
                    state['mouse_held'] = False
                    current_action = "ALTERNATE_RELEASE"
                else:
                    self._mouse_down()
                    state['mouse_held'] = True
                    current_action = "ALTERNATE_HOLD"
                
                print(f"[Fish Control] Icon: {icon_x:.1f} | Bar: {bar_x:.1f} | Action: {current_action} (WAITING)")
                
                state['previous_time'] = current_time
                return
        
        # === PD CONTROL ===
        # Error: positive = need to move right, negative = need to move left
        error = icon_x - bar_x
        
        # Derivative: rate of change of error (how fast we're approaching/leaving target)
        # Negative derivative = error decreasing = approaching target
        derivative = (error - state['previous_error']) / dt
        
        # PD gains (tunable)
        KP = 0.2   # Proportional gain - how strongly to react to position error
        KD = 0.5  # Derivative gain - how strongly to dampen approach velocity
        
        # Calculate control signal
        # Positive = want to move right (hold), Negative = want to move left (release)
        control = KP * error + KD * derivative
        
        # Convert continuous control to binary action
        action = "HOLD" if state['mouse_held'] else "RELEASE"
        ...
        print(f"[Fish Control] Icon: {icon_x:.1f} | Bar: {bar_x:.1f} | "
              f"Err: {error:.1f} | dErr/dt: {derivative:.1f} | "
              f"Control: {control:.2f} | {action}")
