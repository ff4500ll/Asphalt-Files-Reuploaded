...
class HotkeyApp:
    ...
    def setup_ui(self):
        ...
        mode_frame.grid(row=1, column=0, columnspan=2, pady=(0, 15), sticky=tk.W)
        ...
        for idx, (label_text, key) in enumerate(settings):
            ...
            ttk.Label(main_frame, text=label_text, font=('Arial', 10)).grid(
                row=idx+2, column=0, sticky=tk.W, pady=5
            )
            
            # Hotkey display and rebind button
            hotkey_frame = ttk.Frame(main_frame)
            hotkey_frame.grid(row=idx+2, column=1, sticky=tk.E, pady=5)
    ...
    def register_hotkeys(self):
        ...
        # Register mode switching keys (x, v, n) - only work when running
        ...
        keyboard.on_press_key('n', lambda _: self.switch_mode('n') if self.running else None)
    ...
    def detection_loop(self):
        ...
        while not self.stop_detection:
            ...
            try:
                ...
                # Check for red and blue colors in corner_box
                red_found = self.find_color_in_region(corner_rgb, self.red_lower, self.red_upper)
                blue_found = self.find_color_in_region(corner_rgb, self.blue_lower, self.blue_upper)
                
                if red_found and blue_found:
                    print("Found both red and blue in corner_box")
