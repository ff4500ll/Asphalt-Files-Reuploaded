...
MODE_DELAYS = {
    ...
    'x': 0.19,   # 190ms between Q/E presses
    'c': 0.0,    # Fast mode - instant alternation
    'v': 0.1     # 100ms (xbutton5 toggle)
}
...
class HotkeyGUI:
    def __init__(self, root):
        ...
        # Template time-based confirmation (must be detected continuously for 300ms)
        self.current_template = None  # Currently detected template
        self.template_first_seen = 0  # Timestamp when current template was first detected
        self.template_confirmation_time = 0.3  # 300ms continuous detection required
        ...
        # Config file path (in exe directory - use executable directory, not temp folder)
        if getattr(sys, 'frozen', False):
            # Running as compiled exe - save next to exe
            exe_dir = os.path.dirname(sys.executable)
            self.config_file = os.path.join(exe_dir, "p_settings.json")
        else:
            # Running as script - save next to script
            self.config_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "p_settings.json")
    ...
    def create_widgets(self):
        ...
        self.start_stop_label = ttk.Label(frame1, text=self.get_key_name(self.hotkeys['start_stop']), width=10, relief='sunken', anchor='center')
        ...
        self.change_area_label = ttk.Label(frame2, text=self.get_key_name(self.hotkeys['change_area']), width=10, relief='sunken', anchor='center')
        ...
        self.exit_label = ttk.Label(frame3, text=self.get_key_name(self.hotkeys['exit']), width=10, relief='sunken', anchor='center')
        ...
        # Keybinds info section
        keybinds_frame = ttk.LabelFrame(self.root, text="Keybinds", padding="10")
        keybinds_frame.pack(fill='x', padx=10, pady=5)
        
        keybinds_text = (
            "Mode Keys: Z (Disabled) | X (229ms) | C (Fast) | V (120ms)\n"
            "Timing: [ (increase X) | ] (decrease X)\n"
            "Mouse: XButton4 (Z mode) | XButton5 (V mode toggle)\n"
            "Other: V key (right click down when script running)"
        )
    ...
    def on_key_press(self, key):
        ...
        # V key - send right click down (one-time action, only when script is enabled)
        elif hasattr(key, 'char') and key.char == 'v':
            if self.is_running:
                win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN, 0, 0, 0, 0)
                print("Right click down sent (V key pressed)")
            else:
                print("V key ignored (script not running)")
    ...
    def toggle_change_area(self):
        ...
        if self.area_selector is None:
            ...
            key_name = self.get_key_name(self.hotkeys['change_area'])
            self.status_label.config(text=f"Area selection mode - Press {key_name} to save")
        else:
    ...
    def detection_loop(self):
        ...
        while not self.stop_detection:
            ...
            try:
                ...
                if found_key:
                    # Time-based confirmation: must see same PNG continuously for 300ms
                    current_time = time.time()
                    
                    if self.current_template != found_key:
                        # New template detected - start tracking
                        self.current_template = found_key
                        self.template_first_seen = current_time
                    else:
                        # Same template still detected - check if 300ms has passed
                        elapsed = current_time - self.template_first_seen
                        if elapsed >= self.template_confirmation_time and not self.flag:
                            # Continuously detected for 300ms - press the key
                    ...
                    # Auto-switch to disabled mode when PNG is found
                    if self.mode != 'z':
                        self.mode = 'z'
                        self.previous_mode = 'z'
                        mode_name = self.get_mode_name('z')
                        self.mode_label.config(text=f"Mode: {mode_name}")
                        print(f"PNG detected - auto-switching to {mode_name}")
                else:
                    # Reset template tracking when nothing is found
                    self.current_template = None
                    self.template_first_seen = 0
                    self.flag = False
                ...
                # Corner detection - check every loop
                ...
                if self.corner_state == 'waiting_color':
                    ...
                    if corner_found_now:
                        ...
                        # Special case for 3rd press: switch to v mode when corner appears
                        if self.corner_press_count == 2:
                            print("3rd press detected - switching to v mode (corner ON)")
                            self.previous_mode = self.mode
                            self.mode = 'v'
                            mode_name = self.get_mode_name('v')
