...
DEFAULT_GUI_GEOM = "650x650" # Increased height for new options
DEFAULT_SHAKE = "200x200+500+300"
DEFAULT_FISH = "300x100+500+550"
DEFAULT_LIVE_FEED_POS = "+750+550"
...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.shake_mode_var = tk.StringVar()  # "Click" or "Navigation"
    ...
    def load_config(self):
        ...
        default_config = {
            "GUI_GEOM": DEFAULT_GUI_GEOM,
            "SHAKE_GEOM": DEFAULT_SHAKE,
            "FISH_GEOM": DEFAULT_FISH,
            "LIVE_FEED_POS": DEFAULT_LIVE_FEED_POS,
            "START_STOP_KEY": "F4",
            "RESIZE_KEY": "F5",
            "FORCE_EXIT_KEY": "F6",
            "FPS": "240",
    ...
    def _handle_live_feed_toggle(self):
        """Handles closing the live feedback window when unchecked during IDLE/ACTIVE."""
        if not self.show_live_feed.get():
    ...
    def _idle_auto_shake(self, sct_instance, current_time):
        ...
        try:
            ...
            if not should_click:
                return
            # Simulate PHYSICAL mouse movement using relative mouse_event calls
            try:
                import ctypes
                from ctypes import windll
                import time
                ...
                # Get current cursor position to calculate movement needed
                current_x, current_y = pyautogui.position()
                delta_x = click_x - current_x
                delta_y = click_y - current_y
                
                logging.info(f"AutoShake: Moving from ({current_x},{current_y}) to ({click_x},{click_y}) delta=({delta_x},{delta_y})")
                
                # If we need to move, simulate physical mouse movement with user speed controls
                if abs(delta_x) > 2 or abs(delta_y) > 2:  # Only move if significant distance
                    
                    # Get user speed settings
                    try:
                        movement_speed = max(1, int(self.shake_movement_speed_var.get()))  # pixels per step
                        movement_delay = max(0, int(self.shake_movement_delay_var.get()))  # ms between steps
                    except Exception:
                        movement_speed = 10  # Default: 10 pixels per step
                        movement_delay = 1   # Default: 1ms between steps
                    
                    # Method 1: Use relative mouse_event movements (simulates physical mouse)
                    MOUSEEVENTF_MOVE = 0x0001
                    
                    # Calculate steps based on user speed setting
                    steps = max(abs(delta_x), abs(delta_y)) // movement_speed + 1
                    steps = max(1, min(steps, 100))  # Cap between 1-100 steps
                    
                    step_x = delta_x / steps
                    step_y = delta_y / steps
                    
                    logging.info(f"AutoShake: Movement config - speed={movement_speed}px/step, delay={movement_delay}ms, steps={steps}")
                    
                    for i in range(steps):
                        # Send relative movement (like physical mouse movement)
                        windll.user32.mouse_event(MOUSEEVENTF_MOVE, int(step_x), int(step_y), 0, 0)
                        if movement_delay > 0:
                            time.sleep(movement_delay / 1000.0)  # Convert ms to seconds
                    
                    # Final precise positioning
                    final_x, final_y = pyautogui.position()
                    final_delta_x = click_x - final_x
                    final_delta_y = click_y - final_y
                    
                    if abs(final_delta_x) > 0 or abs(final_delta_y) > 0:
                        windll.user32.mouse_event(MOUSEEVENTF_MOVE, final_delta_x, final_delta_y, 0, 0)
                        time.sleep(0.002)  # 2ms settling time
                
                # Now click at current position (hardware cursor should be synced)
                ...
                final_x, final_y = pyautogui.position()
                logging.info(f"AutoShake: Physical movement complete. Final position ({final_x},{final_y})")
                
                self._shake_memory_xy = (click_x, click_y)
                ...
                logging.info(f"AutoShake: Physical mouse movement + click at ({click_x},{click_y}) tol={tol} delay={delay_ms}ms")
