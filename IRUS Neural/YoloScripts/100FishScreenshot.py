...
class FishScreenshotCapture:
    def __init__(self):
        ...
        self.fish_area = None
        self.screenshot_dir = "100FishScreenshots"
        ...
        # Load fish area coordinates
        self._load_fish_area()
        ...
        print("\n=== 100 Fish Screenshot Capture ===")
        print("F3 = Start capturing")
        print("F4 = Pause/Resume")
        print("F5 = Manual capture (take 1 screenshot)")
        print("F6 = Toggle manual/auto mode")
        print("F1 = Exit")
        print(f"\nFish Area: x={self.fish_area['x']}, y={self.fish_area['y']}, "
              f"width={self.fish_area['width']}, height={self.fish_area['height']}")
    ...
    def _load_fish_area(self):
        """Load fish area coordinates from NeuralSettings.json"""
        ...
        try:
            with open(settings_path, 'r') as f:
                ...
                self.fish_area = settings.get("fish_area")
                
                if not self.fish_area:
                    print("ERROR: fish_area not found in NeuralSettings.json")
                    sys.exit(1)
                    
                print(f"Loaded fish area from: {settings_path}")
    ...
    def capture_screenshot(self, number):
        """Capture a screenshot of the fish area"""
        try:
            # Calculate the bounding box for the fish area
            x1 = self.fish_area['x']
            y1 = self.fish_area['y']
            x2 = x1 + self.fish_area['width']
            y2 = y1 + self.fish_area['height']
