...
class HotkeyConfigApp:
    def __init__(self, root):
        ...
        self.first_launch_config = {
            ...
            "shake_timeout": 3.0,
            "fish_timeout": 3.0,
            "shake_confidence_threshold": 0.65,  # Lowered from 0.8 for fewer missed detections
            "fishing_confidence_threshold": 0.4,  # Lowered to match b.py for better Target Line detection
            "auto_minimize": True,
            "cast_mode": "Normal",
            "cast_delay1": 0.0,
            "cast_delay2": 0.3,
            ...
            "fish_bounce_debounce": 0.3,
            "fish_show_live_feed": False
        }
    ...
    def update_fish_flow_details(self):
        ...
        if not self.active_settings.get("flow_fish_enabled", True):
            ...
            # Bounce Debounce Time
            bounce_debounce_frame = tk.Frame(settings_frame, bg="#f5f5f5")
            bounce_debounce_frame.grid(row=18, column=0, padx=10, pady=5, sticky="w")

            bounce_debounce_label = tk.Label(
                bounce_debounce_frame,
                text="Bounce Debounce Time:",
                font=("Segoe UI", 10),
                bg="#f5f5f5"
            )
            bounce_debounce_label.pack(side="left")

            self.fish_bounce_debounce_var = tk.StringVar(value=str(self.active_settings.get("fish_bounce_debounce", 0.3)))
            self.fish_bounce_debounce_var.trace_add("write", lambda *args: self.save_setting("fish_bounce_debounce", self.fish_bounce_debounce_var))
            bounce_debounce_entry = tk.Entry(
                bounce_debounce_frame,
                textvariable=self.fish_bounce_debounce_var,
                font=("Segoe UI", 10),
                width=8
            )
            bounce_debounce_entry.pack(side="left", padx=5)

            bounce_debounce_unit = tk.Label(
                bounce_debounce_frame,
                text="seconds",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
            ...
            bounce_debounce_unit.pack(side="left")

            # Description for Bounce Debounce
            bounce_debounce_desc = tk.Label(
                settings_frame,
                text="Ignore repeated bounces within this time (prevents duplicate bounce detection)",
                font=("Segoe UI", 9, "italic"),
                bg="#f5f5f5",
                fg="#666666"
            )
            bounce_debounce_desc.grid(row=19, column=0, padx=10, pady=(0, 5), sticky="w")
    ...
    def run_loop(self):
        ...
        while self.loop_running:
            ...
            if self.active_settings.get("flow_fish_enabled", True):
                ...
                # Cache debug overlay setting (checked multiple times per loop iteration)
                ...
                # ========== PD (PROPORTIONAL-DERIVATIVE) CONTROL PARAMETERS (EDIT HERE) ==========
                DEAD_ZONE_PX = 0.0               # How close = "good enough" (pixels)

                # Variable cycle timing (adapts to distance)
                MIN_CYCLE_MS = 50                # Minimum cycle time when close (fast spam)
                MAX_CYCLE_MS = 400               # Maximum cycle time when far (longer holds)
                CYCLE_SCALE_DISTANCE = 100.0     # Distance at which cycle time reaches MAX_CYCLE_MS

                # PD Control Gains
                KP = 0.01                      # Proportional gain (how much error affects output)
                KD = 0.05                        # Derivative gain (how much error rate affects output)

                # Control limits
                MAX_RATIO = 0.90                 # Max hold/release ratio (97% hold or 3% hold)
                MIN_RATIO = 0.1                 # Min hold/release ratio

                # Overshoot prevention
                PREDICTION_TIME = 0.2            # How far ahead to predict (seconds)
                BRAKE_THRESHOLD = 0.6            # Start braking when predicted error < this * current error

                # Timeout
                FISH_TIMEOUT_S = 3.0             # Timeout for losing tracking (seconds)
                # ===========================================================

                print(f"[Fish] PD Control - DeadZone={DEAD_ZONE_PX}px, Kp={KP}, Kd={KD}, PredTime={PREDICTION_TIME}s")

                # State variables for spam-click control
                last_time = time.time()
                is_holding = False
                lost_tracking_start = None  # Track when we lost both detections
                last_click_time = 0.0  # Track timing for spam clicks
                last_bar_x = None  # Track bar position for velocity calculation
                last_error = 0.0  # Track previous error for derivative calculation
                ...
                while not fish_complete and self.loop_running:
                    ...
                    fishing_results = fishing_model(
                        ...
                        iou=0.3,      # Lower IoU for overlapping Target Line and Fish Bar
                        max_det=2     # Only 2 objects: Target Line + Fish Bar (faster NMS)
                    )
                    ...
                    # Update debug overlay with current detections (if enabled)
                    # Check setting in real-time in case user toggles during loop
                    if self.active_settings.get("fish_show_live_feed", False):
                    ...
                    if target_line_x is None or bar_center_x is None:
                        # Lost tracking - start or continue timeout
                        if lost_tracking_start is None:
                            lost_tracking_start = time.time()
                            print("[Fish] Lost tracking - starting timeout timer")

                        # Check if timeout exceeded
                        if FISH_TIMEOUT_S > 0 and (time.time() - lost_tracking_start > FISH_TIMEOUT_S):
                            print(f"[Fish] Lost tracking for {FISH_TIMEOUT_S}s - exiting")
                            fish_complete = True
                            break
                        # Continue loop without PD control (can't control without detections)
                        continue
                    else:
                        # Both bar and line detected - reset lost tracking timer
                        if lost_tracking_start is not None:
                            print("[Fish] Tracking restored")
                            # Reset derivative tracking to prevent jump
                            last_error = target_line_x - bar_center_x  # Reset to current error
                            last_bar_x = bar_center_x  # Reset bar position tracking
                    ...
                    if distance < DEAD_ZONE_PX:
                        ...
                        # PD Controller
                        # P term: proportional to error (positive error = target right)
                        p_term = KP * error

                        # D term: proportional to error rate (derivative)
                        # Negative error_rate means error is decreasing (approaching target)
                        # Positive error_rate means error is increasing (moving away or target jumped)
                        d_term = KD * error_rate

                        # Combined control output
                        control_output = p_term + d_term

                        # Predict future error to prevent overshoot
                        predicted_error = error + error_rate * PREDICTION_TIME

                        # Check if we're about to overshoot (error and predicted_error have opposite signs)
                        will_overshoot = (error * predicted_error < 0)
                        approaching_fast = abs(predicted_error) < abs(error) * BRAKE_THRESHOLD

                        # Determine mode for debugging
                        if will_overshoot:
                            mode = "OVERSHOOT"
                            # Reverse control to counter momentum
                            control_output = -control_output * 1.5
                        elif approaching_fast:
                            mode = "BRAKE"
                            # Reduce control output to slow down
                            control_output *= 0.5
                        elif abs(error_rate) > 100:
                            mode = "CHASE"  # Target jumped away
                        else:
                            mode = "TRACK"

                        # Map control output to hold ratio (0.5 = balanced)
                        # Positive output = need to go right = higher hold ratio
                        # Negative output = need to go left = lower hold ratio
                        hold_ratio = 0.5 + control_output

                        # Clamp to valid range
                        hold_ratio = max(MIN_RATIO, min(MAX_RATIO, hold_ratio))

                    # Calculate variable cycle time based on distance
                    # Far away = longer cycles (longer holds), Close = shorter cycles (fast spam)
                    distance_factor = min(distance / CYCLE_SCALE_DISTANCE, 1.0)
                    cycle_time_ms = int(MIN_CYCLE_MS + (MAX_CYCLE_MS - MIN_CYCLE_MS) * distance_factor)

                    # Calculate hold/release timing for spam pattern
                    hold_ms = int(cycle_time_ms * hold_ratio)
                    release_ms = cycle_time_ms - hold_ms

                    # Debug output
                    print(f"[Fish] {mode} | Target: {target_line_x:.0f}px | Bar: {bar_center_x:.0f}px | "
                          f"Error: {error:+.0f}px | dE/dt: {error_rate:+.1f}px/s | "
                          f"P: {p_term:+.3f} | D: {d_term:+.3f} | Ratio: {hold_ratio:.2f} | "
                          f"Cycle: {cycle_time_ms}ms | Hold: {hold_ms}ms | Release: {release_ms}ms")

                    # Execute spam click cycle
                    elapsed_since_click = (current_time - last_click_time) * 1000  # Convert to ms

                    if is_holding:
                        # Currently holding, check if we should release
                        if elapsed_since_click >= hold_ms:
                            mouse_up()
                            is_holding = False
                            last_click_time = current_time
                            # print(f"[Fish] >>> RELEASE (after {hold_ms}ms) <<<")
                    else:
                        # Currently released, check if we should hold
                        if elapsed_since_click >= release_ms:
                            mouse_down()
                            is_holding = True
                            last_click_time = current_time
                            # print(f"[Fish] >>> HOLD (after {release_ms}ms) <<<")
