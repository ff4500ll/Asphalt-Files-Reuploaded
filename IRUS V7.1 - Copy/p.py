...
class HotkeyConfigApp:
    def __init__(self, root):
        ...
        self.first_launch_config = {
            ...
            "shake_timeout": 3.0,
            "fish_timeout": 3.0,
            "shake_confidence_threshold": 0.7,  # Lowered from 0.8 for fewer missed detections
            "fishing_confidence_threshold": 0.1,  # Lowered to match b.py for better Target Line detection
            "auto_minimize": True,
            "cast_mode": "Normal",
            "cast_delay1": 0.0,
            "cast_delay2": 0.3,
            ...
            "fish_kp": 0.7,                    # Proportional gain (KP)
            "fish_kd": 1.0,                    # Derivative gain (KD) - disabled for YOLO
            "fish_target_tol_px": 5.0,        # Dead zone tolerance in pixels
            "fish_pd_clamp": 1000.0,            # Maximum control signal clamp (±100)
            "fish_boundary_factor": 0.6,      # Boundary override margin (15% of box length)
        ...
        # Config file path - handle both .py and .exe (PyInstaller)
        ...
        self.config_file = os.path.join(application_path, "Config.txt")
        ...
        try:
            while self.loop_running:
                ...
                if self.active_settings.get("flow_fish_enabled", True):
                    ...
                    # Create keyboard controller for key presses
                    kb = pynput_keyboard.Controller()
                    ...
                    while self.loop_running:
                        ...
                        if target_line_x is None:
                            ...
                            # Still in cooldown - use alternating spam click to keep fish active
                            ...
                            # Press key "2" every cycle during line lost spam mode
                            kb.press('2')
                            time.sleep(0.05)
                            kb.release('2')
                            ...
                            print(f"[Fish] Line Lost Cooldown | Time Left: {time_left:.2f}s | SPAM {control_state} + Key '2'")
                        ...
                        # Use last known target_line_x if current frame lost it (but we have bar tracking)
                        target_x_to_use = target_line_x if target_line_x is not None else self.last_target_x
                        
                        if target_x_to_use is not None and self.box_center_x is not None and self.has_calculated_length_once:
                            
                                error = target_x_to_use - self.box_center_x  # Positive = target RIGHT of bar (need HOLD)
                                ...
                                is_near_left_boundary = (target_x_to_use < boundary_px_margin)
                                is_near_right_boundary = (target_x_to_use > capture_width - boundary_px_margin)
                                ...
                                if is_near_left_boundary:
                                    ...
                                    memory_indicator = " [Using Last Position]" if target_line_x is None else ""
                                    print(f"[Boundary Override] Left Edge | {control_state}{status_suffix}{memory_indicator}")
                                    ...
                                    # Skip PD control calculation
                                    memory_indicator = " [Using Last Position]" if target_line_x is None else ""
                                    print(f"[Boundary Override] Right Edge | {control_state}{status_suffix}{memory_indicator}")
                                    continue  # Skip to next frame
                                ...
                                memory_indicator = " [Using Last Position]" if target_line_x is None else ""
                                status_suffix = f" | PD Control: Signal={control_signal:+.2f}{memory_indicator}"
                            
                                # Update PD state (only update last_target_x if we have a real detection)
                    ...
                    # Press key "1" when exiting fish stage
                    kb.press('1')
                    time.sleep(0.05)
                    kb.release('1')
                    print("[Fish] Pressed key '1' on fish stage exit")
