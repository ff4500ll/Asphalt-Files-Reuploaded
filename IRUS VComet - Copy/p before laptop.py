"""
IRUS COMET - Advanced Fishing Bot by AsphaltCake
Version: 1.0 Release Candidate
Build Date: November 3, 2025

A comprehensive automation tool featuring:
- Multi-mode casting system (Normal, Perfect, GigaSigmaUltraMode)
- Advanced shake/fish detection with DXCam and MSS support
- Integrated comet minigame with full upgrade system
- Resolution-independent design (scales to any display)
- Thread-safe operation with proper resource management

For support and updates, visit the Discord community.
"""
...
# NOTE: These values are tuned for responsiveness vs CPU usage balance
...
class FishView(ctk.CTkFrame):
    ...
    def _create_color_options(self):
        ...
        control_mode_frame.grid(row=10, column=0, columnspan=2, pady=(15, 10), padx=20, sticky="ew")
        ...
        kp_frame.grid(row=11, column=0, columnspan=2, pady=(0, 10), padx=20, sticky="ew")
        ...
        kd_frame.grid(row=12, column=0, columnspan=2, pady=(0, 10), padx=20, sticky="ew")
    ...
    def _create_line_options(self):
        ...
        # KP Entry Box
        kp_frame = ctk.CTkFrame(self.options_content_frame, fg_color="transparent")
        kp_frame.grid(row=11, column=0, columnspan=2, pady=(15, 10), padx=20, sticky="ew")
        kp_frame.grid_columnconfigure(1, weight=1)
    ...
    def _on_color_control_mode_change(self, mode):
        ...
        print(f"Color mode control changed to: {mode}")
    ...
    def _on_line_control_mode_change(self, mode):
        ...
        print(f"Line mode control changed to: {mode}")
...
class CastView(ctk.CTkFrame):
    ...
    def _create_perfect_cast_settings(self):
        ...
        if not hasattr(self, 'perfect_cast_band1_release_var'):
            self.perfect_cast_band1_release_var = tk.IntVar(value=self.app.settings.get("perfect_cast_band1_release", 95))
            self.perfect_cast_band1_release_var.trace_add("write", lambda *args: self.app.settings.update({"perfect_cast_band1_release": self.perfect_cast_band1_release_var.get()}))
        ...
        if not hasattr(self, 'perfect_cast_band2_release_var'):
            self.perfect_cast_band2_release_var = tk.IntVar(value=self.app.settings.get("perfect_cast_band2_release", 95))
            self.perfect_cast_band2_release_var.trace_add("write", lambda *args: self.app.settings.update({"perfect_cast_band2_release": self.perfect_cast_band2_release_var.get()}))
        ...
        if not hasattr(self, 'perfect_cast_band3_release_var'):
            self.perfect_cast_band3_release_var = tk.IntVar(value=self.app.settings.get("perfect_cast_band3_release", 95))
            self.perfect_cast_band3_release_var.trace_add("write", lambda *args: self.app.settings.update({"perfect_cast_band3_release": self.perfect_cast_band3_release_var.get()}))
        ...
        if not hasattr(self, 'perfect_cast_band4_release_var'):
            self.perfect_cast_band4_release_var = tk.IntVar(value=self.app.settings.get("perfect_cast_band4_release", 92))
            self.perfect_cast_band4_release_var.trace_add("write", lambda *args: self.app.settings.update({"perfect_cast_band4_release": self.perfect_cast_band4_release_var.get()}))
    ...
    def _tune_last_cast(self, direction):
        ...
        # Map band index to setting key (11-band system)
        band_settings = [
        ...
        self.tune_status_label.configure(
            text=f"✅ {arrow} {band_names[band_idx]}: {current_value}% → {new_value}% | {velocity:.0f} px/s ({detailed_range})",
            text_color="#4ECDC4"
        )
        
        print(f"🎯 Auto-tuned {band_names[band_idx]}: {current_value}% → {new_value}% (Velocity: {velocity:.0f} px/s in {detailed_range}, Direction: {direction})")
...
class StarryNightApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.settings = {
            ...
            # Perfect Cast Release - Velocity-Percentage system (NEW)
            ...
            "fish_control_mode": "Normal",        # Control mode: Normal, Steady, or NigGamble
            ...
            "fish_line_control_mode": "Normal",   # Control mode: Normal, Steady, or NigGamble
        ...
        print("👁️ Main GUI is now visible")
    ...
    def run_bot_cycle(self):
        ...
        if not self.global_hotkey_states["Start/Stop"] or self.is_quitting:
            ...
            if not self.is_quitting:
                print("⏸️ Bot stopped, scheduling next check in 100ms...")
        ...
        try:
            ...
            if self.is_quitting:
                ...
                print(f"⏱️ Scheduling next cycle in 100ms... (Start/Stop={self.global_hotkey_states['Start/Stop']}, is_quitting={self.is_quitting})")
                self.after(100, self.run_bot_cycle)
    ...
    def _perform_perfect_cast_release_velocity_percentage(self):
        ...
        # Scale velocity band thresholds with resolution (velocity is measured in current resolution)
        # NEW 6-band system: <400, 400-600, 600-800, 800-1000, 1000-1200, >1200
        velocity_threshold_band1 = int(400 * scaling_factor)   # Band 1/2 boundary
        velocity_threshold_band2 = int(600 * scaling_factor)   # Band 2/3 boundary
        velocity_threshold_band3 = int(800 * scaling_factor)   # Band 3/4 boundary
        velocity_threshold_band4 = int(1000 * scaling_factor)  # Band 4/5 boundary
        velocity_threshold_band5 = int(1200 * scaling_factor)  # Band 5/6 boundary
        ...
        print(f"    🚴 Velocity bands: <{velocity_threshold_band1} | {velocity_threshold_band1}-{velocity_threshold_band2} | {velocity_threshold_band2}-{velocity_threshold_band3} | {velocity_threshold_band3}-{velocity_threshold_band4} | {velocity_threshold_band4}-{velocity_threshold_band5} | >{velocity_threshold_band5}")
            ...
            while True:
                ...
                if green_midpoint_result is not None and white_result is not None and local_white_y is not None:
                    ...
                    if len(white_positions) >= 3:
                        ...
                        # if velocity_y is not None and initial_white_y is not None and len(white_positions) == 3:
                        #     # Time elapsed since first detection
                        #     time_since_first = timestamps[-1] - timestamps[0]
                        #     
                        #     # Distance white traveled since first detection
                        #     distance_moved = abs(velocity_y) * time_since_first
                        #     
                        #     # Extrapolate backwards: true initial Y was further from green
                        #     # (white moves UP toward green, so initial_y was HIGHER value)
                        #     true_initial_white_y = white_positions[0][1] + distance_moved
                        #     true_initial_distance = abs(true_initial_white_y - local_green_y)
                        #     
                        #     # Update initial values with extrapolated true start
                        #     initial_white_y = true_initial_white_y
                        #     initial_distance = true_initial_distance
                        #     
                        #     print(f"    📐 Extrapolated true initial position:")
                        #     print(f"       Time since first detection: {time_since_first*1000:.1f}ms")
                        #     print(f"       Distance traveled: {distance_moved:.1f}px")
                        #     print(f"       True initial distance: {true_initial_distance:.1f}px (was {local_distance:.1f}px)")
                    
                    # Determine release percentage based on SCALED velocity bands (6 bands)
                    # <400: 95%, 400-600: 94%, 600-800: 93%, 800-1000: 92%, 1000-1200: 90%, >1200: 88%
                    # Initialize velocity_band with fallback value
                    velocity_band = "UNKNOWN (calculating...)"
                    
                    if velocity_y is not None:
                        abs_velocity = abs(velocity_y)
                        if abs_velocity < velocity_threshold_band1:
                            # Band 1: <400 px/s (95%)
                            active_release_percentage = self.settings.get("perfect_cast_band1_release", 95)
                            velocity_band = f"<400 px/s"
                        elif abs_velocity < velocity_threshold_band2:
                        ...
                        elif abs_velocity < velocity_threshold_band3:
                        ...
                        elif abs_velocity < velocity_threshold_band4:
                        ...
                        elif abs_velocity < velocity_threshold_band5:
                    ...
                    if gigasigma_state and gigasigma_state['allow_bounce'] and velocity_y is not None:
                        # Store velocity during initial_up phase (before bounce) for accurate tracking
                        # IMPORTANT: Only store ONCE when first good velocity is available (don't overwrite)
                        if 'bounce_phase' in gigasigma_state and gigasigma_state['bounce_phase'] == 'initial_up':
                            # Only store if we haven't stored yet AND we have a good velocity reading
                            if gigasigma_state['stored_velocity'] is None and abs(velocity_y) > 100:
                                ...
                                print(f"    💾 Stored velocity (LOCKED): {velocity_y:.1f}px/s raw → {normalized_velocity:.1f}px/s (1440p) - will use this for entire cast")
                        ...
                        if phase == 'initial_up' and current_direction == -1:
                            ...
                            # Hit green again on return pass, going down again
                            gigasigma_state['bounce_phase'] = 'bounced_down'
                            print(f"    ⬇️ Hit green again! Bouncing away...")
                    ...
                    if gigasigma_state and gigasigma_state['allow_bounce']:
                        # Adjust release threshold based on bounce count
                        # After bouncing, the bar travels much further, so we need early release
                        # But not TOO aggressive - tune based on feedback
                        bounce_adjustment = 0
                        if gigasigma_state['bounce_count'] > 0:
    ...
    def _detect_lines_in_frame(self, frame, original_width=None, filter_noise=False):
        ...
        Based on c.py/b.py improved line detection pipeline.
        
        Detects full-height vertical lines (top to bottom) using:
        - Canny edge detection for sharp edges (solid bars)
        - Sobel horizontal gradient for subtle color transitions (transparent bars)
        ...
        Args:
            ...
            filter_noise: If True, return empty list when >6 lines detected (noisy background)
        """
        try:
            ...
            # Step 2: Detect vertical edges using BOTH methods
            # Method 1: Canny for sharp edges (lowered thresholds for sensitivity)
            edges = cv2.Canny(gray, 50, 150)
            
            # Method 2: Sobel horizontal gradient for subtle color transitions
            sobelx = cv2.Sobel(gray, cv2.CV_64F, 1, 0, ksize=3)
            sobelx_abs = np.abs(sobelx)
            
            # Step 3: Find full-height vertical lines using BOTH methods
            vertical_lines = []
            line_strengths = []  # Track edge strength for selecting best pixel
            
            for col in range(width):
                # Method 1: Canny edge coverage (for sharp edges)
                column_edges = edges[:, col]
                edge_coverage = np.sum(column_edges > 0) / height
                
                # Method 2: Sobel gradient strength (for subtle color transitions)
                ...
                # Accept if strong Canny edges OR strong Sobel gradients (≥50% coverage)
            ...
            # Step 4: Merge adjacent pixels and select best edge from each cluster
            # Scale cluster tolerance based on resolution (2px at 1280x720, 5px at 2560x1440+)
            cluster_tolerance = max(2, int(width / 512))  # 2px minimum, scales with width
            ...
            for i in range(1, len(vertical_lines)):
                if vertical_lines[i] - vertical_lines[current_cluster_end] <= cluster_tolerance:
                    ...
                    # Cluster ended - find the best edge in this cluster
                    cluster = line_strengths[current_cluster_start:current_cluster_end+1]
                    # Prioritize highest Canny coverage, then highest Sobel
            ...
            # Optional noise filtering (used for shake exit condition)
            # If too many lines detected, likely noisy background
            if filter_noise and len(distinct_lines) > 6:
                return []
    ...
    def _execute_fish_stage_color(self):
        ...
        try:
            ...
            while self.global_hotkey_states["Start/Stop"] and not self.is_quitting:
                ...
                # Track edge transition state
                if not hasattr(self, '_was_at_edge'):
                    self._was_at_edge = False  # Were we at edge last scan?
                if not hasattr(self, '_edge_transition_time'):
                    self._edge_transition_time = None  # When did we leave edge?
                if not hasattr(self, '_edge_transition_action'):
                    self._edge_transition_action = None  # What action to do during transition (hold/release)
                ...
                if target_line_last_x is not None and bar_center_found and bar_center_x is not None:
                    ...
                    # STEP 5: Execute mouse control - edges can OVERRIDE PD/stability decision
                    # Check for edge transition first
                    EDGE_TRANSITION_DURATION = 0.15
                    in_transition = False
                    if self._was_at_edge and not target_at_edge:
                        if self._edge_transition_time is None:
                            self._edge_transition_time = current_time
                            # Opposite of where we were: left edge was releasing, so now hold. Right edge was holding, so now release
                            self._edge_transition_action = "release" if target_at_right_edge else "hold"
                            print(f"🔄 TRANSITION: Left edge - opposite action ({self._edge_transition_action}) for {EDGE_TRANSITION_DURATION}s")
                        
                        time_in_transition = current_time - self._edge_transition_time
                        if time_in_transition < EDGE_TRANSITION_DURATION:
                            in_transition = True
                            should_hold = (self._edge_transition_action == "hold")
                        else:
                            self._edge_transition_time = None
                            self._edge_transition_action = None
                            print(f"✅ TRANSITION: Complete")
                    
                    # Update edge tracking
                    if not target_at_edge and not in_transition:
            ...
            while self.global_hotkey_states["Start/Stop"] and not self.is_quitting:
                ...
                if len(line_coordinates) >= 2:
                    ...
                    if last_target_left_x is not None and last_target_right_x is not None:
                        ...
                        if max_jump > TELEPORT_THRESHOLD:
                            ...
                            if (potential_teleport_target_left == target_left_x and
                                ...
                                if time_since_first_detection >= TELEPORT_CONFIRM_TIME:
                                    # Teleport confirmed - accept new positions (don't update memory, edge detection handles it)
                                    print(f"✨ TELEPORT CONFIRMED after {time_since_first_detection:.3f}s - Accepting new positions (jump: {max_jump:.0f}px > {TELEPORT_THRESHOLD}px threshold)")
                                    ...
                                    print(f"⚠️ Potential teleport (jump: {max_jump:.0f}px > {TELEPORT_THRESHOLD}px, confirming: {time_since_first_detection:.3f}s/{TELEPORT_CONFIRM_TIME}s) - Using last positions")
                                    target_left_x = last_target_left_x
                                    target_right_x = last_target_right_x
                                ...
                                print(f"⚠️ New teleport candidate detected (jump: {max_jump:.0f}px > {TELEPORT_THRESHOLD}px threshold) - Starting confirmation")
                                target_left_x = last_target_left_x
                                target_right_x = last_target_right_x
                                left_bar_x = last_left_bar_x
                                right_bar_x = last_right_bar_x
                            ...
                            # Don't update memory here - edge detection will handle it
                            potential_teleport_target_left = None
                            potential_teleport_target_right = None
                            potential_teleport_left_bar = None
                            potential_teleport_right_bar = None
                        ...
                        # First run - don't update memory yet, edge detection will handle it
                        pass
                    ...
                    # Use detected positions for current frame calculations
                    ...
                    # Edge detection logic using bar_ratio (line mode uses bar_ratio instead of bar_ratio_from_side)
                    # Calculate edge boundaries based on current bar width and ratio
                    current_bar_width = abs(last_right_bar_x - last_left_bar_x) if last_left_bar_x is not None and last_right_bar_x is not None else 0
                    edge_threshold = current_bar_width * bar_ratio
                    
                    # Check if target is at extreme edges
                    target_at_left_edge = target_middle_x < edge_threshold
                    target_at_right_edge = target_middle_x > (width - edge_threshold)
                    target_at_edge = target_at_left_edge or target_at_right_edge
                    
                    # Update bar positions ONLY if in safe zone (not at edges)
                    # This updates both bar memory AND target memory for teleport tracking
                    if not target_at_edge:
                        last_target_left_x = target_left_x
                        last_target_right_x = target_right_x
                        last_left_bar_x = left_bar_x
                        last_right_bar_x = right_bar_x
                    elif last_left_bar_x is None or last_right_bar_x is None:
                    ...
                    # Calculate error (ALWAYS, regardless of edge state)
                    error = target_middle_x - bar_middle_x
                    
                    # Check stability state first (ALWAYS, regardless of edge state)
                    # Use MEMORY positions for stability check (only updated in safe zone)
                    if stability_state:
                        current_time = time.time()
                    ...
                    # Edge override and transition logic (happens AFTER stability/PD calculates should_hold)
                    EDGE_TRANSITION_DURATION = 0.15
                    in_transition = False
                    if was_at_edge and not target_at_edge:
                        if edge_transition_time is None:
                            edge_transition_time = current_time
                            edge_transition_action = "release" if target_at_right_edge else "hold"
                            print(f"� TRANSITION: Left edge - opposite action ({edge_transition_action}) for {EDGE_TRANSITION_DURATION}s")
                        
                        time_in_transition = current_time - edge_transition_time
                        if time_in_transition < EDGE_TRANSITION_DURATION:
                            in_transition = True
                            should_hold = (edge_transition_action == "hold")
                        else:
                            edge_transition_time = None
                            edge_transition_action = None
                            print(f"✅ TRANSITION: Complete")
