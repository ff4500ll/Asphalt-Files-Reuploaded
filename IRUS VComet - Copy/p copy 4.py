...
class FishView(ctk.CTkFrame):
    ...
    def _create_line_options(self):
        ...
        # Canny Lower Threshold (Edge Sensitivity)
        canny_lower_frame = ctk.CTkFrame(self.options_content_frame, fg_color="transparent")
        canny_lower_frame.grid(row=6, column=0, columnspan=2, pady=(0, 10), padx=20, sticky="ew")
        canny_lower_frame.grid_columnconfigure(1, weight=1)

        canny_lower_label = ctk.CTkLabel(
            canny_lower_frame,
            text="� Edge Sensitivity:",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#F0F0F0"
        )
        canny_lower_label.grid(row=0, column=0, padx=(0, 10), sticky="w")

        self.fish_line_canny_lower_var = tk.IntVar(value=self._get_rod_specific_setting("fish_line_canny_lower", 50))
        self.fish_line_canny_lower_var.trace_add("write", lambda *_: self._set_rod_specific_setting("fish_line_canny_lower", self.fish_line_canny_lower_var.get()))
        canny_lower_slider = ctk.CTkSlider(
            canny_lower_frame,
            from_=20,
            to=150,
            number_of_steps=130,
            variable=self.fish_line_canny_lower_var,
            progress_color="#00CED1",
            button_color="#00BFFF",
            button_hover_color="#1E90FF"
        )
        canny_lower_slider.grid(row=0, column=1, padx=(0, 10), sticky="ew")

        canny_lower_value = ctk.CTkLabel(
            canny_lower_frame,
            text="50",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#00BFFF",
        ...
        canny_lower_value.grid(row=0, column=2, sticky="e")

        canny_lower_slider.configure(command=lambda value: canny_lower_value.configure(text=f"{int(float(value))}"))
        canny_lower_value.configure(text=f"{self.fish_line_canny_lower_var.get()}")

        # Canny Upper Threshold (Edge Strength)
        canny_upper_frame = ctk.CTkFrame(self.options_content_frame, fg_color="transparent")
        canny_upper_frame.grid(row=7, column=0, columnspan=2, pady=(0, 10), padx=20, sticky="ew")
        canny_upper_frame.grid_columnconfigure(1, weight=1)

        canny_upper_label = ctk.CTkLabel(
            canny_upper_frame,
            text="💪 Edge Strength:",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#F0F0F0"
        )
        canny_upper_label.grid(row=0, column=0, padx=(0, 10), sticky="w")

        self.fish_line_canny_upper_var = tk.IntVar(value=self._get_rod_specific_setting("fish_line_canny_upper", 150))
        self.fish_line_canny_upper_var.trace_add("write", lambda *_: self._set_rod_specific_setting("fish_line_canny_upper", self.fish_line_canny_upper_var.get()))
        canny_upper_slider = ctk.CTkSlider(
            canny_upper_frame,
            from_=80,
            to=300,
            number_of_steps=220,
            variable=self.fish_line_canny_upper_var,
            progress_color="#9370DB",
            button_color="#8A2BE2",
            button_hover_color="#7B68EE"
        )
        canny_upper_slider.grid(row=0, column=1, padx=(0, 10), sticky="ew")

        canny_upper_value = ctk.CTkLabel(
            canny_upper_frame,
            text="150",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#8A2BE2",
        ...
        canny_upper_value.grid(row=0, column=2, sticky="e")

        canny_upper_slider.configure(command=lambda value: canny_upper_value.configure(text=f"{int(float(value))}"))
        canny_upper_value.configure(text=f"{self.fish_line_canny_upper_var.get()}")
        ...
        # KP Entry Box
        kp_frame = ctk.CTkFrame(self.options_content_frame, fg_color="transparent")
        kp_frame.grid(row=10, column=0, columnspan=2, pady=(15, 10), padx=20, sticky="ew")
        kp_frame.grid_columnconfigure(1, weight=1)
...
class StarryNightApp(ctk.CTk):
    ...
    def _detect_lines_in_frame(self, frame, original_width=None):
        """
        Detect vertical lines in frame using Canny edge detection.
        Based on b.py line detection pipeline with Canny edge detection and density filtering.
        ...
        """
        try:
            # Get image processing parameters from settings (configurable via GUI)
            MIN_LINE_DENSITY = self._get_rod_specific_setting("fish_line_min_density", 0.8)
            CANNY_LOWER = self._get_rod_specific_setting("fish_line_canny_lower", 50)
            CANNY_UPPER = self._get_rod_specific_setting("fish_line_canny_upper", 150)
            MERGE_DISTANCE = self._get_rod_specific_setting("fish_line_merge_distance", 2)
            ...
            # Height: 1217*(720/1440) to 1258*(720/1440) = 607 to 629 (height=22)
            ...
            # IMPORTANT: Only downscale if ABOVE reference resolution to avoid blur at 720p
            if original_frame_width > REFERENCE_FISH_WIDTH or original_height > REFERENCE_FISH_HEIGHT:
                # Downscale from higher resolution (e.g., 1440p → 720p reference)
                ...
                # At or below reference resolution - use original to avoid blur!
                width_scale = 1.0

            # Step 1: Convert to grayscale
            grayscale = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

            # Step 2: Canny edge detection (superior to Laplacian for line detection)
            # Canny uses gradient magnitude + double thresholding + hysteresis for robust edge tracking
            edges = cv2.Canny(grayscale, CANNY_LOWER, CANNY_UPPER)

            # Step 3: Filter vertical lines by density
            # Keep only columns where at least MIN_LINE_DENSITY of pixels are edges
            height, width = edges.shape
            
            # Vectorized column density calculation (10x faster than Python loop)
            # Canny returns 255 for edges, 0 for non-edges
            column_densities = np.sum(edges > 0, axis=0) / height
            ...
            # Lines must be within MERGE_DISTANCE pixels to be considered part of the same group
            if line_coordinates:
                merged_lines = []
                ...
                for i in range(1, len(line_coordinates)):
                    if line_coordinates[i] <= group_end + MERGE_DISTANCE:
                        # Within MERGE_DISTANCE pixels, extend current group
                        group_end = line_coordinates[i]
                    else:
                        # Gap > MERGE_DISTANCE pixels detected, save current group's middle position
    ...
    def _execute_fish_stage_line(self):
        ...
        try:
            ...
            print(f"⚙️ Line Settings: FPS={scan_fps}, Lost Timeout={fish_lost_timeout}s, Bar Ratio={bar_ratio}")
