...
class SimpleApp:
    storage = {
        "config": {
            ...
            "fish": {"method": "Color",
                    ...
                    "deadband_velocity_threshold_percent": 0.05,
                    "velocity_smoothing": 0.7,
                    "panic_speed_multiplier": 2.0,
                    "panic_large_error_percent": 0.05,
                    "panic_high_velocity_percent": 0.77,
                    "rods": {
                        "Default": {
                            "target_colors": [[0x5b, 0x4b, 0x43]],  # List of BGR color tuples: [[B, G, R], ...]
                            "bar_colors": [[0xf1, 0xf1, 0xf1], [0xff, 0xff, 0xff]],  # List of BGR color tuples
                            "arrow_colors": [[0x87, 0x85, 0x84]]  # List of BGR color tuples
                        }
                    }},
            "state_check": {"green_tolerance": 0, "top_corner_ratio": 85, "right_corner_ratio": 20},
        },
    }
    ...
    def _create_basic_tab(self):
        ...
        self.green_tolerance_var = tk.IntVar(value=self.storage["config"]["state_check"].get("green_tolerance", 0))
        ...
        self.green_tolerance_value_label = ctk.CTkLabel(scroll_frame, text=str(self.green_tolerance_var.get()), font=ctk.CTkFont(size=12))
        self.green_tolerance_value_label.grid(row=row, column=2, sticky="w", padx=5, pady=5)
    ...
    def _create_fish_tab(self):
        ...
        self.target_tolerance_var = tk.IntVar(value=self.storage["config"]["fish"].get("target_tolerance", 0))
        ...
        self.target_tolerance_value_label = ctk.CTkLabel(self.color_fish_frame, text=str(self.target_tolerance_var.get()), font=ctk.CTkFont(size=12))
        ...
        self.bar_tolerance_var = tk.IntVar(value=self.storage["config"]["fish"].get("bar_tolerance", 0))
        ...
        self.bar_tolerance_value_label = ctk.CTkLabel(self.color_fish_frame, text=str(self.bar_tolerance_var.get()), font=ctk.CTkFont(size=12))
        ...
        # Arrow Color Tolerance
        arrow_tolerance_label = ctk.CTkLabel(self.color_fish_frame, text="Arrow Color Tolerance:", font=ctk.CTkFont(size=12))
        arrow_tolerance_label.grid(row=7, column=0, sticky="w", padx=20, pady=5)
        arrow_tolerance_label.bind("<Button-1>", lambda e: self.root.focus())
        
        self.arrow_tolerance_var = tk.IntVar(value=self.storage["config"]["fish"].get("arrow_tolerance", 0))
        arrow_tolerance_slider = ctk.CTkSlider(
            self.color_fish_frame,
            from_=0,
            to=20,
            number_of_steps=20,
            variable=self.arrow_tolerance_var,
            command=self._on_arrow_tolerance_change,
            width=200
        )
        arrow_tolerance_slider.grid(row=7, column=1, sticky="w", padx=10, pady=5)
        
        self.arrow_tolerance_value_label = ctk.CTkLabel(self.color_fish_frame, text=str(self.arrow_tolerance_var.get()), font=ctk.CTkFont(size=12))
        self.arrow_tolerance_value_label.grid(row=7, column=2, sticky="w", padx=5, pady=5)
        self.arrow_tolerance_value_label.bind("<Button-1>", lambda e: self.root.focus())
        
        # Gap / Separator
        separator1 = ctk.CTkLabel(self.color_fish_frame, text="", font=ctk.CTkFont(size=6))
        separator1.grid(row=8, column=0, columnspan=3, pady=5)
        
        # Move Check Settings Header
        move_check_header = ctk.CTkLabel(self.color_fish_frame, text="Move Check Settings", 
                                         font=ctk.CTkFont(size=12, weight="bold"))
        move_check_header.grid(row=9, column=0, columnspan=3, sticky="w", padx=20, pady=(10, 5))
        ...
        stabilize_threshold_label.grid(row=10, column=0, sticky="w", padx=20, pady=5)
        ...
        stabilize_threshold_slider.grid(row=10, column=1, sticky="w", padx=10, pady=5)
        ...
        self.stabilize_threshold_value_label.grid(row=10, column=2, sticky="w", padx=5, pady=5)
        ...
        movement_threshold_label.grid(row=11, column=0, sticky="w", padx=20, pady=5)
        ...
        movement_threshold_slider.grid(row=11, column=1, sticky="w", padx=10, pady=5)
        ...
        self.movement_threshold_value_label.grid(row=11, column=2, sticky="w", padx=5, pady=5)
        ...
        separator2.grid(row=12, column=0, columnspan=3, pady=5)
        
        # Controller Settings Header
        controller_header = ctk.CTkLabel(self.color_fish_frame, text="Controller Settings", 
                                         font=ctk.CTkFont(size=12, weight="bold"))
        controller_header.grid(row=13, column=0, columnspan=3, sticky="w", padx=20, pady=(10, 5))
        ...
        stopping_distance_label.grid(row=14, column=0, sticky="w", padx=20, pady=5)
        ...
        stopping_distance_slider.grid(row=14, column=1, sticky="w", padx=10, pady=5)
        ...
        self.stopping_distance_value_label.grid(row=14, column=2, sticky="w", padx=5, pady=5)
        self.stopping_distance_value_label.bind("<Button-1>", lambda e: self.root.focus())
        
        # Deadband Percent
        deadband_percent_label = ctk.CTkLabel(self.color_fish_frame, text="Deadband (%):", font=ctk.CTkFont(size=12))
        deadband_percent_label.grid(row=15, column=0, sticky="w", padx=20, pady=5)
        deadband_percent_label.bind("<Button-1>", lambda e: self.root.focus())
        
        self.deadband_percent_var = tk.DoubleVar(value=self.storage["config"]["fish"].get("deadband_percent", 0.0))
        deadband_percent_slider = ctk.CTkSlider(
            self.color_fish_frame,
            from_=0.0,
            to=0.1,
            number_of_steps=100,
            variable=self.deadband_percent_var,
            command=self._on_deadband_percent_change,
            width=200
        )
        deadband_percent_slider.grid(row=15, column=1, sticky="w", padx=10, pady=5)
        
        self.deadband_percent_value_label = ctk.CTkLabel(self.color_fish_frame, text=f"{self.deadband_percent_var.get():.3f}", font=ctk.CTkFont(size=12))
        self.deadband_percent_value_label.grid(row=15, column=2, sticky="w", padx=5, pady=5)
        self.deadband_percent_value_label.bind("<Button-1>", lambda e: self.root.focus())
        
        # Deadband Velocity Threshold Percent
        deadband_velocity_label = ctk.CTkLabel(self.color_fish_frame, text="Deadband Velocity Threshold (%):", font=ctk.CTkFont(size=12))
        deadband_velocity_label.grid(row=16, column=0, sticky="w", padx=20, pady=5)
        deadband_velocity_label.bind("<Button-1>", lambda e: self.root.focus())
        
        self.deadband_velocity_var = tk.DoubleVar(value=self.storage["config"]["fish"].get("deadband_velocity_threshold_percent", 0.05))
        deadband_velocity_slider = ctk.CTkSlider(
            self.color_fish_frame,
            from_=0.01,
            to=0.2,
            number_of_steps=190,
            variable=self.deadband_velocity_var,
            command=self._on_deadband_velocity_change,
            width=200
        )
        deadband_velocity_slider.grid(row=16, column=1, sticky="w", padx=10, pady=5)
        
        self.deadband_velocity_value_label = ctk.CTkLabel(self.color_fish_frame, text=f"{self.deadband_velocity_var.get():.3f}", font=ctk.CTkFont(size=12))
        self.deadband_velocity_value_label.grid(row=16, column=2, sticky="w", padx=5, pady=5)
        self.deadband_velocity_value_label.bind("<Button-1>", lambda e: self.root.focus())
        
        # Velocity Smoothing
        velocity_smoothing_label = ctk.CTkLabel(self.color_fish_frame, text="Velocity Smoothing:", font=ctk.CTkFont(size=12))
        velocity_smoothing_label.grid(row=17, column=0, sticky="w", padx=20, pady=5)
        ...
        velocity_smoothing_slider.grid(row=17, column=1, sticky="w", padx=10, pady=5)
        ...
        self.velocity_smoothing_value_label.grid(row=17, column=2, sticky="w", padx=5, pady=5)
        self.velocity_smoothing_value_label.bind("<Button-1>", lambda e: self.root.focus())
        
        # Panic Speed Multiplier
        panic_speed_label = ctk.CTkLabel(self.color_fish_frame, text="Panic Speed Multiplier:", font=ctk.CTkFont(size=12))
        panic_speed_label.grid(row=18, column=0, sticky="w", padx=20, pady=5)
        panic_speed_label.bind("<Button-1>", lambda e: self.root.focus())
        
        self.panic_speed_var = tk.DoubleVar(value=self.storage["config"]["fish"].get("panic_speed_multiplier", 2.0))
        panic_speed_slider = ctk.CTkSlider(
            self.color_fish_frame,
            from_=1.0,
            to=5.0,
            number_of_steps=40,
            variable=self.panic_speed_var,
            command=self._on_panic_speed_change,
            width=200
        )
        panic_speed_slider.grid(row=18, column=1, sticky="w", padx=10, pady=5)
        
        self.panic_speed_value_label = ctk.CTkLabel(self.color_fish_frame, text=f"{self.panic_speed_var.get():.1f}", font=ctk.CTkFont(size=12))
        self.panic_speed_value_label.grid(row=18, column=2, sticky="w", padx=5, pady=5)
        self.panic_speed_value_label.bind("<Button-1>", lambda e: self.root.focus())
        
        # Panic Large Error Percent
        panic_error_label = ctk.CTkLabel(self.color_fish_frame, text="Panic Large Error (%):", font=ctk.CTkFont(size=12))
        panic_error_label.grid(row=19, column=0, sticky="w", padx=20, pady=5)
        panic_error_label.bind("<Button-1>", lambda e: self.root.focus())
        
        self.panic_error_var = tk.DoubleVar(value=self.storage["config"]["fish"].get("panic_large_error_percent", 0.05))
        panic_error_slider = ctk.CTkSlider(
            self.color_fish_frame,
            from_=0.01,
            to=0.2,
            number_of_steps=190,
            variable=self.panic_error_var,
            command=self._on_panic_error_change,
            width=200
        )
        panic_error_slider.grid(row=19, column=1, sticky="w", padx=10, pady=5)
        
        self.panic_error_value_label = ctk.CTkLabel(self.color_fish_frame, text=f"{self.panic_error_var.get():.3f}", font=ctk.CTkFont(size=12))
        self.panic_error_value_label.grid(row=19, column=2, sticky="w", padx=5, pady=5)
        self.panic_error_value_label.bind("<Button-1>", lambda e: self.root.focus())
        
        # Panic High Velocity Percent
        panic_velocity_label = ctk.CTkLabel(self.color_fish_frame, text="Panic High Velocity (%):", font=ctk.CTkFont(size=12))
        panic_velocity_label.grid(row=20, column=0, sticky="w", padx=20, pady=5)
        panic_velocity_label.bind("<Button-1>", lambda e: self.root.focus())
        
        self.panic_velocity_var = tk.DoubleVar(value=self.storage["config"]["fish"].get("panic_high_velocity_percent", 0.77))
        panic_velocity_slider = ctk.CTkSlider(
            self.color_fish_frame,
            from_=0.1,
            to=2.0,
            number_of_steps=190,
            variable=self.panic_velocity_var,
            command=self._on_panic_velocity_change,
            width=200
        )
        panic_velocity_slider.grid(row=20, column=1, sticky="w", padx=10, pady=5)
        
        self.panic_velocity_value_label = ctk.CTkLabel(self.color_fish_frame, text=f"{self.panic_velocity_var.get():.2f}", font=ctk.CTkFont(size=12))
        self.panic_velocity_value_label.grid(row=20, column=2, sticky="w", padx=5, pady=5)
        self.panic_velocity_value_label.bind("<Button-1>", lambda e: self.root.focus())
    ...
    def _add_rod_type(self):
        ...
        if rod_name and rod_name.strip():
            ...
            # Create new rod type with default colors
            self.storage["config"]["fish"]["rods"][rod_name] = {
                "target_colors": [[0x5b, 0x4b, 0x43]],
                "bar_colors": [[0xf1, 0xf1, 0xf1], [0xff, 0xff, 0xff]],
                "arrow_colors": [[0x87, 0x85, 0x84]]
            }
    ...
    def _delete_rod_type(self):
        ...
        # Don't allow deleting the Default rod
        if current_rod == "Default":
            error_dialog = ctk.CTkToplevel(self.root)
            error_dialog.title("Error")
            error_dialog.geometry("300x100")
            error_label = ctk.CTkLabel(error_dialog, text="Cannot delete the 'Default' rod type!", 
                                      font=ctk.CTkFont(size=12))
            error_label.pack(pady=20)
            ok_button = ctk.CTkButton(error_dialog, text="OK", command=error_dialog.destroy)
            ok_button.pack(pady=10)
        ...
        confirm_dialog.geometry("350x120")
        ...
        yes_button = ctk.CTkButton(button_frame, text="Yes", width=80, command=confirm_delete,
                                   fg_color="#8B0000", hover_color="#A52A2A")
        yes_button.pack(side="left", padx=10)
        
        no_button = ctk.CTkButton(button_frame, text="No", width=80, command=confirm_dialog.destroy)
    ...
    def _on_target_tolerance_change(self, value):
        ...
        self.storage["config"]["fish"]["target_tolerance"] = int_val
        self.target_tolerance_value_label.configure(text=str(int_val))
    
    def _on_bar_tolerance_change(self, value):
        int_val = int(float(value))
        self.storage["config"]["fish"]["bar_tolerance"] = int_val
        self.bar_tolerance_value_label.configure(text=str(int_val))
    
    def _on_arrow_tolerance_change(self, value):
        int_val = int(float(value))
        self.storage["config"]["fish"]["arrow_tolerance"] = int_val
        self.arrow_tolerance_value_label.configure(text=str(int_val))
    ...
    def _on_deadband_percent_change(self, value):
        self.storage["config"]["fish"]["deadband_percent"] = float(value)
        self.deadband_percent_value_label.configure(text=f"{float(value):.3f}")
    
    def _on_deadband_velocity_change(self, value):
        self.storage["config"]["fish"]["deadband_velocity_threshold_percent"] = float(value)
        self.deadband_velocity_value_label.configure(text=f"{float(value):.3f}")
    ...
    def _on_panic_speed_change(self, value):
        self.storage["config"]["fish"]["panic_speed_multiplier"] = float(value)
        self.panic_speed_value_label.configure(text=f"{float(value):.1f}")
    
    def _on_panic_error_change(self, value):
        self.storage["config"]["fish"]["panic_large_error_percent"] = float(value)
        self.panic_error_value_label.configure(text=f"{float(value):.3f}")
    
    def _on_panic_velocity_change(self, value):
        self.storage["config"]["fish"]["panic_high_velocity_percent"] = float(value)
        self.panic_velocity_value_label.configure(text=f"{float(value):.2f}")
    ...
    def _create_cast_tab(self):
        ...
        release_timing_instruction = ctk.CTkLabel(
            self.perfect_cast_frame, 
            text="Release too late? Drag left ←  |  Release too early? Drag right  →",
        ...
        self.release_timing_var = tk.DoubleVar(value=self.release_timing)
        release_timing_slider = ctk.CTkSlider(
            self.perfect_cast_frame,
            from_=-50.0,
            to=50.0,
            number_of_steps=1000,
            variable=self.release_timing_var,
            command=self._on_release_timing_change,
            width=200
        )
        release_timing_slider.grid(row=5, column=1, sticky="w", padx=10, pady=5)
        ...
        self.release_timing_value_label = ctk.CTkLabel(self.perfect_cast_frame, text=get_timing_display(self.release_timing), font=ctk.CTkFont(size=12))
        self.release_timing_value_label.grid(row=5, column=2, sticky="w", padx=5, pady=5)
    ...
    def _create_shake_tab(self):
        ...
        shake_method_menu = ctk.CTkOptionMenu(
            ...
            values=["Pixel", "Navigation", "Circle", "Disabled"],
            width=150,
    ...
    def _on_green_tolerance_change(self, value):
        ...
        self.green_tolerance_value_label.configure(text=str(int_value))
    ...
    def _on_release_timing_change(self, value):
        """Update release timing when slider changes"""
        timing = float(value)
        self.release_timing = timing
        self.storage["config"]["cast"]["release_timing"] = timing
        self.release_timing_value_label.configure(text=self._get_timing_display(timing))
    ...
    def _move_roblox_to_monitor1(self):
        """Move Roblox window to monitor 1 (primary monitor) and maximize it"""
        try:
            # Find Roblox window
            ...
            # Restore window (unmaximize) first
            roblox.restore()
            time.sleep(0.1)  # Small delay to ensure restore completes
            
            # Move to monitor 1 (position 0, 0 is top-left of primary monitor)
            roblox.moveTo(0, 0)
            time.sleep(0.1)  # Small delay before maximizing
        ...
        def on_frame_arrived(frame: Frame, capture_control: InternalCaptureControl):
            ...
            try:
                ...
                if not tracking_mode:
                    ...
                    target_green = np.array([0x4b, 0xac, 0x61], dtype=np.int16)
                    ...
                    if rows.size > 0:
                        ...
                        tracking_mode = True
                        print(f"Cast: Found green bar at Y={green_y}")
                    ...
                    if rows.size > 0:
                        ...
                        # Update overlay arrows for green if enabled
                        if show_overlay and canvas:
                            arrow_ids['green_right_horiz'] = update_or_create_arrow(canvas, arrow_ids.get('green_right_horiz'), 
                                                                                    get_arrow_coords_horizontal(green_right_x + arrow_offset, green_y, 'left'), 'red')
                            arrow_ids['green_right_vert'] = update_or_create_arrow(canvas, arrow_ids.get('green_right_vert'), 
                                                                                   get_arrow_coords_vertical(green_right_x, green_y - arrow_offset, 'down'), 'red')
                            arrow_ids['green_left_horiz'] = update_or_create_arrow(canvas, arrow_ids.get('green_left_horiz'), 
                                                                                   get_arrow_coords_horizontal(green_left_x - arrow_offset, green_y, 'right'), 'red')
                            arrow_ids['green_left_vert'] = update_or_create_arrow(canvas, arrow_ids.get('green_left_vert'), 
                                                                                  get_arrow_coords_vertical(green_left_x, green_y - arrow_offset, 'down'), 'red')
                        
                        # Search for white bar below green (scan to 90% of screen height)
                        scan_bottom = int(screen_height * 0.9)
                        white_frame = bgr[green_y:scan_bottom, green_left_x:green_right_x, :]
                        
                        target_white = np.array([0xf3, 0xfe, 0xff], dtype=np.int16)
        ...
        try:
            with mss.mss(compression_level=0) as sct:  # No compression for speed
                ...
                if not hasattr(self, '_mss_monitor_setup_shown'):
                    print(f"Cast: MSS using monitor 1 - size: {screen_width}x{screen_height}")
                ...
                while not release_executed and not scan_failed and not self.stop_event.is_set():
                    ...
                    if tracking_mode and green_y is not None and green_left_x is not None and green_right_x is not None:
                        ...
                        # Calculate region bounds in monitor coordinates (like Windows Capture)
                        ...
                        region_bottom = int(screen_height * 0.9)  # Scan to 90% of screen height
                        ...
                        # Convert to absolute screen coordinates for MSS
                        region_dict["top"] = int(region_top + monitor["top"])
                        ...
                        # Full monitor capture for initial green bar detection (like Windows Capture)
                        ...
                        if rows.size > 0:
                            ...
                            if show_overlay and canvas:
                                arrow_ids['green_left_horiz'] = update_or_create_arrow(
                                    ...
                                    get_arrow_coords_horizontal(green_left_x - arrow_offset, green_y, 'right'),
                                    'red'
                                )
                                arrow_ids['green_left_vert'] = update_or_create_arrow(
                                    canvas, arrow_ids['green_left_vert'],
                                    get_arrow_coords_vertical(green_left_x, green_y - arrow_offset, 'down'),
                                    'red'
                                )
                                arrow_ids['green_right_horiz'] = update_or_create_arrow(
                                    canvas, arrow_ids['green_right_horiz'],
                                    get_arrow_coords_horizontal(green_right_x + arrow_offset, green_y, 'left'),
                                    'red'
                                )
                                arrow_ids['green_right_vert'] = update_or_create_arrow(
                                    canvas, arrow_ids['green_right_vert'],
                                    get_arrow_coords_vertical(green_right_x, green_y - arrow_offset, 'down'),
                                    'red'
                                )
                                ...
                                if total_distance > 0:
                                    ...
                                    if show_overlay and canvas:
                                        ...
                                        arrow_ids['white_top_left'] = update_or_create_arrow(
                                            ...
                                            get_arrow_coords_horizontal(green_left_x - arrow_offset, predicted_white_y_top, 'right'),
                                            'yellow'
                                        )
                                        arrow_ids['white_top_right'] = update_or_create_arrow(
                                            canvas, arrow_ids['white_top_right'],
                                            get_arrow_coords_horizontal(green_right_x + arrow_offset, predicted_white_y_top, 'left'),
                                            'yellow'
                                        )
                                        ...
                                        if white_y_bottom != white_y_top:
                                            arrow_ids['white_bottom_left'] = update_or_create_arrow(
                                                ...
                                                get_arrow_coords_horizontal(green_left_x - arrow_offset, white_y_bottom, 'right'),
                                                'cyan'
                                            )
                                            arrow_ids['white_bottom_right'] = update_or_create_arrow(
                                                canvas, arrow_ids['white_bottom_right'],
                                                get_arrow_coords_horizontal(green_right_x + arrow_offset, white_y_bottom, 'left'),
                                                'cyan'
                                            )
                                            arrow_ids['white_bottom_up_left'] = update_or_create_arrow(
                                                canvas, arrow_ids['white_bottom_up_left'],
                                                get_arrow_coords_vertical(green_left_x, white_y_bottom + arrow_offset, 'up'),
                                                'cyan'
                                            )
                                            arrow_ids['white_bottom_up_right'] = update_or_create_arrow(
                                                canvas, arrow_ids['white_bottom_up_right'],
                                                get_arrow_coords_vertical(green_right_x, white_y_bottom + arrow_offset, 'up'),
                                                'cyan'
                                            )
    ...
    def _shake(self):
        ...
        if shake_method == "Disabled":
            ...
            # TODO: Implement navigation shake detection
            # Will use navigation-based shake detection
            print("Shake: Navigation detection not yet implemented")
            return None
        
        elif shake_method == "Circle":
            print("Shake: Circle mode")
            # TODO: Implement circle shake detection
            # Will use circle-based shake pattern
            print("Shake: Circle detection not yet implemented")
            return None
    ...
    def _fish(self):
        ...
        elif fish_method == "Line":
            print("Fish: Line mode")
            # TODO: Implement line-based fish detection
            print("Fish: Line detection not yet implemented")
    ...
    def _fish_color_windows_capture(self):
        ...
        target_colors = [tuple(c) for c in rod_colors.get("target_colors", [[0x5b, 0x4b, 0x43]])]
        bar_colors = [tuple(c) for c in rod_colors.get("bar_colors", [[0xf1, 0xf1, 0xf1], [0xff, 0xff, 0xff]])]
        arrow_colors = [tuple(c) for c in rod_colors.get("arrow_colors", [[0x87, 0x85, 0x84]])]
        
        target_tolerance = self.storage["config"]["fish"]["target_tolerance"]
        bar_tolerance = self.storage["config"]["fish"]["bar_tolerance"]
        arrow_tolerance = self.storage["config"]["fish"]["arrow_tolerance"]
        ...
        stopping_distance_multiplier = self.storage["config"]["fish"]["stopping_distance_multiplier"]
        deadband_percent = self.storage["config"]["fish"]["deadband_percent"]
        deadband_velocity_threshold_percent = self.storage["config"]["fish"]["deadband_velocity_threshold_percent"]
        velocity_smoothing = self.storage["config"]["fish"]["velocity_smoothing"]
        panic_speed_multiplier = self.storage["config"]["fish"]["panic_speed_multiplier"]
        panic_large_error_percent = self.storage["config"]["fish"]["panic_large_error_percent"]
        panic_high_velocity_percent = self.storage["config"]["fish"]["panic_high_velocity_percent"]
        ...
        deadband = int(fish_area["width"] * deadband_percent)
        deadband_velocity_threshold = fish_area["width"] * deadband_velocity_threshold_percent
        panic_large_error = int(fish_area["width"] * panic_large_error_percent)
        panic_high_velocity = fish_area["width"] * panic_high_velocity_percent
        ...
        def on_frame_arrived(frame: Frame, capture_control: InternalCaptureControl):
            ...
            nonlocal arrow_previous_click_state, is_capturing
            ...
            if left_x is not None:
                ...
                if bar_left_found and bar_right_found:
                    ...
                    elif abs(error) <= deadband and abs(relative_velocity) < deadband_velocity_threshold:
                        # In deadband - hover
                        if color_check_click_state:
                            win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
                            color_check_click_state = False
                    ...
                    if arrow_colors and color_check_bar_width is not None:
                        ...
                        if arrow_left_found and arrow_right_found:
                            ...
                            if arrow_estimated_bar_left is None or arrow_estimated_bar_right is None:
                                ...
                                # Track arrow movement
                                if arrow_previous_position is not None and arrow_previous_click_state == color_check_click_state:
                                    # Determine which side arrow is on
                                    current_side = "left" if arrow_middle < ((arrow_estimated_bar_left + arrow_estimated_bar_right) // 2) else "right"
                                    
                                    if current_side != arrow_previous_side:
                                        # Arrow switched sides - recalculate bar position
                                ...
                                # Panic mode checks
                                is_panic_mode = False
                                if abs(error) > panic_large_error or abs(relative_velocity) > panic_high_velocity:
                                    is_panic_mode = True
                                
                                # Deadband hover logic
                                in_deadband = abs(error) <= deadband
                                bar_moving_slow = abs(color_check_bar_velocity) < deadband_velocity_threshold
                                
                                if in_deadband and bar_moving_slow:
                                    # Hold position - don't click or release
                                    pass
                                elif error < -stopping_distance:
                                    # Bar too far left - release to move right
                                    if color_check_click_state:
                                        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
                                        color_check_click_state = False
                                elif error > stopping_distance or is_panic_mode:
                                    # Bar too far right or panic - hold to move left
                                    if not color_check_click_state:
                                        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0)
                                        color_check_click_state = True
                                else:
                                    # Within stopping distance - coast
                                    if color_check_click_state:
                                        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, 0, 0, 0, 0)
                                        color_check_click_state = False
            else:
    ...
    def _fish_color_mss(self):
        ...
        # TODO: Implement MSS color-based fish detection
        print("Fish: Color detection (MSS) not yet implemented")
