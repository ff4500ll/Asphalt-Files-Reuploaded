...
class CastView(ctk.CTkFrame):
    ...
    def _create_perfect_cast_settings(self):
        ...
        instructions_title = ctk.CTkLabel(
            instructions_frame,
            text="📖 NEW: Velocity-Based Release System",
            font=ctk.CTkFont(size=16, weight="bold"),
            text_color="#F5A623"
        )
        ...
        instructions_text = ctk.CTkLabel(
            instructions_frame,
            text="How it works:\n"
                 "• System automatically adjusts release point based on white bar speed\n"
                 "• VERY SLOW (<400 px/s): Releases at 95% (earliest, most input lag)\n"
                 "• SLOW (400-600 px/s): Releases at 95%\n"
                 "• MEDIUM (600-1000 px/s): Releases at 93%\n"
                 "• FAST (>1000 px/s): Releases at 91% (late, quick movement)\n"
                 "• Velocity shown normalized to 1440p (same speed = same number at any resolution)",
            font=ctk.CTkFont(size=12),
            text_color="#D0D0D0",
            justify="left"
        )
        ...
        # VERY SLOW speed slider
        very_slow_frame = ctk.CTkFrame(self.perfect_cast_options_frame, fg_color="transparent")
        very_slow_frame.grid(row=4, column=0, pady=(0, 10), sticky="ew")
        very_slow_frame.grid_columnconfigure(1, weight=1)
        
        very_slow_label = ctk.CTkLabel(
            very_slow_frame,
            text="🐢 VERY SLOW (<400 px/s):",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#8B0000"
        )
        very_slow_label.grid(row=0, column=0, padx=(0, 10), sticky="w")

        if not hasattr(self, 'perfect_cast_very_slow_release_var'):
            self.perfect_cast_very_slow_release_var = tk.IntVar(value=self.app.settings.get("perfect_cast_very_slow_release", 95))
            self.perfect_cast_very_slow_release_var.trace_add("write", lambda *args: self.app.settings.update({"perfect_cast_very_slow_release": self.perfect_cast_very_slow_release_var.get()}))
        
        very_slow_slider = ctk.CTkSlider(
            very_slow_frame,
            from_=70,
            to=95,
            number_of_steps=25,
            variable=self.perfect_cast_very_slow_release_var,
            progress_color="#8B0000",
            button_color="#7A0000",
            button_hover_color="#A50000"
        )
        very_slow_slider.grid(row=0, column=1, padx=(0, 10), sticky="ew")
        
        very_slow_value = ctk.CTkLabel(
            very_slow_frame,
            text="95%",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#8B0000",
        ...
        very_slow_value.grid(row=0, column=2, sticky="e")

        def update_very_slow_display(value):
            very_slow_value.configure(text=f"{int(float(value))}%")
        very_slow_slider.configure(command=update_very_slow_display)
        update_very_slow_display(self.perfect_cast_very_slow_release_var.get())
        
        # SLOW speed slider
        slow_frame = ctk.CTkFrame(self.perfect_cast_options_frame, fg_color="transparent")
        slow_frame.grid(row=5, column=0, pady=(0, 10), sticky="ew")
        slow_frame.grid_columnconfigure(1, weight=1)
        
        slow_label = ctk.CTkLabel(
            slow_frame,
            text="🐌 SLOW (400-600 px/s):",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#FF6B6B"
        )
        slow_label.grid(row=0, column=0, padx=(0, 10), sticky="w")

        if not hasattr(self, 'perfect_cast_slow_release_var'):
            self.perfect_cast_slow_release_var = tk.IntVar(value=self.app.settings.get("perfect_cast_slow_release", 95))
            self.perfect_cast_slow_release_var.trace_add("write", lambda *args: self.app.settings.update({"perfect_cast_slow_release": self.perfect_cast_slow_release_var.get()}))
        
        slow_slider = ctk.CTkSlider(
            slow_frame,
            from_=70,
            to=95,
            number_of_steps=25,
            variable=self.perfect_cast_slow_release_var,
            progress_color="#FF6B6B",
            button_color="#E85A5A",
            button_hover_color="#FF8282"
        )
        slow_slider.grid(row=0, column=1, padx=(0, 10), sticky="ew")
        
        slow_value = ctk.CTkLabel(
            slow_frame,
            text="95%",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#FF6B6B",
        ...
        slow_value.grid(row=0, column=2, sticky="e")

        def update_slow_display(value):
            slow_value.configure(text=f"{int(float(value))}%")
        slow_slider.configure(command=update_slow_display)
        update_slow_display(self.perfect_cast_slow_release_var.get())

        # MEDIUM speed slider
        medium_frame = ctk.CTkFrame(self.perfect_cast_options_frame, fg_color="transparent")
        medium_frame.grid(row=6, column=0, pady=(0, 10), sticky="ew")
        medium_frame.grid_columnconfigure(1, weight=1)
        
        medium_label = ctk.CTkLabel(
            medium_frame,
            text="🚶 MEDIUM (600-1000 px/s):",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#FFA500"
        )
        medium_label.grid(row=0, column=0, padx=(0, 10), sticky="w")

        if not hasattr(self, 'perfect_cast_medium_release_var'):
            self.perfect_cast_medium_release_var = tk.IntVar(value=self.app.settings.get("perfect_cast_medium_release", 93))
            self.perfect_cast_medium_release_var.trace_add("write", lambda *args: self.app.settings.update({"perfect_cast_medium_release": self.perfect_cast_medium_release_var.get()}))
        
        medium_slider = ctk.CTkSlider(
            medium_frame,
            from_=70,
            to=95,
            number_of_steps=25,
            variable=self.perfect_cast_medium_release_var,
            progress_color="#FFA500",
            button_color="#E69400",
            button_hover_color="#FFB733"
        )
        medium_slider.grid(row=0, column=1, padx=(0, 10), sticky="ew")
        
        medium_value = ctk.CTkLabel(
            medium_frame,
            text="93%",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#FFA500",
        ...
        medium_value.grid(row=0, column=2, sticky="e")

        def update_medium_display(value):
            medium_value.configure(text=f"{int(float(value))}%")
        medium_slider.configure(command=update_medium_display)
        update_medium_display(self.perfect_cast_medium_release_var.get())

        # FAST speed slider
        fast_frame = ctk.CTkFrame(self.perfect_cast_options_frame, fg_color="transparent")
        fast_frame.grid(row=7, column=0, pady=(0, 10), sticky="ew")
        ...
        fast_label.grid(row=0, column=0, padx=(0, 10), sticky="w")

        if not hasattr(self, 'perfect_cast_fast_release_var'):
            self.perfect_cast_fast_release_var = tk.IntVar(value=self.app.settings.get("perfect_cast_fast_release", 91))
            self.perfect_cast_fast_release_var.trace_add("write", lambda *args: self.app.settings.update({"perfect_cast_fast_release": self.perfect_cast_fast_release_var.get()}))
        
        fast_slider = ctk.CTkSlider(
            fast_frame,
            from_=70,
            to=95,
            number_of_steps=25,
            variable=self.perfect_cast_fast_release_var,
            progress_color="#4CAF50",
            button_color="#45A045",
            button_hover_color="#66BB6A"
        )
        fast_slider.grid(row=0, column=1, padx=(0, 10), sticky="ew")
        
        fast_value = ctk.CTkLabel(
            fast_frame,
            text="91%",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#4CAF50",
        ...
        fast_value.grid(row=0, column=2, sticky="e")

        def update_fast_display(value):
            fast_value.configure(text=f"{int(float(value))}%")
        fast_slider.configure(command=update_fast_display)
        update_fast_display(self.perfect_cast_fast_release_var.get())

        # Fallback slider
        percentage_frame = ctk.CTkFrame(self.perfect_cast_options_frame, fg_color="transparent")
        percentage_frame.grid(row=8, column=0, pady=(0, 15), sticky="ew")
...
class StarryNightApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.settings = {
            ...
            # Perfect Cast Release - NEW: Velocity-based percentage system
            "perfect_cast_release_percentage": 80,     # Fallback when velocity not detected (70-95%)
            "perfect_cast_very_slow_release": 95,      # VERY SLOW speed (<400 px/s) release percentage
            "perfect_cast_slow_release": 95,           # SLOW speed (400-600 px/s) release percentage
            "perfect_cast_medium_release": 93,         # MEDIUM speed (600-1000 px/s) release percentage
            "perfect_cast_fast_release": 91,           # FAST speed (>1000 px/s) release percentage
    ...
    def _perform_perfect_cast_release(self):
        ...
        # Base thresholds at 1440p: <400 very slow, 400-600 slow, 600-1000 medium, >1000 fast
        velocity_threshold_very_slow = int(400 * scaling_factor)  # Very Slow/Slow boundary
        velocity_threshold_slow = int(600 * scaling_factor)       # Slow/Medium boundary
        velocity_threshold_fast = int(1000 * scaling_factor)      # Medium/Fast boundary
        ...
        print(f"    🏎️ Velocity thresholds: <{velocity_threshold_very_slow} VSLOW | {velocity_threshold_very_slow}-{velocity_threshold_slow} SLOW | {velocity_threshold_slow}-{velocity_threshold_fast} MED | >{velocity_threshold_fast} FAST")
            ...
            while True:
                ...
                if green_midpoint_result is not None and white_result is not None and local_white_y is not None:
                    ...
                    if initial_white_y is None:
                        ...
                        print(f"    📍 Initial white detected: Y={local_white_y}px, Distance={local_distance}px")
                    ...
                    if len(white_positions) >= 3:
                        ...
                        # CRITICAL FIX: Extrapolate true initial position using velocity
                        # When white is first detected, it's already moving - backtrack to find true start
                    ...
                    # Determine release percentage based on SCALED velocity bands
                    # Slower = Release earlier (higher %) to account for input lag
                    # Faster = Release later (lower %) since movement is quick
                    # Thresholds scale with resolution since velocity is measured in current resolution
                    if velocity_y is not None:
                        abs_velocity = abs(velocity_y)
                        if abs_velocity < velocity_threshold_very_slow:
                            # Very slow speed: Release earliest (get from GUI settings)
                            active_release_percentage = self.settings.get("perfect_cast_very_slow_release", 95)
                            velocity_band = f"VERY SLOW (<{velocity_threshold_very_slow} px/s)"
                        elif abs_velocity < velocity_threshold_slow:
                            # Slow speed: Release earlier (get from GUI settings)
                            active_release_percentage = self.settings.get("perfect_cast_slow_release", 95)
                            velocity_band = f"SLOW ({velocity_threshold_very_slow}-{velocity_threshold_slow} px/s)"
                        elif abs_velocity < velocity_threshold_fast:
                            # Medium speed (get from GUI settings)
                            active_release_percentage = self.settings.get("perfect_cast_medium_release", 93)
                            velocity_band = f"MEDIUM ({velocity_threshold_slow}-{velocity_threshold_fast} px/s)"
                        else:
                            # Fast speed: Release later (get from GUI settings)
                            active_release_percentage = self.settings.get("perfect_cast_fast_release", 91)
                            velocity_band = f"FAST (>{velocity_threshold_fast} px/s)"
                    else:
                    ...
                    # Release when white has traveled X% of the total distance to green
                    if progress_percentage >= active_release_percentage:
                        # Normalize velocity display to 1440p reference for consistency across resolutions
                        ...
                        print(f"       Progress: {progress_percentage:.1f}% (threshold: {active_release_percentage}%)")
