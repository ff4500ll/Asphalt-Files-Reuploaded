...
def z():
    ...
    # Create capture instance - Windows Capture with border, no cursor
    ...
    def on_frame_arrived(frame: Frame, capture_control: InternalCaptureControl):
        nonlocal output_log, log_sequence, last_logged_state
        ...
        # Check for green pixel in bottom-left corner (RGB format in frame_array)
        ...
        # Check if green pixel exists (RGB in BGRA format, so indices are B=0, G=1, R=2)
    ...
    try:
        # Start capture (runs indefinitely)
        capture.start()
    finally:
