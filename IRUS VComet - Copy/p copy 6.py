...
class FishView(ctk.CTkFrame):  # AI_TARGET: FISH_GUI
    ...
    def _create_color_options(self):
        ...
        # Edge Transition Duration
        edge_transition_frame = ctk.CTkFrame(self.options_content_frame, fg_color="transparent")
        edge_transition_frame.grid(row=10, column=0, columnspan=2, pady=(0, 10), padx=20, sticky="ew")
        edge_transition_frame.grid_columnconfigure(1, weight=1)

        edge_transition_label = ctk.CTkLabel(
            edge_transition_frame,
            text="🔄 Edge Transition:",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#F0F0F0"
        )
        edge_transition_label.grid(row=0, column=0, padx=(0, 10), sticky="w")

        current_transition = self._get_rod_specific_setting("fish_edge_transition_duration", 0.25)
        edge_transition_slider = ctk.CTkSlider(
            edge_transition_frame,
            from_=0.0,
            to=1.0,
        ...
        edge_transition_slider.set(float(current_transition))
        edge_transition_slider.grid(row=0, column=1, padx=(0, 10), sticky="ew")

        edge_transition_value = ctk.CTkLabel(
            edge_transition_frame,
            text="0.25s",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color="#9AE84A",
        ...
        edge_transition_value.grid(row=0, column=2, sticky="e")

        def update_edge_transition(value):
            duration = round(float(value), 2)
            edge_transition_value.configure(text=f"{duration:.2f}s")
            self._set_rod_specific_setting("fish_edge_transition_duration", duration)

        edge_transition_slider.configure(command=update_edge_transition)
    ...
    def _create_line_options(self):
        ...
        # Edge Transition Duration
        edge_transition_frame = ctk.CTkFrame(self.options_content_frame, fg_color="transparent")
        edge_transition_frame.grid(row=10, column=0, columnspan=2, pady=(15, 10), padx=20, sticky="ew")
        edge_transition_frame.grid_columnconfigure(1, weight=1)
        ...
        self.fish_line_edge_transition_var = tk.DoubleVar(value=self._get_rod_specific_setting("fish_line_edge_transition_duration", 0.25))
        self.fish_line_edge_transition_var.trace_add("write", lambda *_: self._set_rod_specific_setting("fish_line_edge_transition_duration", self.fish_line_edge_transition_var.get()))
        edge_transition_slider = ctk.CTkSlider(
            edge_transition_frame,
            from_=0.0,
            to=1.0,
            number_of_steps=100,
            variable=self.fish_line_edge_transition_var,
            progress_color="#7ED321",
            button_color="#9AE84A",
            button_hover_color="#6BB91A"
        )
        ...
        edge_transition_slider.configure(command=lambda value: edge_transition_value.configure(text=f"{float(value):.2f}s"))
        edge_transition_value.configure(text=f"{self.fish_line_edge_transition_var.get():.2f}s")
...
class StarryNightApp(ctk.CTk):  # AI_TARGET: MAIN_APPLICATION_CLASS
    ...
    def __init__(self):  # AI_TARGET: APP_INITIALIZATION
        ...
        self.settings = {
            ...
            "fish_edge_transition_duration": 0.25, # Duration of opposite action after leaving edge (0.0-1.0s)
            ...
            "fish_line_edge_transition_duration": 0.25, # Duration of opposite action after leaving edge (0.0-1.0s)
        ...
        try:
            ...
            while self.global_hotkey_states["Start/Stop"] and not self.is_quitting:
                ...
                if target_line_last_x is not None and bar_center_found and bar_center_x is not None:
                    ...
                    EDGE_TRANSITION_DURATION = self._get_rod_specific_setting("fish_edge_transition_duration", 0.25)
                    in_transition = False
        ...
        try:
            ...
            while self.global_hotkey_states["Start/Stop"] and not self.is_quitting:
                ...
                if len(line_coordinates) >= 2:
                    ...
                    # Lock edge threshold when entering edge zone (don't update while at edge)
                    if 'locked_edge_threshold_line' not in locals() or locked_edge_threshold_line is None:
                        # Not at edge or first time - calculate from current bar width
                        edge_threshold = current_bar_width * bar_ratio
                        locked_edge_threshold_line = edge_threshold
                    else:
                        # Already at edge - use locked threshold
                        edge_threshold = locked_edge_threshold_line
                    ...
                    if not target_at_edge:
                        # Clear locked threshold when in safe zone
                        locked_edge_threshold_line = None
                        last_target_left_x = target_left_x
                        last_target_right_x = target_right_x
                        last_left_bar_x = left_bar_x
                        last_right_bar_x = right_bar_x
                    ...
                    EDGE_TRANSITION_DURATION = self._get_rod_specific_setting("fish_line_edge_transition_duration", 0.25)
                    in_transition = False
    ...
    def _load_config(self):  # AI_TARGET: CONFIG_LOAD
        try:
            ...
            if not encoded:
                print("  ⚠️ Warning: Config file is empty, using defaults")
                return
            ...
            try:
                ...
                print(f"  ⚠️ Warning: Invalid base64 in config file: {e}")
            ...
            if not isinstance(config_data, dict):
                print("  ⚠️ Warning: Config data is not a dictionary, using defaults")
                return
    ...
    def _validate_and_update_settings(self, loaded_settings):
        ...
        validation_rules = {
            ...
            "fish_edge_transition_duration": (0.0, 1.0), "fish_line_edge_transition_duration": (0.0, 1.0),
